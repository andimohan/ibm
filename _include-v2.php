...
// load settings dulu, karena ada sebagian load settings di construction class
...
// nanti akan deprecated, ambil dr database
$taxValueOpt = json_decode($class->loadSetting('ppnValueOpt'));
$taxValue = array();

// biar gk ad warning Invalid argument supplied for foreach()
if(!empty($taxValueOpt)){ 
	foreach($taxValueOpt as $row) $taxValue[$row] = $row;
	define('TAX_VALUE',$taxValue); 
}
...
if (PLAN_TYPE['categorykey'] == COMPANY_TYPE['trucking']){
	//cek ada koneksi account ke TMS atau FF gk
	
	$PARTNER_ACCOUNT['TMS'] = $class->getTMSPartner(); 
	
} 
define('PARTNER_ACCOUNT', $PARTNER_ACCOUNT);
...
function prepareOnLoadDataForm($obj,$showMainCompany=true){
    ...
    echo $obj->inputHidden('reftablekey', array('value' => $obj->getTableKeyAndObj($obj->tableName,array('key'))['key']));  
        ...
        for($j=0;$j<count($arrColumn);$j++){ 
            ...
            switch($format){
                ...
                case 'time':  $content = $obj->formatDbDate($content,'H:i');
                                 break; 
                case 'monthperiod':  $content = $obj->formatDbDate($content,'F Y');
                                 break; 
                case 'datetime':  $content = $obj->formatDbDate($content,'d / m / Y H:i',array('returnOnEmpty' => true));
                                 break;
            }
                
             // gk boleh replace EOL, jd double
             $content = str_replace(array(chr(13)),array('<br>'),(string) $content);
...
function getPerformanceLog($start_time){ 
    ...
    $mem_usage = memory_get_usage() / 1024;

    /* Peak memory usage */
    $mem_peak = memory_get_peak_usage() / 1024;


   $varMemory = 'The script is now using: ' . number_format($mem_usage) . ' KB of memory'.chr(13);
   $varMemory .= 'Peak usage: ' . number_format($mem_peak) . ' KB of memory.'.chr(13);

   $var .= chr(13).$varMemory;

    return array('msg' => $var, 'timeTaken' => $time_taken, 'memoryPeak' => $mem_peak);
}


function preloadSystemSettings(){
	$dbCon = $GLOBALS['oDbCon'];
	 
	//$SYSTEM_SETTINGS = $class->loadSystemSettings(); 
	$sql = 'select _setting.code,_user_setting.value  from _user_setting,_setting where _setting.pkey = _user_setting.settingkey';
 	$rs = $dbCon->doQuery($sql); 

	define('SYSTEM_SETTINGS', array_column($rs,'value','code'));
    
    //CEK LANG DULU
    $userkey = (isset($_SESSION[LOGIN_ADMIN_SESSION]['id']) )  ? base64_decode($_SESSION[LOGIN_ADMIN_SESSION]['id']) : 0;
    
    // load theme settings
    $sql = 'select pkey,settingkey,userkey,value from user_themes_settings
            where userkey = ' .$dbCon->paramString($userkey);
    
    $rsThemes = $dbCon->doQuery($sql);
    define('THEME_SETTINGS', array_column($rsThemes,null,'settingkey'));
 
}

function getJournal($obj,$id){ 
     
    $journal = '<div class="data-card border-pink">
                        <h1>'.ucwords($obj->lang['generalJournal']).'</h1> 
                        <div class="content">
                        '.$obj->compileGeneralJournalForAdminForm($id).'
                        </div>
        </div>';                
...
// DEFINE 
$useSN = ($class->loadSetting('showSerialNumber') == 1) ? true : false;
define('USE_SN', $useSN);

define('SN_SPLIT_REGEX','/[,\s]+/');
?>