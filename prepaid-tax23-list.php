...
includeClass(array("ARPrepaidTax23.class.php","ARPayment.class.php","ARPrepaidTax23Payment.class.php"));
$arPrepaidTax23 = new ARPrepaidTax23();
$arPrepaidTax23Payment = new ARPrepaidTax23Payment();

$arPrepaidTax23->oDbCon = $CUSTOMER_CONN;
$arPrepaidTax23Payment->oDbCon = $CUSTOMER_CONN;
...
$criteria = '';
$criteria .=  ' and '.$arPrepaidTax23->tableName.'.customerkey = '.$class->oDbCon->paramString(USERKEY);

if(isset($_POST['txtSearch']) && !empty($_POST['txtSearch'])){
$key = $_POST['txtSearch'];
$criteria .=  ' and (
						'.$arPrepaidTax23->tableName.'.refcode like '.$class->oDbCon->paramString('%'.$key.'%').' 
					)'
			  ;
}
...
$criteria .=   ' and '.$arPrepaidTax23->tableName.'.statuskey in ('.$class->oDbCon->paramString($statuskeyCriteria,',').')';

$rs = $arPrepaidTax23->searchData('','',true,$criteria,$orderBy,$limit);
$rsPayment = $arPrepaidTax23Payment->getDetailPaymentByARKey(array_column($rs,'pkey'));
$rsPayment = $arPrepaidTax23Payment->reindexDetailCollections($rsPayment,'arkey');

foreach($rs as $key=>$arRow) 
	$rs[$key]['paymentHistory'] = (isset($rsPayment[$arRow['pkey']])) ? $rsPayment[$arRow['pkey']] : array();

$totalPages = ceil( $arPrepaidTax23->getTotalRows($criteria) / $totalrowsperpage);  
    
$arrTwigVar ['totalPages'] =  $totalPages;

$totalOutstanding = $arPrepaidTax23->getAROutstanding(USERKEY);


$arrTwigVar['rsAR'] =  $rs;
$arrTwigVar['totalOutstanding'] =  $totalOutstanding; 

$arrStatus = $arPrepaidTax23->getAllStatus();
array_pop($arrStatus);
...
echo $twig->render('prepaid-tax23-list.html', $arrTwigVar);
