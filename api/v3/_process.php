...
switch($ACTION){
    case 'POST' :  
            ...
            for($i=0;$i<$totalPostVars;$i++){ 
                ...
                $PARAM = array();    
                $errorMessage = array(); 
				$skipedErrorMessage = array();
                $postVars = $arrPostVars[$i];
                  
				
                getDetailsAPIParam($ACTION,$PARAM,$postVars, $API_FIELDS, array(), $errorMessage,$skipedErrorMessage);
                ...
                if(!empty($errorMessage)){  
                    ...
                    try{
                            ...
                            if(!$OBJ->oDbCon->startTrans(true))
								...
								// buat catat jika ad error yang di skip
								// asumsi semua namanya "trDesc"
								if (!empty($PARAM['trDesc'])) $PARAM['trDesc'] .= chr(13).'--------------'.chr(13);

								$PARAM['trDesc'] .= implode(chr(13),$skipedErrorMessage); 
...
//    $ordertype = (isset($_GET['order_type']) && !empty($_GET['order_type']) && $_GET['order_type'] != 1) ? 'asc' : 'desc'; 
            ...
            // default
            $orderby =  $OBJ->tableName.'.pkey';
        
            if(!empty($_GET['order_by'])){
                 
                // harus terdaftar
                if(isset($SORT_COLUMN[$_GET['order_by']]))
                    $orderby = $OBJ->oDbCon->paramOrder($SORT_COLUMN[$_GET['order_by']]); 
              
            } 
        
            $ordertype = (isset($_GET['order_type']) && in_array($_GET['order_type'], array(0,1))) ? 'asc' : 'desc'; 
            ...
            // kalo parameter pasti pkey / kode
        
            if (isset($_GET['key']) && !empty($_GET['key'])) 
                array_push($criteria, $OBJ->tableName.'.pkey in('.$OBJ->oDbCon->paramString(explode(',',$_GET['key']),',').')');  
          
        
            if (isset($_GET['code']) && !empty($_GET['code']))  
                array_push($criteria, $OBJ->tableName.'.code in('.$OBJ->oDbCon->paramString( explode(',',$_GET['code']),',').')'); 
            ...
            foreach($_GET as $key => $searchBy){
                ...
                // kalo yg dikirim tipe array
                
                $searchField = $searchDataSet[$key]['search']['field'];
                
                if (!is_array($searchField)) $searchField = array($searchField);
                
                $fieldOR = array();
                foreach($searchField as $fieldRow) 
                    array_push($fieldOR, $fieldRow . ' like ' .$OBJ->oDbCon->paramString('%'.$searchBy.'%'));
                
                $fieldOR = implode(' OR ' , $fieldOR);
                
                array_push($criteria, ' ('.$fieldOR.') ');  
            }
