...
function getAPIValue($postVars,$row, $dataset, &$errorMessage, &$skipedErrorMessage=array()){ 
    global $class;
    ...
    if(isset($row['ref']) && !empty($row['ref'])){
        ...
        if (isset($row['ref']['obj'])  && !empty($row['ref']['obj'])){
            ...
            //$refObj->setLog( '  >>  and '.$refField .'='.$refObj->oDbCon->paramString($value)   ,true);
		...
		if ( isset($row['mandatory']) && $row['mandatory'] && empty($value)){ 
			...
			$errMsg = $tempName.'. ' . $class->errorMsg[213];
			
			if (isset($row['skipOnNotFound']) && $row['skipOnNotFound']){
				array_push($skipedErrorMessage, $errMsg);   
			}else{
				array_push($errorMessage, $errMsg);  
			}
		}
			
		
//		if(isset($row['skipOnNotFound']) && $row['skipOnNotFound']){
//			array_push($skipedErrorMessage, $tempName.'. ' . $class->errorMsg[213]);   
//		}else{
//			if ( isset($row['mandatory']) && $row['mandatory'] && empty($value)) 
...
function getDetailsAPIParam($ACTION, &$PARAM,$postVars,$apiFields, $currAPIFieldsRow = array(), &$errorMessage = array(),&$skipedErrorMessage = array()){ 
    global $OBJ;
        ...
        if(in_array($key,$arrExcludeField)) continue;
         ...
         if (isset($row['dataset'])){  
            ...
            foreach($values as $valueRow)
                getDetailsAPIParam($ACTION, $PARAM,$valueRow,$row['detail'], $row ,$errorMessage, $skipedErrorMessage);
            ...
            // untuk data header
            if(empty($currAPIFieldsRow)){ 
                $value = getAPIValue($postVars,$row, $OBJ->arrData[$key], $errorMessage,$skipedErrorMessage);  
                ...
                $value = ($key == 'pkey' && $ACTION == 'POST') ? 0 : getAPIValue($postVars, $row, $dataset[$key], $errorMessage,$skipedErrorMessage);  
                
				// gk bisa, jadinya detailnya gk sama total barisnya
//				if(isset($row['skipOnNotFound']) && $row['skipOnNotFound'] && !empty($value))
...
function updatingPkeyAndPostVars($OBJ,&$arrPostVars,$API_FIELDS){
    ...
    if(!empty(($pkey))) {
     ...
     if(!empty($rs)){ 
		 ...
		 // harus plus karena dalam satu payload, bisa ada beberapa variasi, kirim pkey, code atau request_id
