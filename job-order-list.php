...
$endUserStatusName = array('2' => $class->lang['onDelivery'],
						   '3' => $class->lang['documentCollection'],
						   '4' => $class->lang['documentCollection'],
						   '5' => $class->lang['documentCollection'],
						   '6' => $class->lang['invoiced']);
...
$orderBy = ' order by trdate desc,pkey desc,consigneename desc, stuffinglocationkey desc'; 
...
for($i=0;$i<count($rs);$i++){ 
    ...
    $joKey =$rs[$i]['pkey'];
    $spkRow = (isset($rsSPKCol[$joKey])) ? $rsSPKCol[$joKey] : array();
	...
	$vehicleNumber = array_filter(array_column($spkRow, 'vehicle'));
    $rs[$i]['vehiclenumber']  =  implode(', ',$vehicleNumber); 
	
	$rs[$i]['gpsdetail'] = array();
	foreach($spkRow as $row){
		array_push( $rs[$i]['gpsdetail'], array('wokey' => $row['pkey'] ,
			'spkstatuskey' => $row['statuskey'],
			'vehiclenumber'  => $row['vehicle'] ,
			'vehicleid' => str_replace(' ','',$row['vehicle']) ));
	}
