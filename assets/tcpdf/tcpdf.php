...
class TCPDF {
	...
	public function setPageOrientation($orientation, $autopagebreak='', $bottommargin='') {
		...
		$this->w = (!empty($this->k)) ? ($this->wPt / $this->k) : 0;
		$this->h = (!empty($this->k)) ? ($this->hPt / $this->k) : 0;
	...
	public function Header() {
		if ($this->header_xobjid === false) {
            ...
            $this->SetY((2.835 / $this->k) + max($imgy, $this->y)  + 10);
	...
	public function SetFontSize($size, $out=true) {
		...
		$this->FontSize = (!empty( $this->k )) ? $size / $this->k : 0;
		...
		$this->FontAscent = (!empty($this->k)) ? ($font_ascent / $this->k) : 0;
		$this->FontDescent = (!empty($this->k)) ? ($font_descent / $this->k) : 0;
			...
			try {
				...
				for ($xpx = 0; $xpx < $wpx; ++$xpx) {
					for ($ypx = 0; $ypx < $hpx; ++$ypx) {
						...
						imagesetpixel($imgalpha, (int)round($xpx), (int)round($ypx), (int)round($alpha));
					}
				}
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					//while (list($id, $name) = each($attr_array[1])) { // <-- deprecated
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						//while (list($id, $name) = each($style_array[1])) { // <-- deprecated
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	protected function setFontBuffer($font, $data) {
		...
		if (!in_array($font, (array) $this->fontkeys)) {
			$this->fontkeys[] = $font;
	...
	*/
	 ...
	 * @since 5.3.002 (2010-06-07)
	 */
	public function setFontSubsetting($enable=false) {
		if ($this->pdfa_mode) {
