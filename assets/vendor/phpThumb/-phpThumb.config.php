...
if (defined('PHPT_CONFIG_INCLUDED')) {
    return;
}
define('PHPT_CONFIG_INCLUDED', true);
...
$PHPTHUMB_CONFIG['high_security_password']      =  'oX`81ms85&{9sH!C$cIO';     // required if 'high_security_enabled' is true, and must be at complex (uppercase, lowercase, numbers, punctuation, etc -- punctuation is strongest, lowercase is weakest; see PasswordStrength() in phpthumb.functions.php). You can use a password generator like http://silisoftware.com/tools/password-random.php to generate a strong password
...
$PHPTHUMB_CONFIG['error_message_image_default'] = '/include/img/no-image.jp2';       // Set this to the name of a generic error image (e.g. '/images/error.png') that you want displayed in place of any error message that may occur. This setting is overridden by the 'err' parameter, which does the same thing.
...
function phpThumbURL($ParameterString, $path_to_phpThumb='/phpThumb/phpThumb.php') {
	...
	$returnVal = $path_to_phpThumb.'?'.$ParameterString.$PHPTHUMB_CONFIG['high_security_url_separator'].'hash='.hash_hmac('sha256', $ParameterString, $PHPTHUMB_CONFIG['high_security_password']);
	setLog($returnVal,true);
	return $returnVal;
} 

function getPHPThumbHash($ParameterString){
  global $PHPTHUMB_CONFIG;
  return hash_hmac('sha256', (string) ($ParameterString ?? '') , $PHPTHUMB_CONFIG['high_security_password']);
}
