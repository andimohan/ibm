...
if (!empty($phpThumb->config_high_security_enabled)) { 
	...
	// manual modified agar bisa compatible
	// gk bisa pke bawaan phpThumbUrl karena beberapa case, pathnya blm ketauan, jd hanya bisa pake filename
	$queryString =  $_SERVER['QUERY_STRING'];
	$temp = explode ('/',$queryString);
	$temp = $temp[count($temp)-1];
	$temp = explode('&',$temp); 

	$parameter = $temp[0].$phpThumb->config_high_security_password;
	...
	} elseif ($_GET['hash'] != md5($parameter)){ 
