...
//$img =  HTTP_HOST.'phpthumb/phpThumb.php?src='.$obj->phpThumbURLSrc.'setting/companyLogo/'.$profileImg.'&h=70&hash='.getPHPThumbHash($profileImg);
    
$arrRecipient = array();
array_push($arrRecipient, '<b>'.$rs[0]['recipientname'].'</b>', str_replace(chr(13),'<br>',$rs[0]['recipientaddress']), $rs[0]['recipientphone']);
    
$logo = '';
if($rs[0]['customcodekey'] == 8000){
$logo = '<img src="'.HTTP_HOST.'template/wintera.co.id/img/usc.jpg" style="width: 120px">';
}

$imgSignature = ($rs[0]['customcodekey'] == 8000) ? '' : HTTP_HOST.'template/wintera.co.id/img/ttd-noncap.png';    
...
$top= ($rs[0]['customcodekey'] == 8000) ? '' : '<tr><td style="width:150px">Term of Payment (days)</td><td style="width:20px;">:</td><td style="width:120px;text-align:right;">'.$topSaid.'</td></tr>  ';
...
$cellArray = array ();
array_push($cellArray, array('label' => ($rs[0]['customcodekey'] == 8000) ? ' Description' : 'Services')); 
...
for ($i=0;$i<count($rsDetail);$i++){  
  $itemName = ($rs[0]['customcodekey'] == 8000)  ? '' : $rsDetail[$i]['itemname'];
  $detailDesc = (!empty($rsDetail[$i]['trdesc'])) ? '<br><i>'.str_replace(chr(13),'<br>',$rsDetail[$i]['trdesc']).'</i>' : '';
  $html .= '<tr><td style="">'.$itemName.''.$detailDesc.'</td><td style="text-align:center;">'.$obj->formatNumber($rsDetail[$i]['qty']).'</td><td style="text-align:right;">'.$obj->formatNumber($rsDetail[$i]['priceinunit']).'</td><td style="text-align:right;">'.$obj->formatNumber($rsDetail[$i]['total']).'</td></tr>' ; 
}
$html .= '</table>' ;
...
if($rs[0]['customcodekey'] == 8000){
    $bankName = '<tr><td style="width:100px">Account Name</td><td style="width:10px">:</td><td style="width:200px">PT. Unggul Sukses Cemerlang</td></tr>
<tr><td style="width:100px">Bank</td><td style="width:10px">:</td><td style="width:180px">Bank Central Asia</td></tr>
<tr><td style="width:100px">Bank Account</td><td style="width:10px">:</td><td style="width:180px">083 8391 888</td></tr>';
}else{
      $bankName = '<tr><td style="width:100px">Account Name</td><td style="width:10px">:</td><td style="width:200px">Martin Halim Kusuma</td></tr>
<tr><td style="width:100px">Bank</td><td style="width:10px">:</td><td style="width:180px">Bank Central Asia</td></tr>
...
<tr> 
<td style="height:30px; width:150px; text-align:center;">Jakarta, '.$obj->formatDBDate($rs[0]['trdate'],'d F Y').'</td></tr>
<tr><td style="height:80px;text-align:center;"><img src="'.$imgSignature.'" /></td></tr>
<tr> 
