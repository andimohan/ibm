...
if (isset($_POST) && !empty($_POST['hidAction'])){   
    ...
    $orderBy = (!empty($_POST['hidOrderBy'])) ? $obj->oDbCon->paramOrder($_POST['hidOrderBy']) : $obj->tableSupplier.'.name'; // order by harus dr kolom yg terdaftar saja
	...
	$rsStartBalance = $obj->searchAPCard($startDate,$criteria,$order);
	
	$startingBalance = array();
	foreach($rsStartBalance as $key=>$row){
		$supplierkey = $row['supplierkey'];
		if(!isset($startingBalance[$supplierkey])) $startingBalance[$supplierkey]['startingbalance'] = 0;
		$startingBalance[$supplierkey]['startingbalance'] += $row['outstanding'];
	}
	
	
	$rs = $supplier->searchDataRow(array($supplier->tableName.'.pkey as supplierkey',$supplier->tableName.'.name as suppliername'),
										   ' and '.$supplier->tableName.'.statuskey in (1) ' . $supplierCriteria
										  ); 
		

	$arrSupplierKey = (isset($_POST['selSupplier'])) ? $_POST['selSupplier'] : array();
 	$rsHistory = $obj->getSupplierAPCard($arrSupplierKey,$cardCriteria);
	...
	$rsHistory = $obj->reindexDetailCollections($rsHistory,'supplierkey');
        
	$rsPayment = $apPayment->getPaymentMethodDetail($arrPaymentKey);
    ...
    for( $i=0;$i<$totalRs;$i++) {   
		$supplierkey = $rs[$i]['supplierkey'];
       	...
       	if (!isset($rsHistory[$supplierkey]))  continue;
        $rsDetail =  $rsHistory[$supplierkey] ; 


		// buat header sama starting balance
		$currencyName = $rsDetail[0]['currencyname'];
            
        $balance = (isset($startingBalance[$supplierkey])) ? $startingBalance[$supplierkey]['startingbalance'] : 0; // nanti perlu lihat saldo awal
...
echo $twig->render('@custom/reportAPCardCommissionSupplier.html', $arrTwigVar);   
?>