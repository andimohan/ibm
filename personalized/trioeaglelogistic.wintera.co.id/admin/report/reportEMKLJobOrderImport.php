...
$securityObject = 'reportSalesOrderImportFF'; // the value of security object is manually inserted to handle 
								  // some modules that have different security object from that of their class
								  
if(!$security->isAdminLogin($securityObject,10,true));  
$_POST['hidTotalFreezeCol'] = 3;

// karena personalized, harus pake ini
$arrTwigVar['INPUT_HID_TOTAL_FREEZE_COL'] = $class->inputHidden('hidTotalFreezeCol');
...
$arrDataStructure['totalBuying'] = array('title'=>ucwords($obj->lang['totalBuying']),'dbfield' => 'totalbuying','align'=>'right', 'width'=>"120px",'format'=>'number','sortable'=>false,'calculateTotal' => true); 
//$arrDataStructure['adminFee'] = array('title'=>ucwords($obj->lang['adminFee']),'dbfield' => 'totalemployeecommission','align'=>'right', 'width'=>"120px",'format'=>'number','calculateTotal' => true); 
...
$arrDataStructure['prepaidTax23'] = array('title'=>ucwords($obj->lang['prepaidTax23']),'dbfield' => 'payabletax23statusname', 'width'=>"100px"); 
$arrDataStructure['note'] = array('title'=>ucwords($obj->lang['note']), 'width'=>"300px",'dbfield' => 'trdesc');
$arrDataStructure['status'] = array('title'=>ucwords($obj->lang['status']),'dbfield' => 'statusname', 'width'=>"100px");
		   
$arrHeaderTemplate = array();
$arrHeaderTemplate['reportTitle'] = $obj->lang['salesOrderImportReport']; 
...
$arrTwigVar['inputMblNumber'] =  $class->inputText('mblNumber');$arrTwigVar['inputSelContainer'] =  $class->inputSelect('selContainer[]', $arrContainer, array('etc' => 'multiple="multiple"', 'class' => 'multi-selectbox'));  
...
/*        if(isset($_POST) && !empty($_POST['selCurrency'])) { 
	...
	// Admin Fee
//	$rsAPEmployeeCommission = $employeeCommission->searchDataRow(array($employeeCommission->tableName.'.pkey',$employeeCommission->tableName.'.refkey',$employeeCommission->tableName.'.amount'),
//																' and '.$employeeCommission->tableName.'.reftabletype = '.$class->oDbCon->paramString($tableKey).'
//																  and '.$employeeCommission->tableName.'.refkey in ('.$class->oDbCon->paramString($arrKeys,',').')'
//																);
//	
//	$rsAPEmployeeCommission = array_column($rsAPEmployeeCommission,null,'refkey');
//		
    // utk LCL
    ...
    //prepaid tax 23 status
    $rsARPrepaidTax23 = $obj->getARPrepaidTax23ByJO($arrKeys);
    $rsARPrepaidTax23Information = $obj->reindexDetailCollections($rsARPrepaidTax23, 'refsalesorderheaderkey');
    $totalRs = count($rs);
    for( $i=0;$i<$totalRs;$i++) {    
        $sokey = $rs[$i]['pkey'];
        ...
        $rs[$i]['code'] = '<a href="/admin/print/emklJobOrderImport/'.$rs[$i]['pkey'].'" target="_blank">'.$rs[$i]['code'].'</a>';
        $rs[$i]['totalinvoiced'] = 0;
        $rs[$i]['totalbeforetaxinvoiced'] = 0;
        $rs[$i]['totaltax23value'] = 0;
		$rs[$i]['totalbuying'] += $rs[$i]['totalemployeecommission'];
        $arrStatus = array();  
        $arrInvoiceCode = array();
        
        $hasOutstanding = false;

        $arrARPrepaidTax23Status = array();
        $hasOutStandingPayableTax23 = false;

        $prepaidTaxMap = [];
        if (!empty($rsARPrepaidTax23Information[$sokey])) {
            foreach ($rsARPrepaidTax23Information[$sokey] as $taxRow) {
                $invoiceKey = $taxRow['invoicekey']; 
                $prepaidTaxMap[$invoiceKey][] = [
                    'statusname' => $taxRow['statusname']
                ];

                if(in_array($taxRow['statuskey'],array(1,2)))
                    $hasOutStandingPayableTax23 = true;
            }
        }
        ...
        if(isset($rsInvoiceInformation[$sokey])){
            foreach($rsInvoiceInformation[$sokey] as $invoiceRow){ 
                ...
                $invoiceKey = $invoiceRow['pkey']; 
                if (isset($prepaidTaxMap[$invoiceKey])) {
                    $statuses = array_column($prepaidTaxMap[$invoiceKey], 'statusname');
                    $taxStatus = implode(', ', $statuses);
                } else {
                    $taxStatus = '&nbsp;';
                }
                $arrARPrepaidTax23Status[] = $taxStatus;
        ...
        $arrHeaderStyle['payabletax23statusname']['textColor'] = ($hasOutStandingPayableTax23) ? 'C41E3A' : '568203';
        ...
        $rs[$i]['payabletax23statusname'] = implode('<br>',$arrARPrepaidTax23Status);
        $containertype = $rs[$i]['loadcontainertypekey'];
        
        $arrContainerQty = $rsContainerQty[$rs[$i]['pkey']];
        //$obj->setLog($arrContainerQty,true);
        
        $totalBuying = $rs[$i]['totalcommission'] + $rs[$i]['totalbuying'] + $rs[$i]['totalemployeecommission'] - $rs[$i]['totalcreditnote'] + $rs[$i]['totaldebitnote'] ;
        ...
        $return = $obj->formatReportRows(array('data' => $rs[$i], 'style' => $arrHeaderStyle,'totalFreezeCol' => $_POST['hidTotalFreezeCol']),$arrTemplate);
...
echo $twig->render('reportEMKLJobOrderImport.html', $arrTwigVar);  
