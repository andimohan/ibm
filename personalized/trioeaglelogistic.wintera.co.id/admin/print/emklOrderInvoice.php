...
$pdf->setCustomSettings(
    array( 
         ...
         'footer' => '', //'<div style="text-align:center; font-size:2em; font-weight: bold">ORIGINAL</div>',
         'fontName' => 'Courier'
         ) 
);
...
$carrierName = (!empty($rsJobOrder[0]['carrieraliasname'])) ? $rsJobOrder[0]['carrieraliasname'] : $rsJobOrder[0]['carriername'] ;
...
// dipindah kan keatas, agar ketika pembulatan, said amountnya benar    
$grandTotal = $obj->formatNumber($rs[0]['grandtotal'],$currencyDecimal,'.',','); 
    
// format dan unformat ulang, biar angkayna sama dengana grandotoal diatas
$sayNumber = $obj->sayNumberInEnglish(  $obj->unformatNumber( $obj->formatNumber($rs[0]['grandtotal'],$currencyDecimal))  ); 
...
$useContainer = ($rsJobOrder[0]['loadcontainertypekey']==EMKL['container']['lcl']) ? false:true;
...
<div style="font-size:2em; font-weight:bold">INVOICE'.$draft.'</div><br>
...
<td style="border:1px solid #333;"><div style="text-align: center;">' . $rsJobOrder[0]['code']. '</div></td>
...
<td style="border:1px solid #333;"><div style="text-align: center; ">'. $rsJobOrder[0]['vesselname'] . ' '. $rsJobOrder[0]['vesselnumber'] . '<br>'. $carrierName.'</div></td>
...
array_push($cellArray, array('label' => 'Qty', 'align' => 'center', 'width' => '70'));
array_push($cellArray, array('label' => 'Price', 'align' => 'right', 'width' => '130'));
array_push($cellArray, array('label' => 'Cur', 'align' => 'center', 'width' => '50'));
array_push($cellArray, array('label' => 'Rate', 'align' => 'center', 'width' => '90'));
array_push($cellArray, array('label' => 'Amount', 'align' => 'right', 'width' => '130'));
...
for($i=0;$i<count($rsDetail);$i++){ 
    ...
    for($j=0; $j<count($rsInvoiceDetail);$j++){ 
        ...
        $rsContainer = $jobOrder->getItemDetail( $rsDetail[$i]['salesorderkey'] , $rsInvoiceDetail[$j]['refsodetailkey']);  
        $containerName = ($rsInvoiceDetail[$j]['iscontainer']==1 && $useContainer) ? $rsContainer[0]['containername'] : '';
         
        $itemname = (!empty($rsInvoiceDetail[$j]['aliasname'])) ? $rsInvoiceDetail[$j]['aliasname'] : $rsInvoiceDetail[$j]['itemname']. ' '.$containerName; 
...
$taxPercentage = (date("Y", strtotime($rs[0]['trdate'])) <= 2024) ?  ' ' .$obj->formatNumber($rs[0]['taxpercentage'],-2).'%' : '';
...
if ($rs[0]['taxvalue'] != 0){
    ...
    array_push($arrSubtotal, '<tr><td colspan="3" style="border:1px solid #333; text-align:right; font-weight:bold;">'.strtoupper($obj->lang['tax']).$taxPercentage.'</td><td style="border:1px solid #333; text-align:center">'.$rsCurrency[$rs[0]['currencykey']].'</td><td style="border:1px solid #333;"></td><td style="border:1px solid #333; text-align:right;"  >'.$obj->formatNumber($rs[0]['taxvalue'],$currencyDecimal,'.',',').'</td></tr>');
...
$html .= '
    <tr>
        ...
        <td style="border:1px solid #333; text-align:right">'.$grandTotal.'</td> 
    </tr>';
...
$html .= '<div style="clear:both;"></div>
    ...
    if($rs[0]['taxvalue'] != 0) { 
        ...
        $html .='<br><br><table cellpadding="2" style="border:1px solid #333; font-size:0.9em">';
        $html .='<tr><td style="width: 20px">1.</td><td  style="width: 650px">
Harap Invoice dan faktur pajak ini dicek dan dikonfirmasi, permintaan revisi lebih dari 5 hari kerja dari tgl invoice dan faktur pajak tidak dapat diproses.</td></tr>';
        $html .='<tr><td  style="width: 20px">2.</td><td style="width: 650px">Invoice sudah mengenakan bea materai sebagaimana diatur  dalam Undang-undang No.10 tahun 2020.</td></tr>
         </table>';
