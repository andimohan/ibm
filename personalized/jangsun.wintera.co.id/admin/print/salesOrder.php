...
$employeeKey =  base64_decode($_SESSION[$obj->loginAdminSession]['id']);  
$rsSales = $employee->getDataRowById($employeeKey); 
$signatureFile = $rsSales[0]['signaturefile'];
$imgSignature =  HTTP_HOST.'phpthumb/phpThumb.php?src='.$obj->phpThumbURLSrc.'employee-signature/'.$employeeKey.'/'.$signatureFile.'&w=200&h=100&hash='.getPHPThumbHash($signatureFile);
$confirmedName = $rsSales[0]['name']; 
    
	
$customerkey = $rs[0]['customerkey'];
$rsCustomer = $customer->searchDataRow(array($customer->tableName.'.name',$customer->tableName.'.address',$customer->tableName.'.phone'),
									   ' and ' . $customer->tableName.'.pkey = ' . $obj->oDbCon->paramString($customerkey));
...
if(!empty($rs[0]['recipientname'])){
	...
	array_push($arrRecipient, '<b>'.$rsCustomer[0]['name'].'</b>');
	if(!empty($rsCustomer[0]['address'])) array_push($arrRecipient, $rsCustomer[0]['address']);
...
<tr><td style="width:100px">Account Name</td><td style="width:10px">:</td><td style="width:200px">PT Jangsun Cakrawala Indonesia</td></tr>
<tr><td style="width:100px">Bank</td><td style="width:10px">:</td><td style="width:180px">Bank Permata Cab. Banjarbaru</td></tr>
<tr><td style="width:100px">Bank Account</td><td style="width:10px">:</td><td style="width:180px">9807488802</td></tr>
</table>
</td> 
...
<tr><td></td><td style="height:30px;text-align:center;"><img src="'.$imgSignature.'" width="60px" /></td></tr>
<tr>
<td></td>
