...
includeClass(array('Item.class.php','PricingCategory.class.php'));
...
$pricingCategory = createObjAndAddToCol(new PricingCategory());

$color = createObjAndAddToCol(new Color());
...
$rsItemModelDetail = array();
$rsItemTextureDetail = array();
$rsItemColorDetail = array();
$rsItemCharacterDetail = array();
$rsItemVariationDetail = array();
$rsItemSKUAlias = array();
...
$_POST['selWeightUnit'] = UNIT['gram']; 

$marketplaceObjs = ($isActiveMarketplace) ? $marketplace->getMarketplaceObj() : array();
...
if (!empty($_GET['id'])){ 
    ...
    $rsDetailVideo = $obj->getDetailVideo($id);
    $rsSyncMarketplace = $obj->getItemSyncMarketplace($id);
    $rsSyncMarketplace = array_column($rsSyncMarketplace,null,'marketplacekey');
    $rsItemModelDetail = $obj->getItemModelDetail($id);
    $rsItemTextureDetail = $obj->getItemTextureDetail($id);
    $rsItemSKUAlias = $obj->getDetailItemSKUAlias($id);
    $rsItemColorDetail = $obj->getItemColorDetail($id);
    $rsItemCharacterDetail = $obj->getItemCharacterDetail($id);
    $rsItemVariationDetail = $obj->getItemVariationDetail($id);
	...
	$_POST['qtyOnHand'] = $obj->formatNumber($itemMovement->getItemQOH($rs[0]['pkey'],$rsWarehouseQOHCount)); 
	...
	$_POST['marginPercentage'] = $obj->formatNumber($rs[0]['marginpercentage']);
    ...
    $_POST['carat'] = $obj->formatNumber($rs[0]['carat'],2);     
    
    // $_POST['hidColorKey'] = $rs[0]['colorkey'];
    // if(!empty($rs[0]['colorkey'])) {
    //     $rsColor = $color->getDataRowById($rs[0]['colorkey']);
    //     $_POST['colorName'] = $rsColor[0]['name'];
    // }

    $_POST['hidPlatingKey'] = $rs[0]['platingkey'];
    if(!empty($rs[0]['platingkey'])) {
        $rsPlating = $plating->getDataRowById($rs[0]['platingkey']);
        $_POST['platingName'] = $rsPlating[0]['name'];
    }

    // $_POST['hidTextureKey'] = $rs[0]['texturekey'];
    // if(!empty($rs[0]['texturekey'])) {
    //     $rsTexture = $texture->getDataRowById($rs[0]['texturekey']);
    //     $_POST['textureName'] = $rsTexture[0]['name'];
    // }

    $_POST['hidMaterialKey'] = $rs[0]['materialkey'];
    if(!empty($rs[0]['materialkey'])) {
        $rsMaterial = $material->getDataRowById($rs[0]['materialkey']);
        $_POST['materialName'] = $rsMaterial[0]['name'];
    }

    // $_POST['hidModelKey'] = $rs[0]['modelkey'];
    // if(!empty($rs[0]['modelkey'])) {
    //     $rsModel = $model->getDataRowById($rs[0]['modelkey']);
    //     $_POST['modelName'] = $rsModel[0]['name'];
    // }

    $_POST['hidRingSizeKey'] = $rs[0]['ringsizekey'];
    if(!empty($rs[0]['ringsizekey'])) {
        $rsRingSize = $ringSize->getDataRowById($rs[0]['ringsizekey']);
        $_POST['ringSizeName'] = $rsRingSize[0]['name'];
    }


    $_POST['hidAgeKey'] = $rs[0]['agekey'];
    if(!empty($rs[0]['agekey'])) {
        $rsItemAge = $age->getDataRowById($rs[0]['agekey']);
        $_POST['ageName'] = $rsItemAge[0]['name'];
    }

    $_POST['selSexKey'] = $rs[0]['sexkey'];

    // $_POST['hidCharacterKey'] = $rs[0]['characterkey'];
    // if(!empty($rs[0]['characterkey'])) {
    //     $rsCharacter = $character->getDataRowById($rs[0]['characterkey']);
    //     $_POST['characterName'] = $rsCharacter[0]['name'];
    // }
    $_POST['size'] = $obj->formatNumber($rs[0]['size']);
    $_POST['selSizeUnitKey'] = $rs[0]['sizeunitkey'];

    $_POST['aliasName'] = $rs[0]['aliasname'];
    ...
    $editInactivePricingCategory = ' or '.$pricingCategory->tableName.'.pkey = ' . $obj->oDbCon->paramString($rs[0]['pricingcategorykey']); 
...
/*    if ($showItemFilter) { 
	...
	$qrResult = $obj->createQR(DOMAIN_NAME.'/products-detail/'. $obj->URLFilter($rs[0]['name']).'/'. $rs[0]['pkey'],12);
			 
} 


if(in_array(PLAN_TYPE['categorykey'], array(COMPANY_TYPE['jewelry']))){  
    $arrPricingCategory  = $pricingCategory->generateComboboxOpt(null,array('criteria' => ' and ('.$pricingCategory->tableName.'.statuskey = 1' .$editInactivePricingCategory.')'),'-----');  
}
...
$arrUnitNonWeight = $itemUnit->generateComboboxOpt(null,array('criteria' =>' and '.$itemUnit->tableName.'.unittype = 1 and  ('.$itemUnit->tableName.'.statuskey = 1 ' . $editUnitInactiveCriteria. ')')); 
$arrCondition = $itemCondition->generateComboboxOpt(null,array('criteria' =>' and ('.$itemCondition->tableName.'.statuskey = 1 ' . $editConditionInactiveCriteria. ')')); 
...
.marketplace-attributes li {width: 100%;   display: inline-block; padding: 0.2em 1em}
.shipment-flex > div:first-child{margin-left: 0.5em}
.shipment-flex > div {margin: 0 0.5em}
</style>  
...
<script type="text/javascript">   
  ...
  jQuery(document).ready(function(){  
        ...
        opt.hasActiveMarketplace =  <?php echo ($isActiveMarketplace && $obj->hasActiveMarketplace()) ? 'true' : 'false'; ?>;  

        opt.rsItemModelDetail = <?php echo json_encode($rsItemModelDetail); ?>;
        opt.rsItemTextureDetail = <?php echo json_encode($rsItemTextureDetail); ?>;
        opt.rsItemSKUAliasDetail = <?php echo json_encode($rsItemSKUAlias); ?>;
        opt.rsItemColorDetail = <?php echo json_encode($rsItemColorDetail); ?>;
        opt.rsItemCharacterDetail = <?php echo json_encode($rsItemCharacterDetail); ?>;
        opt.rsItemVariationDetail = <?php echo json_encode($rsItemVariationDetail); ?>;
        ...
        <?php
        if(isset($_GET) && !empty( $_GET['shortcut'])){
                $refItemKey = $_GET['refkey'];

                $rsItemSC = $obj->searchDataRow(array($obj->tableName.'.pkey',$obj->tableName.'.code'), 
                            ' and ' .$obj->tableName.'.pkey = ' . $obj->oDbCon->paramString($refItemKey).' ');
                         
                if(!empty($rsItemSC)){ 
                    echo  'getTabObj().getDataForDuplicated('.$rsItemSC[0]['pkey'].');';
                }
         }
                    ...
                    <div class="div-table-col">  
                  		   	<div class="div-tab-panel">  
                                    ...
                                    <div class="form-group">
									 ...
									 <div class="form-group">
										<label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['code']. ' / '.$obj->lang['barcode']); ?></label> 
										<div class="col-xs-9"> 
											<div class="flex">
												<div class="consume"><?php echo $obj->inputAutoCode('code'); ?></div>
												<div>/</div>
												<div class="consume"><?php echo $obj->inputText('barcode',array('add-class' => 'mnv-barcode-input')); ?></div>
											</div>
										</div> 
									 </div>
                                    ...
                                    <?php if(in_array(PLAN_TYPE['categorykey'], array(COMPANY_TYPE['jewelry']))) { ?> 
                                    <div class="form-group">
                                        <label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['pricingScheme']); ?></label> 
                                        <div class="col-xs-9"> 
                                            <?php echo  $obj->inputSelect('selPricingCategory', $arrPricingCategory); ?>
                                        </div> 
                                    </div>    
                                    <div class="form-group additional-price">
                                        <label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['additionalPrice']); ?></label> 
                                        <div class="col-xs-9"> 
                                            <?php echo  $obj->inputNumber('additionalPrice'); ?>
                                        </div> 
                                    </div>   
                                    <?php } ?> 
                                        <div class="form-group">
                                            ...
                                            <div class="col-xs-9"> 
                                                <div class="flex">
                                                     <div><?php echo $obj->inputNumber('sellingPrice',array('readonly' => !$hasUpdatePriceAccess)); ?></div>
                                                     <div style="margin-left: 2em">Modal</div>
                                                     <div><?php echo $obj->inputNumber('cogs', array('readonly' => true)); ?></div>
                                                </div>
                                            </div> 
                                        </div>   
...
-->
									...
									<?php if ($metaOptions == 1){ ?> 
										<div class="form-group text-muted">
											<label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['metaDescription']); ?></label> 
											<div class="col-xs-9"> 
...
-->
									<?php if ($useAutoSellingPrice > 0) { ?> 
									<div class="col-xs-6"> 
                                        <div class="form-group">					
										<label class="col-xs-12 control-label"><?php echo ucwords($obj->lang['margin']); ?> (%)</label> 
                                        <div class="col-xs-12" > 
											 <?php echo $obj->inputNumber('marginPercentage'); ?> 
                                        </div> 
                                        </div> 
                                    </div>   
									<?php } ?>
...
-->
                        ...
                        <div class="div-table-caption border-black"><?php echo ucwords('SKU Alias'); ?></div> 
                                <div class="form-group"> 
                                    <div class="col-xs-12">  
                                        <div class="div-table mnv-sku-alias transaction-detail" style="width: 100%">
                                        <?php 
                                            $totalRows = count($rsItemSKUAlias);
                                            for ($i=0;$i<=$totalRows; $i++){ 
                                                ...
                                                if ($i == $totalRows ){
                                                    $class = 'sku-alias-row-template row-template';
                                                    $overwrite = false;
                                                    $disabled = true; 
                                                    $isLocked = false;
                                                    //$style = 'style="display:none"';
                                                } else{ 
                                                    $_POST['hidSKUAliasDetailKey[]'] =  $rsItemSKUAlias[$i]['pkey'];
                                                    $_POST['skuAlias[]'] =  $rsItemSKUAlias[$i]['skualias'];
                                            ...
                                            <div class="div-table-row  odd-style-adjustment <?php echo $class; ?> "> 
                                                <div class="div-table-col"> 
                                                    <div class="flex">     
                                                        <div class="consume">
                                                            <?php echo $obj->inputHidden('hidSKUAliasDetailKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled, 'etc' => 'placeholder="http://"')); ?>
                                                            <?php echo $obj->inputText('skuAlias[]', array('overwritePost' => $overwrite ,'readonly' => $readonly, 'disabled' => $disabled)); ?>
                                                        </div>
                                                        <div class="icon-col <?php echo $obj->hideOnDisabled(); ?>"><?php echo $obj->inputLinkButton('btnAddDetailRow' , '<i class="fas fa-plus-circle"></i>', array('class' => 'btn btn-link add-row-button', 'etc' => 'attr-template="sku-alias-row-template"')); ?></div>
                      ...
                      <?php if ($showItemImage) { ?>
                        ...
                        <div class="div-table-caption border-black"><?php echo ucwords($obj->lang['video']); ?></div> 
                                <div class="form-group"> 
                                    <div class="col-xs-12">  
                                        <div class="div-table mnv-transaction transaction-detail" style="width: 100%">
                                        <?php 
                                            $totalRows = count($rsDetailVideo);
                                            for ($i=0;$i<=$totalRows; $i++){ 
                                                ...
                                                if ($i == $totalRows ){
                                                    ...
                                                    $_POST['hidDetailVideoKey[]'] =  $rsDetailVideo[$i]['pkey'];
                                                    $_POST['url[]'] =  $rsDetailVideo[$i]['url'];
                                            ...
                                            <div class="div-table-row  odd-style-adjustment <?php echo $class; ?> "> 
                                                <div class="div-table-col"> 
                                                    <div class="flex">     
                                                        <div class="consume">
                                                            <?php echo $obj->inputHidden('hidDetailVideoKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled, 'etc' => 'placeholder="http://"')); ?>
                                                            <?php echo $obj->inputText('url[]', array('overwritePost' => $overwrite ,'readonly' => $readonly, 'disabled' => $disabled , 'etc' => 'placeholder="http://"')); ?>
                                                        </div>
      ...
      <!--                  <?php /* if ($showVendorPartNumber) { */?> 
                         ...
                         <?php /* }*/ ?>-->
                            ...
                            <div class="div-tab-panel"> 
                                <div class="div-table-caption border-red"><?php echo ucwords($obj->lang['productInformation']); ?></div>  
                                ...
                                <?php if ($useVariant) { ?>  
									<div class="form-group">
                                        ...
                                        <div class="col-xs-9"> 
											<div class="flex">
												<div ><?php echo $obj->inputCheckBox('chkIsVariant'); ?></div>
												<div class="consume">
												<?php    
                                ...
                                <div class="form-group">
                                    ...
                                    <?php if(in_array(PLAN_TYPE['categorykey'], array(COMPANY_TYPE['jewelry']))) { ?>
                                        ...
                                        <div class="form-group">
                                            <label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['material']); ?></label> 
                                            <div class="col-xs-9">  
                                                <?php   echo $obj->inputAutoComplete(array(  
                                                                                    'element' => array('value' => 'materialName',
                                                                                                        'key' => 'hidMaterialKey'),
                                                                                    'source' =>array(
                                        ...
                                        <div class="form-group">
                                            <label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['plating']); ?></label> 
                                            <div class="col-xs-9">  
                                                <?php   echo $obj->inputAutoComplete(array(  
                                                                                    'element' => array('value' => 'platingName',
                                                                                                    'key' => 'hidPlatingKey'),
                                                                                    'source' =>array(
                                                                                                        'url' => 'ajax-plating.php',
                                                                                                        'data' => array('action' =>'searchData', 'searchField' => 'code,name' )
                                                                                                    ) 
                                                                                )
                                                                            );  
                                        ...
                                        <div class="form-group">
                                            <label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['model']); ?></label> 
                                            <div class="col-xs-9">
                                            <div class="div-table mnv-item-model transaction-detail" style="width:100%">
                                                <?php 
                                                    $totalRows = count($rsItemModelDetail);
                                                    ...
                                                    for ($j=0;$j<=$totalRows; $j++){ 
                                                        ...
                                                        if ($j == $totalRows ){
                                                            $class = 'item-model-row-template row-template';
                                                            $overwrite = false;
                                                            $disabled = true; 
                                                            $isLocked = false; 
                                                        } else{ 
                                                            $_POST['hidModelDetailKey[]'] =  $rsItemModelDetail[$j]['pkey'];
                                                            $_POST['hidModelKey[]'] =  $rsItemModelDetail[$j]['modelkey'];
                                                            $_POST['modelName[]'] =  $rsItemModelDetail[$j]['modelname'];
                                                    ...
                                                    <div class="div-table-row <?php echo $class; ?>  odd-style-adjustment" > 
                                                        <div class="div-table-col"  style="padding-left:0"> 
                                                            <div class="flex" style="width:100%">     
                                                                    <div style="width:100%;">
                                                                    <?php echo $obj->inputHidden('hidModelDetailKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled)); ?>
                                                                        <?php echo $obj->inputHidden('hidModelKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled)); ?>
                                                                        <?php echo $obj->inputText('modelName[]', array('overwritePost' => $overwrite ,'readonly' => $readonly, 'disabled' => $disabled )); ?>
                                                                    </div>
                                                                    <div class="icon-col <?php echo $obj->hideOnDisabled(); ?>"><?php echo $obj->inputLinkButton('btnAddDetailRow' , '<i class="fas fa-plus-circle"></i>', array('class' => 'btn btn-link add-row-button', 'etc' => 'attr-template="item-model-row-template"')); ?></div>
                                                                    <div class="icon-col <?php echo $obj->hideOnDisabled(); ?>"><?php echo $obj->inputLinkButton('btnDeleteRows' , '<i class="fas fa-times"></i>', array('class' => 'btn btn-link remove-button', 'etc' =>  'tabIndex="-1" style="padding:6px 0; '.$hideDeleteIcon.'"')); ?></div>
                                        ...
                                        <div class="form-group">
                                            <label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['texture']); ?></label> 
                                            <div class="col-xs-9">
                                                <div class="div-table mnv-item-texture transaction-detail" style="width:100%">
                                                <?php 
                                                    $totalRows = count($rsItemTextureDetail);   
                                                    for ($j=0;$j<=$totalRows; $j++){ 
                                                        ...
                                                        if ($j == $totalRows ){
                                                            $class = 'item-texture-row-template row-template';
                                                            $overwrite = false;
                                                            $disabled = true; 
                                                            $isLocked = false; 
                                                        } else{ 
                                                            $_POST['hidTextureDetailKey[]'] =  $rsItemTextureDetail[$j]['pkey'];
                                                            $_POST['hidTextureKey[]'] =  $rsItemTextureDetail[$j]['texturekey'];
                                                            $_POST['textureName[]'] =  $rsItemTextureDetail[$j]['texturename'];
                                                    ...
                                                    <div class="div-table-row <?php echo $class; ?>  odd-style-adjustment" > 
                                                        <div class="div-table-col"  style="padding-left:0"> 
                                                            <div class="flex" style="width:100%">     
                                                                    <div style="width:100%;">
                                                                    <?php echo $obj->inputHidden('hidTextureDetailKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled)); ?>
                                                                        <?php echo $obj->inputHidden('hidTextureKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled)); ?>
                                                                        <?php echo $obj->inputText('textureName[]', array('overwritePost' => $overwrite ,'readonly' => $readonly, 'disabled' => $disabled )); ?>
                                                                    </div>
                                                                    <div class="icon-col <?php echo $obj->hideOnDisabled(); ?>"><?php echo $obj->inputLinkButton('btnAddDetailRow' , '<i class="fas fa-plus-circle"></i>', array('class' => 'btn btn-link add-row-button', 'etc' => 'attr-template="item-texture-row-template"')); ?></div>
                                                                    <div class="icon-col <?php echo $obj->hideOnDisabled(); ?>"><?php echo $obj->inputLinkButton('btnDeleteRows' , '<i class="fas fa-times"></i>', array('class' => 'btn btn-link remove-button', 'etc' =>  'tabIndex="-1" style="padding:6px 0; '.$hideDeleteIcon.'"')); ?></div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-xs-9">
                                                <div class="div-table mnv-item-color transaction-detail" style="width:100%">
                                                <?php 
                                                    $totalRows = count($rsItemColorDetail);   
                                                    for ($j=0;$j<=$totalRows; $j++){ 
                                                        ...
                                                        if ($j == $totalRows ){
                                                            $class = 'item-color-row-template row-template';
                                                            $overwrite = false;
                                                            $disabled = true; 
                                                            $isLocked = false; 
                                                        } else{ 
                                                            $_POST['hidColorDetailKey[]'] =  $rsItemColorDetail[$j]['pkey'];
                                                            $_POST['hidColorKey[]'] =  $rsItemColorDetail[$j]['colorkey'];
                                                            $_POST['colorName[]'] =  $rsItemColorDetail[$j]['colorname'];
                                                    ...
                                                    <div class="div-table-row <?php echo $class; ?>  odd-style-adjustment" > 
                                                        <div class="div-table-col"  style="padding-left:0"> 
                                                            <div class="flex" style="width:100%">     
                                                                    <div style="width:100%;">
                                                                    <?php echo $obj->inputHidden('hidColorDetailKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled)); ?>
                                                                        <?php echo $obj->inputHidden('hidColorKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled)); ?>
                                                                        <?php echo $obj->inputText('colorName[]', array('overwritePost' => $overwrite ,'readonly' => $readonly, 'disabled' => $disabled )); ?>
                                                                    </div>
                                                                    <div class="icon-col <?php echo $obj->hideOnDisabled(); ?>"><?php echo $obj->inputLinkButton('btnAddDetailRow' , '<i class="fas fa-plus-circle"></i>', array('class' => 'btn btn-link add-row-button', 'etc' => 'attr-template="item-color-row-template"')); ?></div>
                                                                    <div class="icon-col <?php echo $obj->hideOnDisabled(); ?>"><?php echo $obj->inputLinkButton('btnDeleteRows' , '<i class="fas fa-times"></i>', array('class' => 'btn btn-link remove-button', 'etc' =>  'tabIndex="-1" style="padding:6px 0; '.$hideDeleteIcon.'"')); ?></div>
  ...
  <div class="form-group">
                                            <label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['character']); ?></label> 
                                            <div class="col-xs-9">
                                                <div class="div-table mnv-item-character transaction-detail" style="width:100%">
                                                <?php 
                                                    $totalRows = count($rsItemCharacterDetail);   
                                                    for ($j=0;$j<=$totalRows; $j++){ 
                                                        ...
                                                        if ($j == $totalRows ){
                                                            $class = 'item-character-row-template row-template';
                                                            $overwrite = false;
                                                            $disabled = true; 
                                                            $isLocked = false; 
                                                        } else{ 
                                                            $_POST['hidCharacterDetailKey[]'] =  $rsItemCharacterDetail[$j]['pkey'];
                                                            $_POST['hidCharacterKey[]'] =  $rsItemCharacterDetail[$j]['characterkey'];
                                                            $_POST['characterName[]'] =  $rsItemCharacterDetail[$j]['charactername'];
                                                    ...
                                                    <div class="div-table-row <?php echo $class; ?>  odd-style-adjustment" > 
                                                        <div class="div-table-col"  style="padding-left:0"> 
                                                            <div class="flex" style="width:100%">     
                                                                    <div style="width:100%;">
                                                                    <?php echo $obj->inputHidden('hidCharacterDetailKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled)); ?>
                                                                        <?php echo $obj->inputHidden('hidCharacterKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled)); ?>
                                                                        <?php echo $obj->inputText('characterName[]', array('overwritePost' => $overwrite ,'readonly' => $readonly, 'disabled' => $disabled )); ?>
                                                                    </div>
                                                                    <div class="icon-col <?php echo $obj->hideOnDisabled(); ?>"><?php echo $obj->inputLinkButton('btnAddDetailRow' , '<i class="fas fa-plus-circle"></i>', array('class' => 'btn btn-link add-row-button', 'etc' => 'attr-template="item-character-row-template"')); ?></div>
                                                                    <div class="icon-col <?php echo $obj->hideOnDisabled(); ?>"><?php echo $obj->inputLinkButton('btnDeleteRows' , '<i class="fas fa-times"></i>', array('class' => 'btn btn-link remove-button', 'etc' =>  'tabIndex="-1" style="padding:6px 0; '.$hideDeleteIcon.'"')); ?></div>
                                          ...
                                          <!-- <div class="form-group">
                                            ...
                                            <div class="col-xs-9">  
                                                <?php   echo $obj->inputAutoComplete(array(  
                                                                                    'element' => array('value' => 'characterName',
                                                                                                    'key' => 'hidCharacterKey'),
                                                                                    'source' =>array(
                                                                                                        'url' => 'ajax-character.php',
                                                                                                        'data' => array('action' =>'searchData', 'searchField' => 'code,name' )
                                                                                                    ) 
                                                                                )
                                                                            );  
                                        ...
                                        <div class="form-group">
 <label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['itemVariation']); ?></label> 
                                            <div class="col-xs-9">
                                                <div class="div-table mnv-item-variation transaction-detail" style="width:100%">
                                                <?php 
                                                    $totalRows = count($rsItemVariationDetail);   
                                                    for ($j=0;$j<=$totalRows; $j++){ 
                                                        ...
                                                        if ($j == $totalRows ){
                                                            $class = 'item-variation-row-template row-template';
                                                            $overwrite = false;
                                                            $disabled = true; 
                                                            $isLocked = false; 
                                                        } else{ 
                                                            $_POST['hidVariationDetailKey[]'] =  $rsItemVariationDetail[$j]['pkey'];
                                                            $_POST['hidVariationKey[]'] =  $rsItemVariationDetail[$j]['variationkey'];
                                                            $_POST['variationName[]'] =  $rsItemVariationDetail[$j]['variationname'];
                                                    ...
                                                    <div class="div-table-row <?php echo $class; ?>  odd-style-adjustment" > 
                                                        <div class="div-table-col"  style="padding-left:0"> 
                                                            <div class="flex" style="width:100%">     
                                                                    <div style="width:100%;">
                                                                    <?php echo $obj->inputHidden('hidVariationDetailKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled)); ?>
                                                                        <?php echo $obj->inputHidden('hidVariationKey[]',array('overwritePost' => $overwrite, 'readonly' => $readonly, 'disabled' => $disabled)); ?>
                                                                        <?php echo $obj->inputText('variationName[]', array('overwritePost' => $overwrite ,'readonly' => $readonly, 'disabled' => $disabled )); ?>
                                                                    </div>
                                                                    <div class="icon-col <?php echo $obj->hideOnDisabled(); ?>"><?php echo $obj->inputLinkButton('btnAddDetailRow' , '<i class="fas fa-plus-circle"></i>', array('class' => 'btn btn-link add-row-button', 'etc' => 'attr-template="item-variation-row-template"')); ?></div>
                                                                    <div class="icon-col <?php echo $obj->hideOnDisabled(); ?>"><?php echo $obj->inputLinkButton('btnDeleteRows' , '<i class="fas fa-times"></i>', array('class' => 'btn btn-link remove-button', 'etc' =>  'tabIndex="-1" style="padding:6px 0; '.$hideDeleteIcon.'"')); ?></div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-xs-9"> 
                                                <?php echo $obj->inputSelect('selSexKey',$arrSex); ?>
                                            </div> 
                                        </div> 
                                            
                                
                                        <div class="form-group">
                                            <label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['size']); ?></label> 

                                            <div class="col-xs-9"> 
                                                <div class="flex">
                                                    <div class="consume" style="position:relative; top:0.1em">
                                                        <?php   echo $obj->inputAutoComplete(array(  
                                                                                    'element' => array('value' => 'ringSizeName',
                                                                                                    'key' => 'hidRingSizeKey'),
                                                                                    'source' =>array(
                                                                                                        'url' => 'ajax-ring-size.php',
                                                                                                        'data' => array('action' =>'searchData' )
                                                                                                    ) 
                                                                                )
                                                                            );  
                                                    ...
                                                    <div><?php echo $obj->inputNumber('size'); ?></div>
                                                    <div><?php echo $obj->inputSelect('selSizeUnitKey', $arrUnitNonWeight); ?></div>
                                                </div>
                                            </div>  
                                        </div> 

                                        <div class="form-group">
                                            <label class="col-xs-3 control-label"><?php echo $obj->lang['itemDimension']; ?> (cm)</label> 

                                            <div class="col-xs-9"> 
                                                <div class="flex">
                                                    <div><?php echo $obj->lang['length']; ?></div>
                                                    <div class="consume"><?php echo $obj->inputDecimal('length'); ?></div>
                                                    <div class="text-muted">x</div>
                                                    <div><?php echo $obj->lang['width']; ?></div>
                                                    <div class="consume"><?php echo $obj->inputDecimal('width'); ?></div>
                                                    <div class="text-muted">x</div>
                                                    <div><?php echo $obj->lang['height']; ?></div>
                                                    <div class="consume"><?php echo $obj->inputDecimal('height'); ?></div>
                                                </div>
                                            </div>  
                                        ...
                                        <div class="form-group">
                                            <label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['carat']); ?> / <?php echo ucwords($obj->lang['weight']); ?></label> 
                                            <div class="col-xs-9">  
                                                <div class="flex">
                                                    <div> <?php echo $obj->inputDecimal('carat', array('etc' =>'style=" width: 6em; padding-right:7px"')); ?></div>
                                                    <div  style="margin-left:1em">%</div>
                                                    <div style="margin-left:2em"> <?php echo ucwords($obj->lang['weight']); ?> </div>
                                                    <div>
                                                    <div class="flex">          
                                                        <div class="consume"><?php echo $obj->inputDecimal('gramasi', array('etc' =>'style="padding-right:7px"')); ?></div>
                                        ...
                                        <?php } else {  
                                          ...
                                          echo '<div class="div-table" style="margin:auto;  width:95%; "> 
                                                     <div class="div-table-row"> 
                                                         ...
                                                         <div class="div-table-col-5" style="border-top:1px solid #666;border-bottom:1px solid #666; text-align:right;" >  <strong>'.ucwords($obj->lang['qty']).' Gr</strong>  </div> 
                                                     </div>';
                                                     ...
                                                     for($i=0;$i<count($rsWarehouse);$i++){
                                                         ...
                                                         if (!empty($id)) 
                                                            ...
                                                            $qtyInPcs = $obj->formatNumber($itemMovement->getItemQOHInPcs($id, $rsWarehouse[$i]['pkey']),2);
                                                            $rsItemUnit = $itemUnit->getDataRowById($rs[0]['baseunitkey']);
                                                            $baseUnitName = $rsItemUnit[0]['name'];

                                                         $colorClass= ($qoh == 0) ? "text-red-cardinal" : '';
                                                         $qtyInPcsColorClass= ($qtyInPcs == 0) ? "text-red-cardinal" : '';
                                                         echo '
                                                         <div class="div-table-row"> 
                                                             <div class="div-table-col-5 '.$colorClass.'" style="border-bottom:1px solid #dedede;" >'.$rsWarehouse[$i]['name'].'</div> 
                                                             <div class="div-table-col-5 '.$colorClass.'" style="border-bottom:1px solid #dedede; text-align:right;" > '.$qoh.'</div> 
                                                             <div class="div-table-col-5 '. $qtyInPcsColorClass.'" style="border-bottom:1px solid #dedede; text-align:left;" > '.$baseUnitName.'</div> 
                                                             <div class="div-table-col-5 '. $qtyInPcsColorClass.'" style="border-bottom:1px solid #dedede; text-align:right;" > '.$qtyInPcs.'</div> 
                                                         </div> 
                                                         ';
                                                      }
                                            echo '</div>';          
                            ...
                            <div class="div-tab-panel"> 
                                <!--<div class="div-table-caption border-green"><?php echo ucwords($obj->lang['packagingInformation']); ?></div> -->

                                    <?php

                                         echo '<div class="div-table" style="margin:auto;  width:95%; "> 
                                                    <div class="div-table-row"> 
                                                        <div class="div-table-col-5" style="border-top:1px solid #666;border-bottom:1px solid #666;" >   <strong>'.ucwords($obj->lang['warehouse'] ).'</strong> </div> 
                                                        <div class="div-table-col-5" style="border-top:1px solid #666;border-bottom:1px solid #666;" >  <strong>'.ucwords($obj->lang['packaging']).'</strong>  </div> 
                                                        <div class="div-table-col-5" style="border-top:1px solid #666;border-bottom:1px solid #666; text-align:center" >  <strong>'.ucwords($obj->lang['date']).'</strong>  </div> 
                                                        <div class="div-table-col-5" style="border-top:1px solid #666;border-bottom:1px solid #666;text-align:right;" >   <strong>'.ucwords($obj->lang['qty']).'</strong> </div> 
                                                         <div class="div-table-col-5" style="border-top:1px solid #666;border-bottom:1px solid #666; text-align:left;" >  <strong>'.ucwords($obj->lang['unit']).'</strong>  </div> 
                                                        <div class="div-table-col-5" style="border-top:1px solid #666;border-bottom:1px solid #666;text-align:right;" >   <strong>'.ucwords($obj->lang['qty'] . ' Gr').'</strong> </div>  
                                                    </div>';
                                            ...
                                            $rsItemPackagingDetail = (!empty($id)) ? $itemMovement->getItemMovementPackagingDetail($id) : [];
                                                 
                                            for($i=0;$i<count($rsItemPackagingDetail);$i++){

                                                $qtyInPcs = $rsItemPackagingDetail[$i]['qtyinpcs'];
                                                $qtyInBaseUnit = $rsItemPackagingDetail[$i]['qtyinbaseunit'];

                                                $qtyInPcsColorClass = ($qtyInPcs == 0) ? "text-red-cardinal" : '';
                                                $qtyInBaseUnitColorClass = ($qtyInPcs == 0) ? "text-red-cardinal" : '';

                                                echo '<div class="div-table-row"> 
                                                            <div class="div-table-col-5 '.$qtyInPcsColorClass.'" style="border-bottom:1px solid #dedede;" >'.$rsItemPackagingDetail[$i]['warehousename'].'</div> 
                                                            <div class="div-table-col-5 '. $qtyInPcsColorClass.'" style="border-bottom:1px solid #dedede; text-align:left;" >'.$rsItemPackagingDetail[$i]['packagingcode'].'</div> 
                                                            <div class="div-table-col-5 '. $qtyInPcsColorClass.'" style="border-bottom:1px solid #dedede; text-align:center;" >'.$obj->formatDBDate($rsItemPackagingDetail[$i]['packagingdate']).'</div> 
                                                            <div class="div-table-col-5 '.$qtyInBaseUnitColorClass.'" style="border-bottom:1px solid #dedede; text-align:right;" >'.$obj->formatNumber($qtyInBaseUnit).'</div> 
                                                            <div class="div-table-col-5 '. $qtyInPcsColorClass.'" style="border-bottom:1px solid #dedede; text-align:left;" >'.$rsItemPackagingDetail[$i]['baseunitname'].'</div> 
                                                            <div class="div-table-col-5 '.$qtyInPcsColorClass.'" style="border-bottom:1px solid #dedede; text-align:right;" >'.$obj->formatNumber($qtyInPcs,2).'</div>
                                                        </div> ';

                                                            //<div class="div-table-col-5 '. $qtyInPcsColorClass.'" style="border-bottom:1px solid #dedede; text-align:left;" >'.$rsItemPackagingDetail[$i]['packagingname'].'</div> 
                                            }
...
-->
						 	<?php if (isset($qrResult) && !empty($qrResult)){ 
									...
									echo ' <div class="div-tab-panel">';
	 								echo '<div class="div-table-caption border-blue">'.ucwords($obj->lang['productLink']).'</div>';
									echo ' <div class="col-xs-12">';
                                        echo '<img src="/download.php?temp=1&filename=qr/'.$qrResult['filename'].'" style="width:15em">';
                ...
                <?php   
                  ...
                  foreach($marketplaceObjs as  $marketplaceRow){ 
                        ...
                        $marketplacecategorykey = (!empty($rsMarketplaceCategory)) ?  $rsMarketplaceCategory[0]['marketplacecategorykey'] : 0 ;
					  	...
					  	// knp diatas ? agar selalu ad nilainya ?
                        $_POST['hidSyncMarketplaceKey[]'] = $marketplaceKey;
                        
					  	// default value
					  	$_POST['hidMarketplaceSyncDetailKey[]'] = 0;
						$_POST['chkSyncToMarketplace[]'] = 0; 
						$_POST['marketplaceItemName[]'] = ''; 
						$_POST['marketplaceItemShortDescription[]'] = ''; 
					  
                        // data lama harus add manual didatabase
                        if(isset($rsSyncMarketplace) && !empty($rsSyncMarketplace[$marketplaceKey])) { 
                            $_POST['hidMarketplaceSyncDetailKey[]'] = $rsSyncMarketplace[$marketplaceKey]['pkey'];
                            $_POST['chkSyncToMarketplace[]'] = $rsSyncMarketplace[$marketplaceKey]['issync']; 
                            $_POST['marketplaceItemName[]'] = $rsSyncMarketplace[$marketplaceKey]['name']; 
                            $_POST['marketplaceItemShortDescription[]'] = $rsSyncMarketplace[$marketplaceKey]['shortdescription']; 
						}
					  
					    echo '<div class="section-title"> '.
									$obj->inputHidden('hidMarketplaceSyncDetailKey[]').
									$obj->inputHidden('hidSyncMarketplaceKey[]').
									$obj->inputCheckBox('chkSyncToMarketplace[]').
									'&nbsp;&nbsp;'.strtoupper($marketplaceRow['name']).'</div>';
				
                      	echo '<div class="div-table"  style="width:100%">'; 
					    echo '<div class="div-table-row">';
					    echo '<div class="div-table-col" style="width:50%; padding-right: 1em">';
					  
					    echo '<div class="flex" style="margin-bottom:0.3em">
                                <div style="width:120px">
                                    <div class="attribute-label">'.$obj->lang['itemName'].'</div> 
                                </div>
                                <div class="consume attribute-value">'. $obj->inputText('marketplaceItemName[]',array('etc' => 'placeholder="'.$obj->lang['leaveItBlankForDefaultInformation'].'"')).'</div>
                            </div>';
					  	
					  	echo '<div class="flex" style=" align-items:flex-start">
                                <div style="width:120px;">
                                    <div class="attribute-label" >'.$obj->lang['shortDescription'].'</div> 
                                </div>
                                <div class="consume attribute-value">'. $obj->inputTextArea('marketplaceItemShortDescription[]', array( 'etc' => 'style="height:8em;" placeholder="'.$obj->lang['leaveItBlankForDefaultInformation'].'"')).'</div>
                            </div>';
					  	
					    echo '</div>';
					    echo '<div class="div-table-col" style="width:50%">'; 
                        if($useVariant){
                         ...
                         if($marketplaceProviderKey == MARKETPLACE['tokopedia']){//sementara
							...
							if(!empty($rs[0]['pkey'])){    
								$rsVariantDetail = $obj->getVariantValueForMarketplace($rs[0]['pkey'], $marketplaceKey);  
                                ...
                                if(!empty($rsVariantDetail[0]['optionvalue'])){  
                                    ...
                                    $optkey = $optkey['value']  ; //array_keys($optkey)[0];
                                } 
...
//                                $arrVariantOption = $obj->convertForCombobox($rsVariantOption[$selectedVariantKey],'value_id','value');  
                        ...
                        ?>
                            ...
                            <div class="flex" style="padding: 0.2em 1em">
                                <div class="attribute-label" style="width:120px;vertical-align:top"> <?php echo $obj->lang['variantProduct']; ?></div>
                                <div class="consume">
                                    <div class="variant-row">
										<div class="flex">
                                        <div class="consume"><?php echo $obj->inputHidden($hidVariantDetailKeyName); ?><?php echo  $obj->inputSelect($selVariantName, $arrVariant, array('add-class' => 'select-variant', 'etc' => 'attr-marketplacekey="'.$marketplaceKey.'"')); ?></div>
<!--                                        <div class="consume variant-option-col"><?php echo  $obj->inputSelect($selOptionName,$arrVariantOption); ?></div> -->
											<div class="consume variant-option-col"><?php echo  $obj->inputText($selOptionName,array('placeholder="'.$obj->lang['option'].'"')); ?></div> 
										</div>	
                                    </div>
                                </div>    
                            </div>  
                            ...
                            ?>
                             <div class="flex" style="padding: 0.2em 1em">
                                ...
                                <div class="consume">
                                    <div class="variant-row">
										<div class="flex">
										 <div class="consume"><?php echo $obj->inputHidden($hidVariantDetailKeyName); ?><?php echo  $obj->inputText($selVariantName, array('etc' => 'placeholder="'.$obj->lang['variant'].'"  attr-marketplacekey="'.$marketplaceKey.'"','add-class' => 'txt-variant' )); ?></div>
                                         <div class="consume variant-option-col"><?php echo $obj->inputText($selOptionName); ?></div> 
										</div> 
                                    </div>
                                </div>    
                            </div>  
                       <?php  }  
                        ...
                        echo '<ul class="marketplace-attributes">
					  	...
					  	// logistics
	    				$rsAvailableLogistics = $marketplaceObj->getAvailableMarketplaceLogisticsForItem();
                        ...
                        if(!empty($rs[0]['pkey'])){  
							...
							$rsLogisticsDetail = $obj->getMarketplaceLogistics($rs[0]['pkey'], $marketplaceKey); 
							$arrActiveShipment = array_column($rsLogisticsDetail,'reflogistickey'); 
                        }
                       
						//$obj->setLog($marketplaceAttributes,true);
					  
                        foreach($marketplaceAttributes as $row){
							
                                $attributeKey = $row['attributekey'];
                             	if ($attributeKey == 0) continue; // brand, ambil dr master lg skrg, nanti dihapus saja di tableny
								...
								// khusus brand shopee
								if($marketplaceProviderKey == MARKETPLACE['shopee'] && $row['attributekey'] == 0 && strtolower($row['label']) == 'brand' ){
									$options =  $marketplaceObj->getBrandOption(json_decode($row['value']));  
								} 
...
//								$obj->setLog(">>>",true);
//								$obj->setLog($marketplaceObj->getSelectOptions($row['value']),true);
//								$obj->setLog(array_keys($marketplaceObj->getSelectOptions($row['value'])),true);	
//								$obj->setLog("<<<",true);
//							
                                ...
                                $input = $setting->getInput(array('multivalue' => 0, 
																  'type' => $row['inputtype'], 
																  'code' => 'attributeValue[]', 
																  'options' => $options,
																  'url' => 'ajax-marketplce-brand.php',
																  'etc' => 'attr-label="'.strtolower($label).'"' 
																 ));
                            
                                $input .= $obj->inputHidden('hidRawOpt[]',array('value' => base64_encode($row['value'])));
                     ...
                     <?php   }
                          ...
                          echo '<div style="clear:both;"></div>';
					  	 ...
					  	 if(!empty($rsAvailableLogistics)){
						   echo '<div style="padding:1em">';	 
						   echo '<div class="attribute-label" style="margin-bottom:0.5em">'.$obj->lang['shipment'].'</div>';
						   echo '<div class="flex shipment-flex" style="align-items: flex-start; align-content:flex-start; flex-wrap: wrap">'; 
							   
						   foreach($rsAvailableLogistics as $logisticsRow){ 
									$_POST['chkLogisticsMarketplace[]'] = (empty($rs[0]['pkey']) || in_array( $logisticsRow['pkey'],$arrActiveShipment)) ? 1 : 0;
							?>
									<div >
									    <div  style="width: 185px">
											<?php echo $obj->inputCheckBox('chkLogisticsMarketplace[]'); ?> <?php echo $logisticsRow['name']; ?>
										</div> 
										<?php echo $obj->inputHidden('hidLogisticsMarketplaceDetailKey[]'); ?>
										<?php echo $obj->inputHidden('hidLogisticsMarketplaceKey[]',array('value' => $marketplaceKey)); ?>  
										<?php echo $obj->inputHidden('hidLogisticsRefKey[]',array('value' => $logisticsRow['pkey'])); ?>  
									</div> 
						<?php  } 
						   echo '</div>';
						   echo '</div>';
