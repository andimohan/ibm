...
$generatePartnersTable = function ($obj,$rs,&$totalVendor){ 
     
    $obj = new TruckingServiceOrder();
    $rsVendor = $obj->getPartnersVehicleInformation($rs[0]['pkey']);
	$rsVendor = $obj->groupSupplierAmount($rsVendor);
    
    $html  = ''; 
    $total = 0; 
	
	$html = ' <br>
   <div style="font-weight:bold; line-height:30px; ">'.strtoupper($obj->lang['shareProfit']).'</div>
   <table cellpadding="4" class="table-transaction" style="font-size:10px; border-bottom:0px solid #fff">';
    ...
    for($i=0;$i<count($rsVendor);$i++){
    
        $qty = $rsVendor[$i]['qty']; 
        $amount = $rsVendor[$i]['amount']; 
        $subtotal = $qty * $amount;
        $total += $subtotal; 
        
        $borderStyle = ($i<count($rsVendor) -1 ) ? 'col-border-bottom' : 'last-border-bottom';
 
        $html .= '
        <tr>   
        <td class="'.$borderStyle.'">'.$rsVendor[$i]['vehiclepartnersname'].'</td>    
        <td class="'.$borderStyle.'" style="text-align:right">'.$obj->formatNumber($qty).'</td>  
        <td class="'.$borderStyle.'" style="text-align:right">'.$obj->formatNumber($amount).'</td>  
        <td class="'.$borderStyle.'" style="text-align:right">'.$obj->formatNumber($subtotal).'</td>  
        </tr>
    ...
    $html .= ' 
	<tr><td colspan="3"></td><td class="subtotal" style="text-align:right;">'.$obj->formatNumber($totalVendor).'</td></tr>
	</table>';
...
$generateContainerTable = function ($obj,$rs){
    ...
    $html  = '<div style="font-weight:bold;">'.strtoupper($obj->lang['container']).'<br></div>
    ...
    array_push($cellArray, array('label' => $obj->lang['owner'], 'width' => '180'));
    ...
    for($i=0;$i<count($rsSPK);$i++){  
        ...
        if (empty($rsSPK[$i]['TL'])){ 
            ...
            $supplierName = (!empty($rsSPK[$i]['vehiclepartnersname'])) ? $rsSPK[$i]['vehiclepartnersname'] : 'AMT';
        }else{
        ...
        $html  .= '<tr>
                        ...
                        <td class="'.$borderStyle.'">'.$supplierName.'</td>
...
$vendorTable =   $generatePartnersTable($obj,$rs, $totalVendor);
...
<table>
<tr>
<td style="width:330px;">'.$vendorTable.'</td>  
<td style="width:20px;"></td>  
...
if ($hasSellingPriceAccess) {
    $grossProfit = ($totalSelling + $totalSellingReimburse ) - ($totalCost + $totalReimburseCost + $totalVendor);
...
$generatePartnersVehicleTable = function ($rs,&$totalCost){
     
    $obj = new TruckingServiceOrder(); 
    $rsVendor = $obj->getPartnersVehicleInformation($rs[0]['pkey']);
 
    $html  = '';
    $title = strtoupper($obj->lang['shareProfit']);
    $total = 0; 
 
    for($i=0;$i<count($rsVendor);$i++){
            
        $subtotal = $rsVendor[$i]['amount'];  
        $total += $subtotal;    
        ...
        <tr>  
        <td class="'.$borderStyle.'" style="font-weight:bold">'.$title.'</td>  
        <td class="'.$borderStyle.'"><span class="service-name">'.$rsVendor[$i]['registrationnumber'].'</span></td>   
        <td class="'.$borderStyle.'">'.$rsVendor[$i]['vehiclepartnersname'].'</td>   
        <td class="'.$borderStyle.'">'.$rsVendor[$i]['wocode'].'</td>  
        <td class="'.$borderStyle.'">'.$rsVendor[$i]['apcode'].'</td>   
        <td class="'.$borderStyle.'" style="text-align:right">'.$obj->formatNumber($subtotal).'</td>   
        </tr>
        ';
...
array_push($cellArray, array('label' => $obj->lang['description']));
array_push($cellArray, array('label' => $obj->lang['recipient'], 'width' => '160'));
array_push($cellArray, array('label' => $obj->lang['transactionCode'],  'width' => '120'));
array_push($cellArray, array('label' => $obj->lang['cash/ap'],  'width' => '95'));
...
$costTable .= $generateAdditionalCostTable($obj,$rs,$totalCost);
$costTable .= $generatePartnersVehicleTable($rs,$totalCost);
