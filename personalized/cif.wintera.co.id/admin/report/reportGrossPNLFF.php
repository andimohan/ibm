...
$arrDataStructure['sales'] = array('title' => ucwords($obj->lang['salesman']), 'width' => "150px", 'dbfield' => 'salesname');
...
//        $arrTempStructure['invoice'.$currRow['pkey']] = array('title' => strtoupper($currRow['name']), 'group' => ucwords($obj->lang['invoice']), 'dbfield' => 'totalinvoice'. $currRow['pkey'], 'width' => "90px", "format" => 'decimal', 'calculateTotal' => true, "sortable" => false);
    ...
    $arrActiveCurrProfit = getActiveProfitCurrency($arrActiveCurrInvoiced, $arrActiveCurrCost, $arrActiveCurrDN, $arrActiveCurrCN);
    ...
    for ($i = 0; $i < $totalRs; $i++) {
        ...
        foreach ($rsCurrency as $currRow) { 
            ...
            $cost = (isset($rsBuyingCol[$currRow['pkey']])) ?  $rsBuyingCol[$currRow['pkey']][0]['amount'] : 0;
            ...
            $profitCurr = $selling - $cost - $cnAmount + $dnAmount;
...
function getActiveProfitCurrency($arrInvoiced, $arrCost, $arrDN, $arrCN) {
    $temp = [];

    foreach ([$arrInvoiced, $arrCost, $arrDN, $arrCN] as $arr) {
        foreach ($arr as $row) {
            $temp[$row['pkey']] = $row;
        }
    }
