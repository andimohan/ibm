...
$statusWidth = ($useCurrencyRevaluation) ? '800px': '200px';
...
$arrDataDetailStructure['JOreference'] = array('title'=>ucwords($obj->lang['jobOrder']),'dbfield' => 'refjocode', 'width'=>"130px" );  
...
if (isset($_POST) && !empty($_POST['hidAction'])){    
    ...
    // ambil tipe dan pkey dari reftabletype dulu
    $refJobDetail = array();
    $refTableTypeCol = $obj->reindexDetailCollections($rsCol,'reftabletype');
    foreach($refTableTypeCol as $tablekey=>$tabletypeRow){
        
        // sementara
        if (!in_array($tablekey,array(424,499,501,3987,315))) continue;
        
        $arrReftableKey = array_column($tabletypeRow, 'refkey');
        $reftableObj = $obj->getObjMapping('',$tablekey); 
        $refJobDetail[$tablekey] = $obj->reindexDetailCollections($reftableObj->getJobInformation($arrReftableKey),'reftablekey'); // array_column($reftableObj->getJobInformation($arrReftableKey),null,'reftablekey'); 
        
    }
    
    //$obj->setLog($refJobDetail,true);
	...
	foreach ($rsCOA as $coa) {  
		...
		for($i=0;$i<count($rs);$i++) {    
            ...
            // informasi JO
            $rsJO = (isset($refJobDetail[$rs[$i]['reftabletype']][$rs[$i]['refkey']])) ?  $refJobDetail[$rs[$i]['reftabletype']][$rs[$i]['refkey']] : array();
            $rs[$i]['refjocode'] =  implode(', ', array_column($rsJO,'jocode'));
