...
//	$selectedWarehouse = ((!empty($_POST['selWarehouse']))) ? $_POST['selWarehouse'] : array(); 
    ...
    switch ($reportType) {
        ...
        case 0:
            ...
            $rsIncome = generateIncome2(array('income'), $_POST['trStartDate'], $_POST['trEndDate'], $selectedWarehouse);
            $arrIncome[$_POST['trStartDate']] = $rsIncome;

            $rsExpense = generateIncome2(array('expense'), $_POST['trStartDate'], $_POST['trEndDate'], $selectedWarehouse);
        ...
        case 1:
            ...
            $arrIncome = getCOAAmount2(array('income'), $startPeriod, $endPeriod, $reportType, $selectedWarehouse);
            $arrExpense = getCOAAmount2(array('expense'), $startPeriod, $endPeriod, $reportType, $selectedWarehouse);
            ...
            array_push($tempExcelCol, array('value' => $labelTotal, 'style' => $EXPORT_EXCEL_DATA['class']['col-header']));
        ...
        case 4:
            ...
            foreach ($arrYear as $yearRow) {
                ...
                $arrIncome = array_merge($arrIncome, getCOAAmount2(array('income'), $startPeriod, $endPeriod, $reportType, $selectedWarehouse));
                $arrExpense = array_merge($arrExpense, getCOAAmount2(array('expense'), $startPeriod, $endPeriod, $reportType, $selectedWarehouse));
            }
    ...
    array_push($tempExcelCol, array('value' => ''));
    array_push($tempExcelCol, array('value' => strtoupper($class->lang['profitLoss']), 'style' => $EXPORT_EXCEL_DATA['class']['row-header']));
    ...
    foreach ($arrIncomeTotal as $key => $row) {
        ...
        array_push($tempExcelCol, array('value' => $profitloss, 'style' => $excelStyle, 'format' => array('format' => 'decimal')));
    }
...
echo $twig->render('@custom/reportIncomeStatement.html', $arrTwigVar);
...
{
    $dataToExport[$rowIndex][$colIndex]['excelValue'] = $cell['value'] ?? 0;
...
function getCOAAmount2($arrCOAType, $startPeriod, $endPeriod, $reportType, $arrWarehouseKey)
{
    global $class;
    ...
    do {
        ...
        $rsIncome = generateIncome2($arrCOAType, $formattedCurrDate, $formattedEndDate, $arrWarehouseKey);
...
function generateRow2($arrCOAValue, &$arrIncomeTotal, $reportType, $totalGroupYOY = 0)
{
    ...
    foreach ($arrCOAValue as $coakey => $row) {
        ...
        if (in_array($reportType, array(1, 2, 3))) {
            $returnVal .= '<td style="text-align:right;' . $parentStyle . '">' . $class->formatNumber($totalAmount) . '</td>';
            array_push($tempExcelCol, array('value' => $totalAmount, 'format' => array('format' => 'decimal')));
        }
...
function generateIncome2($arrCOAType, $startDt, $endDt, $arrWarehouse)
{
    ...
    for ($i = 0; $i < count($rs); $i++) {
        ...
        if ($rs[$i]['debittype'] == 1)
            $rs[$i]['balanceAsPositive'] *= BALANCE_AS_POSITIVE;
        if (BALANCE_AS_POSITIVE == -1 && ($rs[$i]['pkey'] == 5 || $rs[$i]['rootkey'] == 5))
            $rs[$i]['balanceAsPositive'] *= BALANCE_AS_POSITIVE;

        // $date = $chartOfAccount->formatDBDate($startDt, 'Y-m-d');
        // $chartOfAccount->setLog($startDt, true);
        // $chartOfAccount->setLog($startDt, true);
