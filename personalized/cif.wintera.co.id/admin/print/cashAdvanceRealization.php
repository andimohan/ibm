...
$generateReportContent = function ($dataset) {
    ...
    $rsCreated = $employee->getDataRowById($rs[0]['createdby']);
...
$arrCashType[3] = $obj->lang['cost'];*/
    ...
    $html .= ' 
<style>.header-row-header{width: 140px; font-weight: bold}</style>    
<table cellpadding="0" > 
<tr><td><div class="title">' . strtoupper($obj->lang['cashBankRealization']) . '</div></td></tr>
<tr><td><div class="subtitle">' . $rs[0]['code'] . '</div></td></tr>
</table>
<div style="clear:both"></div>
<table style="width: 640px"  cellpadding="0" >
<tr>
    <td style="width:390px; vertical-align:top"><table cellpadding="0" style="width:360px;">
            <tr><td class="header-row-header">' . $obj->lang['date'] . '</td><td style="width:10px; text-align:center">:</td><td style="width:210px">' . $obj->formatDBDate($rs[0]['trdate'], 'd / m / Y') . '</td></tr>
    ...
    <td style="width:280px; vertical-align:top"><strong>' . $obj->lang['note'] . '</strong><br>' . str_replace(chr(13), '<br>', $rs[0]['trdesc']) . '
    </td>
</tr>
</table>
...
<table cellpadding="4" class="table-transaction">';
    $html .= '<tr class="col-header">
			...
			<td style="width:120px">' . $obj->lang['service'] . '</td>
            ...
            <td style="text-align:right;width:60px">PPH</td>
    ...
    for ($i = 0; $i < count($rsDetail); $i++) {
        ...
        $html .= '<tr>
				...
				<td style ="text-align:right">' . $obj->formatNumber($rsDetail[$i]['pphamount']) . '</td>
        ...
        if (!empty($supplierName))
            array_push($arrSup, $supplierName);
        if (!empty($invoiceReference))
            array_push($arrSup, $invoiceReference);
        if (!empty($arrSup))
            array_push($arrScndRow, implode(', ', $arrSup));
        if (!empty($rsDetail[$i]['description']))
            array_push($arrScndRow, str_replace(chr(13), '<br>', $rsDetail[$i]['description']));
...
<tr><td></td><td style="text-align:right;font-weight:bold;" >PPH</td><td style="text-align:right; font-weight:bold;"  >' . $obj->formatNumber($rs[0]['totalpph']*-1) . '</td></tr>
...
';
    ...
    $html .= ' 
        ...
        <tr>
            <td  class="sign-name">'. $rsCreated[0]['name'] .'</td>
            <td class="sign-col-space"></td>
    ...
    return '<div style="font-size:1.1em"><b>'.$html.'</b></div>';
}
?>