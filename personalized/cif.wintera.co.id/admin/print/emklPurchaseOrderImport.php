<?php
includeClass(array('EMKLPurchaseOrder.class.php', 'Employee.class.php'));
...
$generateReportContent = function ($dataset) {
    ...
    $emklJobOrderHeaderImport = new EMKLJobOrderHeader(EMKL['jobType']['import']);
    ...
    $html .= ' 
<table cellpadding="2" > 
<tr><td><div class="title">Purchase Order Import</div></td></tr>
...
<tr><td class="header-row-header">' . $obj->lang['importir'] . '</td><td style="text-align:center">:</td><td>' . $rsEmkl[0]['customername'] . '</td></tr>
...
<table cellpadding="4" class="table-transaction">';
    ...
    array_push($cellArray, array('label' => $obj->lang['service'],'width' => '150px'));
    ...
    $totalWillSaid = $obj->unFormatNumber($obj->formatNumber($rs[0]['grandtotal'], $headerDecimal));
    $sayNumber = ($rs[0]['currencykey'] == CURRENCY['idr']) ? $obj->sayNumber($totalWillSaid) : $obj->sayNumberInEnglish($totalWillSaid);
    ...
    for ($j = 0; $j < count($rsPaymentMethodDetail); $j++) {
        if ($rsPaymentMethodDetail[$j]['amount'] == 0)
            continue;
        $payment .= '<tr>';
