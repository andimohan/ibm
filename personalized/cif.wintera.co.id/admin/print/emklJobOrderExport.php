...
includeClass(array('EMKLJobOrder.class.php', 'EMKLCommission.class.php', 'EMKLCommission.class.php', 'CreditNote.class.php', 'DebitNote.class.php', 'CashAdvanceRealization.class.php'));
...
$generateReportContent = function ($dataset) {
    ...
    $rsShipmentTerm1 = $obj->getShipmentTerm($rs[0]['shipmenttermkey']);
    $rsShipmentTerm2 = $obj->getShipmentTerm($rs[0]['shipmentterm2key']);

    $shipmentTermName = $rsShipmentTerm1[0]['name'] .' - '. $rsShipmentTerm2[0]['name'];

    $rsFreightTerm1 = $obj->getFreightTerm($rs[0]['freighttermkey']);
    $rsFreightTerm2 = $obj->getFreightTerm($rs[0]['freightterm2key']);

    $freightTermName = $rsFreightTerm1[0]['name'] .' - '. $rsFreightTerm2[0]['name'];

    $party = '';
    if (in_array($rs[0]['loadcontainertypekey'], array(EMKL['container']['fcl'], EMKL['container']['trucking'], EMKL['container']['freightcustomfcl'], EMKL['container']['customfcl'])) && $rs[0]['transportationtypekey'] == EMKL['shipping']['sea']) {
        $arrParty = array();
        ...
        $temp = array();
        if (!empty($rs[0]['weight']))
            array_push($temp, $obj->formatNumber($rs[0]['weight'], 2) . ' KG');
        if (!empty($rs[0]['volume']))
            array_push($temp, $obj->formatNumber($rs[0]['volume'], 2) . ' CBM');
    ...
    $emklPurchaseOrderExportAccess = true; //$security->hasSecurityAccess($obj->userkey, $security->getSecurityKey($emklPurchaseOrderExport->securityObject), 10);
    $emklCommissionAccess = true; //$security->hasSecurityAccess($obj->userkey, $security->getSecurityKey($emklCommission->securityObject), 10);
    $creditNoteAccess = true;// $security->hasSecurityAccess($obj->userkey, $security->getSecurityKey($creditNote->securityObject), 10);
    $debitNoteAccess = true; //$security->hasSecurityAccess($obj->userkey, $security->getSecurityKey($debitNote->securityObject), 10);
...
<table cellpadding="2" > 
<tr><td><div class="title">' . $obj->lang['jobOrderSummary'] . ' Export</div></td></tr>
...
<tr><td class="header-row-header">' . $obj->lang['shipper'] . '</td><td style="text-align:center">:</td><td>' . $rs[0]['customername'] . '</td></tr>
<tr><td class="header-row-header">' . $obj->lang['salesman'] . '</td><td style="text-align:center">:</td><td>' . $rs[0]['salesname'] . '</td></tr>
<tr><td class="header-row-header">AJU / PEB</td><td style="text-align:center">:</td><td>' . $rs[0]['aju'] . ' / ' . $rs[0]['peb'] . '</td></tr>
<tr><td class="header-row-header">' . $obj->lang['party'] . '</td><td style="text-align:center">:</td><td>' . $party . '</td></tr>
<tr><td class="header-row-header">' . $obj->lang['shipmentTerm'] . '</td><td style="text-align:center">:</td><td>' . $shipmentTermName . '</td></tr>
</table>
</td>
<td style="width:10px;"></td>
<td style="width:360px;"> 
<table cellpadding="2">
<tr><td class="header-row-header">'. ($rs[0]['transportationtypekey'] == EMKL['shipping']['sea'] ? 'MBL' : 'MAWB') .'</td><td style="width:10px; text-align:center">:</td><td style="width:230px">' . $rs[0]['mblnumber'] . '</td></tr> 
...
<tr><td class="header-row-header">' . $obj->lang['vessel'] . ' / ' . $obj->lang['voyage'] . '</td><td style="text-align:center">:</td><td>' . $rs[0]['feedervesselname'] . ' / ' . $rs[0]['feedernumber'] . '</td></tr>
...
<tr><td class="header-row-header">' . $obj->lang['freightTerm'] . '</td><td style="text-align:center">:</td><td>' . $freightTermName . '</td></tr>
</table>
</td>
</tr>
</table>';
    ...
    <br>';
    $buyingTable .= '<table cellpadding="2" class="table-transaction">';
    ...
    $buyingTable .= $obj->generatePrintTableRow(array('class' => 'col-header', 'docWidth' => '670', 'cell' => $cellArray));
    ...
    $rsBuying = array();
        ...
        $rsBuying = $emklPurchaseOrderExport->searchData('', '', true, ' and ' . $emklPurchaseOrderExport->tableName . '.refkey in (' . $obj->oDbCon->paramString($jobOrderKeys, ',') . ') and ' . $emklPurchaseOrderExport->tableName . '.statuskey in (1,2,3)');

        $arrCADRKey = array_unique(array_column($rsBuying,'refcashadvancekey'));
        $rsCashAdvanceRealizationCol = $cashAdvanceRealization->searchDataRow(array($cashAdvanceRealization->tableName.'.pkey',$cashAdvanceRealization->tableName.'.code'),
                                                                               ' and pkey in ('.$obj->oDbCon->paramString($arrCADRKey, ',').') ');
        $rsCashAdvanceRealizationCol = array_column($rsCashAdvanceRealizationCol, null,'pkey');


        foreach ($rsBuying as $buyingRow) {
						$totalPPN += ($buyingRow['currencykey'] == CURRENCY['idr']) ? $buyingRow['taxvalue'] : $buyingRow['rate'] * $buyingRow['taxvalue'];
				
			
            $rsDetailBuying = $emklPurchaseOrderExport->getDetailWithRelatedInformation($buyingRow['pkey']);
            $status = ($buyingRow['statuskey'] == 1) ? ' * ' : '';

            if (empty($rsDetailBuying))
                continue;
            
            $rsCashAdvanceRealization = $rsCashAdvanceRealizationCol[$buyingRow['refcashadvancekey']];
            $cadrCode = (!empty($rsCashAdvanceRealization['code'])) ? '<br>'.$rsCashAdvanceRealization['code']  : '';

            $buyingTable .= '<tr><td colspan="' . count($cellArray) . '" style="font-style:italic;font-size: 0.9em"><br><br>' . $status . '<span style="font-weight: bold; ">' . $buyingRow['suppliername'] . '</span></td></tr>';
            ...
            for ($i = 0; $i < count($rsDetailBuying); $i++) {
                ...
                $priceInUnit = ($buyingRow['ispriceincludetax'] == 1) ? ($rsDetailBuying[$i]['priceinunit'] - ($buyingRow['taxpercentage']/(100 + $buyingRow['taxpercentage'])) * $rsDetailBuying[$i]['priceinunit']) : $rsDetailBuying[$i]['priceinunit'];
               
                $subtotal = $priceInUnit * $rsDetailBuying[$i]['qty'] * $rate;   
                $buyingTable .= '<tr>
                                    <td>' . $buyingRow['code'] .$cadrCode. '</td>
                            ...
                            <td style ="text-align:right">' . $obj->formatNumber($priceInUnit, -2) . '</td> 
        ...
        $buyingTable .= '<tr> <td colspan="' . count($cellArray) . '"></td></tr>';
   

    $buyingTable .= '</table>';
	$buyingTable .= '<table cellpadding="3">
					<tr><td style="font-style:italic" colspan="' . (count($cellArray) - 1) . '">*) Pending Approval</td><td style="font-weight:bold">Subtotal</td><td style="text-align:right; font-weight: bold">' . $obj->formatNumber($totalBuying) . '</td></tr>
					<tr><td style="font-style:italic" colspan="' . (count($cellArray) - 1) . '"></td><td style="font-weight:bold">PPN</td><td style="text-align:right; font-weight: bold">' . $obj->formatNumber($totalPPN) . '</td></tr>
					<tr><td style="font-style:italic" colspan="' . (count($cellArray) - 1) . '"></td><td style="font-weight:bold">Total</td><td style="text-align:right; font-weight: bold">' . $obj->formatNumber($totalBuying+$totalPPN) . '</td></tr>
			 </table>';
	
	$totalBuying += $totalPPN;
    ...
    // REFUND
    $refundTable = '<div style="text-align:left" class="subtitle"><strong>' . strtoupper($obj->lang['purchaseRefund']) . '</strong></div>
    <br>';

    $refundTable .= '<table cellpadding="2" class="table-transaction">';
    ...
    array_push($cellArray, array('label' => $obj->lang['currencyRate'], 'align' => 'right', 'width' => '50'));
    array_push($cellArray, array('label' => $obj->lang['subtotal'] . ' (IDR)', 'align' => 'right', 'width' => '90'));
    $refundTable .= $obj->generatePrintTableRow(array('class' => 'col-header', 'docWidth' => '670', 'cell' => $cellArray));
    ...
    foreach ($rsRefund as $buyingRow) {
        ...
        $refundTable .= '<tr><td colspan="' . count($cellArray) . '" style="font-style:italic;font-size: 0.9em"><br><br>' . $status . '<span style="font-weight: bold; ">' . $buyingRow['suppliername'] . '</span></td></tr>';
    ...
    $refundTable .= '<tr> <td colspan="' . count($cellArray) . '"></td></tr>';

    
    $refundTable .= '</table>';
    $refundTable .= '<table cellpadding="3"><tr><td style="font-style:italic" colspan="' . (count($cellArray) - 1) . '">*) Pending Approval</td><td style="text-align:right; font-weight: bold">' . $obj->formatNumber($totalRefund) . '</td></tr></table>';
 
    $refundTable .= '<div></div>';

    if ($emklCommissionAccess && !empty($rsRefund))
        $html .= $refundTable;
    ...
    $cnTable .= '<table cellpadding="2" class="table-transaction">';
    ...
    array_push($cellArray, array('label' => $obj->lang['currencyRate'], 'align' => 'right', 'width' => '50'));
    array_push($cellArray, array('label' => $obj->lang['subtotal'] . ' (IDR)', 'align' => 'right', 'width' => '90'));
    $cnTable .= $obj->generatePrintTableRow(array('class' => 'col-header', 'docWidth' => '670', 'cell' => $cellArray));
    ...
    foreach ($rsCN as $key => $row) {
        // kalo JO keynya gk terdaftar, continue

        //$rsCNProportion = $creditNote->getProportionalAmount($row);	

        $status = ($row[0]['statuskey'] == 1) ? ' * ' : '';
        $cnTable .= '<tr><td colspan="' . count($cellArray) . '" style="font-style:italic;font-size: 0.9em"><br><br>' . $status . '<span style="font-weight: bold; ">' . $row[0]['customername'] . '</span></td></tr>';

        foreach ($row as $detailRow) {
            //if ($jokey <> $rs[0]['pkey']) continue;
    ...
    $cnTable .= '<tr> <td colspan="' . count($cellArray) . '"></td></tr>';
    

    $cnTable .= '</table>';
    $cnTable .= '<table cellpadding="3"><tr><td style="font-style:italic" colspan="' . (count($cellArray) - 1) . '">*) Pending Approval</td><td style="text-align:right; font-weight: bold">' . $obj->formatNumber($totalCN) . '</td></tr></table>';
 
    $cnTable .= '<div></div>';

    if ($creditNoteAccess && !empty($rsCN))
        $html .= $cnTable;
...
<br>';
    ...
    $dnTable .= '<table cellpadding="2" class="table-transaction">';
    ...
    array_push($cellArray, array('label' => $obj->lang['currencyRate'], 'align' => 'right', 'width' => '50'));
    array_push($cellArray, array('label' => $obj->lang['subtotal'] . ' (IDR)', 'align' => 'right', 'width' => '90'));
    $dnTable .= $obj->generatePrintTableRow(array('class' => 'col-header', 'docWidth' => '670', 'cell' => $cellArray));
...
// jgn per customer, agar bisa bedain statusnya
    ...
    foreach ($rsDN as $key => $row) {
        ...
        $dnTable .= '<tr><td colspan="' . count($cellArray) . '" style="font-style:italic;font-size: 0.9em"><br><br>' . $status . '<span style="font-weight: bold; ">' . $row[0]['suppliername'] . '</span></td></tr>';
    ...
    $dnTable .= '<tr> <td colspan="' . count($cellArray) . '"></td></tr>';

    $dnTable .= '</table>';
    $dnTable .= '<table cellpadding="3"><tr><td style="font-style:italic" colspan="' . (count($cellArray) - 1) . '">*) Pending Approval</td><td style="text-align:right; font-weight: bold">' . $obj->formatNumber($totalDN) . '</td></tr></table>';
    $dnTable .= '<div></div>';

    if ($debitNoteAccess && !empty($rsDN))
        $html .= $dnTable;



    $html .= '<div style="clear:both; text-align:left" class="subtitle"><strong>' . strtoupper($obj->lang['salesOrder']) . '</strong></div>
<br>';
    ...
    // kalo ad tax, biar muat deskrpsi turunin saja
//    if(!$updateTaxAtJobOrder)
//        array_push($cellArray, array('label' => $obj->lang['description']));
    ...
    if($updateTaxAtJobOrder){ 
        array_push($cellArray, array('label' => $obj->lang['beforeTax'], 'align' => 'right', 'width' => '75'));
        array_push($cellArray, array('label' => $obj->lang['tax'], 'align' => 'right', 'width' => '60'));
    }
    ...
    for ($i = 0; $i < count($rsDetail); $i++) {
        ...
        for ($j = 0; $j < count($rsItemDetail); $j++) {
            ...
            $tabelItem .= '<tr>
                            ...
                            <td></td>
                            <td>' . $rsService[$rsItemDetail[$j]['servicekey']] .  '<br>'.$rsItemDetail[$j]['trdesc']  . '</td>';
            
//            if(!$updateTaxAtJobOrder)
//                $tabelItem .= '<td>' . $rsItemDetail[$j]['trdesc'] . '</td>';
            
            $tabelItem .= '<td style ="text-align:center">' . $rsCurrency[$rsItemDetail[$j]['currencykey']] . '</td>';
            
            if(!$updateTaxAtJobOrder)
                $tabelItem .= ' <td style ="text-align:right">' . $obj->formatNumber($rsItemDetail[$j]['priceinunit'], -2) . '</td> ';

            $tabelItem .= ' <td style ="text-align:right">' . $obj->formatNumber($rate, -2) . '</td>';
                            
            
            if($updateTaxAtJobOrder){
                   $tabelItem .= ' <td style ="text-align:right">' . $obj->formatNumber($rsItemDetail[$j]['beforetaxdetailvalue'], -2) . '</td>';
                   $tabelItem .= ' <td style ="text-align:right">' . $obj->formatNumber($rsItemDetail[$j]['taxdetailvalue'], -2) . '</td>';
            }
                
                   
            $tabelItem .= '<td style ="text-align:right">' . $obj->formatNumber($rsItemDetail[$j]['subtotal']) . '</td> 
                        </tr>
                        ';
            
            $totalTaxDetail +=  ($rsItemDetail[$j]['taxdetailvalue']*$rate);
    ...
    $balance = $totalSelling  - $totalTaxDetail - $totalBuying - $totalRefund - $totalCN + $totalDN;
    ...
    $tabelItem .= '<table cellpadding="3" style="width:655px;">
                    <tr><td  colspan="' . (count($cellArray) - 2) . '"></td><td style="text-align:right; font-weight: bold; width: 60px">' . $obj->formatNumber($totalTaxDetail) . '</td><td style="text-align:right; font-weight: bold; width: 90px">' . $obj->formatNumber($totalSelling) . '</td></tr>
