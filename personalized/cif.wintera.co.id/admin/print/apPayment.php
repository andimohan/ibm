...
includeClass(array('APPayment.class.php','EMKLPurchaseOrder.class.php', 'EMKLCommission.class.php','EMKLJobOrder.class.php','Employee.class.php','Tax.class.php' ));
...
$rsTax = $tax->searchDataRow(array($tax->tableName.'.pkey', $tax->tableName.'.name'),
                            ' and ' . $tax->tableName . '.typekey=' . $tax->oDbCon->paramString(TAX_TYPE['PPH']) . ' and ' . $tax->tableName . '.statuskey = 1'
                            );
$rsTax = array_column($rsTax,'name','pkey');
...
array_push($cellArray, array('label' =>'PPh', 'align' => 'right', 'width' => '90'));
...
if ($rs[0]['payabletax23'] != 0){
    ...
    for ($i=0;$i<count($rsDetail);$i++){   
        ...
        if(!isset($arrPPHValue[$pphTypeKey])) $arrPPHValue[$pphTypeKey] = 0;
        $arrPPHValue[$pphTypeKey] += $rsDetail[$i]['taxamount'];
    }
    
    foreach($arrPPHValue as $pphkey=>$pphRow)
        array_push($arrSubtotal, '<tr><td style="text-align:right; font-weight:bold;">'.ucwords($rsTax[$pphkey]).'</td><td style="text-align:right; font-weight:bold;"  >'.$obj->formatNumber($pphRow * -1).'</td></tr>');
}
