...
//									}
        ...
        case $arPaymentTableKey : $rsTemp = $refObj->getDetailWithRelatedInformation($rs[0]['refkey']);  
                                ...
                                $rsDetailObj = array();
                                $rsDetailObj[0]['description'] = implode('<br>',$arDesc);
                                $rsDetailObj[0]['amount'] = $rs[0]['amount'];
    ...
    switch($rs[0]['reftabletype']){
            ...
            case $cashAdvanceRealizationKey : $description = '<b>'.$rs[0]['refcode'].'</b>'; 
                                   $description .= '<br>'.$obj->lang['cashAdvanceRealization']. ' ' . $rsDetailObj[0]['description'];
                                      break; 
			case $cashBankInTableKey : // harusnya udah gk ad cost name
										$arrDesc = array();
                                        if(empty($rsDetailObj[$i]['revenuename'])) $rsDetailObj[$i]['revenuename'] = 'Ayat Silang'; // harusnya kalo kosong itu ayat silang
            
										if(!empty($rsDetailObj[$i]['revenuename'])) array_push($arrDesc ,$rsDetailObj[$i]['revenuename']);
			...
			case $cashBankOutTableKey : 
										...
										$arrDesc = array();
                                        if(empty($rsDetailObj[$i]['costname'])) $rsDetailObj[$i]['costname'] = 'Ayat Silang'; // harusnya kalo kosong itu ayat silang
...
if (TABLEKEY_SETTINGS[$rsKey['key']]['printjournal'] == 1) {

    $rsGeneralJournal = $generalJournal->searchDataRow(array($generalJournal->tableName.'.pkey'),
                                                   ' and statuskey in (2,3)
                                                     and '.$generalJournal->tableName.'.reftabletype = '.$generalJournal->oDbCon->paramString($rs[0]['reftabletype']).' 
                                                     and '.$generalJournal->tableName.'.refkey = '.$generalJournal->oDbCon->paramString($rs[0]['refkey']).'
                                                   '
                                                  );
    $rsGeneralJournalDetail = $generalJournal->getDetailWithRelatedInformation($rsGeneralJournal[0]['pkey']); 
    $html .= ' 
    <div><b>'.$refObj->lang['generalJournal'].'</b></div>
    <table  cellpadding="4" class="table-transaction">
    <tr class="col-header" ><td style="width:450px;">'.$refObj->lang['account'].'</td><td style="width:110px; text-align:right;" >'.$refObj->lang['debit'].'</td><td style="text-align:right; width:110px;">'.$refObj->lang['credit'].'</td></tr>';
    for ($i=0;$i<count($rsGeneralJournalDetail);$i++){ 
      $html .= '<tr><td>'.$rsGeneralJournalDetail[$i]['coacodename'].'</td><td style="text-align:right">'.$obj->formatNumber($rsGeneralJournalDetail[$i]['debit']).'</td><td style="text-align:right">'.$obj->formatNumber($rsGeneralJournalDetail[$i]['credit']).'</td></tr>' ; 
    }
    $html .= '</table>' ;
}
...
array_push($arrSign, array($obj->lang['created'] , $user) );
...
array_push($arrSign, array($obj->lang['approved'] , $user) );

array_push($arrSign, array($obj->lang['received'], $recipient));

$html .= $obj->generateSignLabel($rs,$arrSign); 
