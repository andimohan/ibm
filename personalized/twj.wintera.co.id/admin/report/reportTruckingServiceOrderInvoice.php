...
includeClass(array('AR.class.php','TruckingServiceOrderInvoice.class.php', 'TruckingServiceWorkOrder.class.php'));
...
$arrDataStructure['receiptdate'] = array('title'=>ucwords($obj->lang['receiptDate']),'dbfield' => 'receiptdt', 'width'=>"120px",'format'=>'date','returnOnEmpty' => array('returnOnEmpty' => true, 'value' => ''));
...
$arrDataStructure['customerTaxId'] = array('title'=>'NPWP','dbfield' => 'customertaxid', 'width'=>"200px", "sortable" => false); 
...
// if(!$isGrouping){ 
// }
$arrDataStructure['route'] = array('title'=>ucwords($obj->lang['route']), 'width'=>"250px",'dbfield' => 'routes');
...
if ($isGrouping && $isShowDetail){ 
    ...
    $arrDataDetailStructure['route'] = array('title'=>ucwords($obj->lang['route']),'dbfield' => 'routes', 'width'=>"250px");
...
if (isset($_POST) && !empty($_POST['hidAction'])){
    ...
    $rsWorkOrder = $truckingServiceWorkOrder->searchData('','',true,' and '.$truckingServiceWorkOrder->tableName.'.statuskey = 3');
		
    $rsWorkOrderRefCols = $obj->reindexDetailCollections($rsWorkOrder,'refkey');
    $rsWorkOrderKeyCols = $obj->reindexDetailCollections($rsWorkOrder,'pkey');
    ...
    for( $i=0;$i<$totalRs;$i++) {   
        ...
        for($j=0;$j<$totalRsDetail;$j++){
            $salesorderkey = $rsDetail[$j]['salesorderkey'];
            $workorderkey = $rsDetail[$j]['workorderkey'];
                    
            $rsWorkOrderCol = array();
            if(empty($workorderkey)) {
                $rsWorkOrderCol = $rsWorkOrderRefCols[$salesorderkey];
            } else {
                $rsWorkOrderCol = $rsWorkOrderKeyCols[$workorderkey];
            }

            foreach ($rsWorkOrderCol as $workOrder) {
                $route = $workOrder['routefrom'] . (!empty($workOrder['routeto']) ? ' - ' . $workOrder['routeto'] : '') . (!empty($workOrder['routelast']) ? ' - ' . $workOrder['routelast'] : '');
                array_push($arrRoutes, $route);
            }
        }
        ...
        if($isGrouping){ 
            ...
            if ($isShowDetail){
                ...
                for($j=0;$j<$totalRsDetail;$j++){
                    ...
                    $routes = array_unique($arrRoutes);
                    $rs[$j]['routes'] = implode(',<br>', $routes);
                    ...
                    $arrDetailRoutes = array();
                    foreach ($rsWorkOrderCol as $workOrder) {
                        $route = $workOrder['routefrom'] . (!empty($workOrder['routeto']) ? ' - ' . $workOrder['routeto'] : '') . (!empty($workOrder['routelast']) ? ' - ' . $workOrder['routelast'] : '');
                        array_push($arrDetailRoutes, $route);
                    }
                    ...
                    $routes = array_unique($arrDetailRoutes);
                    $rsDetail[$j]['routes'] = implode(',<br>', $routes); 
