...
<td style="width:390px">
<table cellpadding="2" >
<tr><td class="header-row-header">'.$obj->lang['carRegistrationNumber'].'</td><td style="width:10px; text-align:center">:</td><td>'.$rsCar[0]['policenumber'].'</td></tr> 
...
array_push($cellArray, array('label' => $obj->lang['description'], 'width' => '150'));
...
for ($i=0;$i<count($rsDetail);$i++){  
    ...
    $html .= '<tr><td>'.$rsDetail[$i]['itemname'] .'</td><td>'.$rsDetail[$i]['trdesc'] .'</td><td style="text-align:right">'.$obj->formatNumber($rsDetail[$i]['qty']).'</td><td>'. $rsDetail[$i]['unitname'] .'</td><td style="text-align:right">'.$obj->formatNumber($rsDetail[$i]['priceinunit']).'</td>'.$isDiscount.'<td style="text-align:right">'.$obj->formatNumber($rsDetail[$i]['total']).'</td></tr>' ; 
}
$html .= '</table>';
...
<div style="clear:both"></div> 
<table cellpadding="4"> 
<tr><td rowspan="'.$ctr.'" style="width:460px"><strong>Terbilang</strong> :<br>'.ucwords($sayNumber).' Rupiah.'.$top.'</td><td style="text-align:right; font-weight:bold;  width:100px; ">'.$totalLabel.'</td><td style="text-align:right; font-weight:bold;  width:110px;"  >'.$obj->formatNumber($rs[0]['subtotal']).'</td></tr>
';
