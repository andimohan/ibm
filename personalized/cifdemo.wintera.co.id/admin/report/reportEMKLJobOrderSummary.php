...
if (isset($_POST) && !empty($_POST['hidAction'])) {
    ...
    $rsTotalBuying = $obj->getAmountCost($arrPkey, $asIDR);
    $rsTotalBunyingCols = $obj->reindexDetailCollections($rsTotalBuying, 'jokey');
     ...
     if(!$isShowFunctionalValue) {
        for ($i = 0; $i < $totalRs; $i++) {   
            ...
            foreach($rsCurrency as $currRow) {
                ...
                $grossProfit = $totalSelling - $totalBuying - $employeeCommission - $totalRefund - $totalCreditNote + $totalDebitNote;
    ...
    for ($i = 0; $i < $totalRs; $i++) {
        ...
        $rsTotalBuyingCol = (isset($rsTotalBunyingCols[$sokey])) ? $rsTotalBunyingCols[$sokey] : array();
        ...
        if($isShowFunctionalValue) {
            ...
            $grossProfit = $totalSelling - $totalBuying - $totalEmployeeCommission - $totalRefund - $totalCreditNote + $totalDebitNote;
