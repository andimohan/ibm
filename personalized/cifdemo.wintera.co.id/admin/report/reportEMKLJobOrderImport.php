...
includeClass(array('EMKLJobOrder.class.php', 'CurrencyRate.class.php'));
...
$arrDataStructure['created'] = array('title' => ucwords($obj->lang['createdBy']), 'dbfield' => 'createdname', 'width' => "200px");
...
//	}
    ...
    if (isset($_POST) && !empty($_POST['selCurrency'])) {
        ...
        $arrCurrencyRate = $currencyRate->getCurrencyLastRate($_POST['selCurrency'], $_POST['trStartDate']);
...
//        $rs[$i]['totalinvoiced'] = ($rs[$i]['totalinvoiced'] / $rate);
        $rs[$i]['totalbuying'] = ($rs[$i]['totalbuying'] / $rate);
        $rs[$i]['totalcommission'] = ($rs[$i]['totalcommission'] / $rate);
        $rs[$i]['grossprofit'] = ($rs[$i]['grossprofit'] / $rate);
        $rs[$i]['totalcreditnote'] = ($rs[$i]['totalcreditnote'] / $rate);
        ...
        if (isset($rsInvoiceInformation[$sokey])) {
            foreach ($rsInvoiceInformation[$sokey] as $invoiceRow) {
                ...
                $rs[$i]['totalinvoiced'] += (($invoiceRow['amount']+$invoiceRow['taxvalue']) / $rate);
                $rs[$i]['totaltax23value'] += ($invoiceRow['tax23value'] / $rate);
            ...
            if ($invoiceRow['ispriceincludetax'] == 1) {
                ...
                $beforeTaxInvoice = ($invoiceRow['amount'] - $beforeTax);
                $rs[$i]['totalbeforetaxinvoiced'] += ($beforeTaxInvoice / $rate);
            } else {
                // $rs[$i]['totalbeforetaxinvoiced'] += $invoiceRow['amount']; 

                $rs[$i]['totalbeforetaxinvoiced'] += ($invoiceRow['amount'] / $rate);
            }
        }
        ...
        if ($isShowDetail || !empty($detailCriteria)) {
            ...
            if (!empty($detailCriteria) && empty($rsDetail))
                continue;
...
echo $twig->render('@custom/reportEMKLJobOrderImport.html', $arrTwigVar);
