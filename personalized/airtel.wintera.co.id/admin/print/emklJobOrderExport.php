...
$employeeCommission = new APEmployeeCommission();
...
$rsEmployeeCommission = $employeeCommission->searchData('','', true, ' and '.$employeeCommission->tableName.'.reftabletype = '.$obj->oDbCon->paramString($tableKey).'  and '.$employeeCommission->tableName.'.refkey in ('.$obj->oDbCon->paramString($jobOrderKeys,',').') and '.$employeeCommission->tableName.'.statuskey in (1,2,3)');
...
$tabelItem .= '<tr><td colspan="'.count($cellArray).'" style="font-style:italic;font-size: 0.9em"></td></tr>';
...
$tabelItem .= '<tr>
					...
					<td></td>
					<td>'.$obj->lang['adminFee'].'</td>
					<td style ="text-align:center">'.$rsCurrency[CURRENCY['idr']].'</td>
					<td style ="text-align:right">'.$obj->formatNumber($rs[0]['totalemployeecommission']).'</td> 
					<td style ="text-align:right">1</td>  
					<td style ="text-align:right">'.$obj->formatNumber($rs[0]['totalemployeecommission']).'</td> 
				</tr>';
$totalBuying += $rs[0]['totalemployeecommission'];
...
if(!empty($rsRefund)){
		$html .='<div style="text-align:left" class="subtitle"><strong>'.strtoupper($obj->lang['purchaseRefund']).'</strong></div>
	<br>';
...
$totalCN = 0; 
if(!empty($rsCN)){
	$html .='<div style="text-align:left" class="subtitle"><strong>'.strtoupper($obj->lang['creditNote']).'</strong></div>
	<br>';
...
//	foreach($rsCNGroup as $key=>$row){
//	
//		$rsCNProportion = $creditNote->getProportionalAmount($row);	
//		
//		$status = ($detailRow['statuskey'] == 1) ? ' * ' : '';
//		$tabelItem .= '<tr><td colspan="'.count($cellArray).'" style="font-style:italic;font-size: 0.9em"><br><br>'.$status.'<span style="font-weight: bold; ">'.$row[0]['customername'].'</span></td></tr>';
// 
//		foreach($row as $detailRow){
//			$totalCredit = $detailRow['totalcredit'];
...
$totalDN = 0; 
if(!empty($rsDN)) {
	$html .='<div style="text-align:left" class="subtitle"><strong>'.strtoupper($obj->lang['debitNote']).'</strong></div>
	<br>';
...
for($i=0;$i<count($rsDetail);$i++){
    ...
    $invoiceKey = array_column($rsInvoice,'pkey');
    $rsInvoiceItem = $emklOrderInvoice->getItemDetail($invoiceKey,'refheaderkey');
  
    // karena masih ad bugs di frmework, dimana ketika delete detail, level ke 2 gk kehapus di database
    // sementar cek ulang
    $rsInvoiceDetail = $emklOrderInvoice->getDetailByColumn('','',true,' and '.$emklOrderInvoice->tableNameDetail.'.refkey in ('.$emklOrderInvoice->oDbCon->paramString($invoiceKey,',').') ');
    $arrInvoiceDetailKey = array_column($rsInvoiceDetail, 'pkey');    
        
	if(empty($rsInvoiceItem)) continue;
    ...
    for($j=0;$j<count($rsInvoiceItem);$j++){ 

        // karena masih ad bugs di frmework, dimana ketika delete detail, level ke 2 gk kehapus di database
        // sementar cek ulang
        if(!in_array($rsInvoiceItem[$j]['refkey'],$arrInvoiceDetailKey)) continue;
        
        $qty = $rsInvoiceItem[$j]['qtyinbaseunit'];
        $priceIUnUnit = $rsInvoiceItem[$j]['priceinunit'];
        $rate = ($rsInvoiceItem[$j]['currencykey'] == CURRENCY['idr'] ) ? 1 : $rsInvoiceItem[$j]['rate'] ;
  
        // kalo invoice ny dalam usds, kali ulang dengan rate nya
        
        $sellingSubtotal = $qty * $priceIUnUnit * $rate;// kalo pake field total, akan ngebug kalo invoiceny ditgih dlm dollar
        if ($rsInvoiceItem[$j]['ispriceincludetax'] == 1)
            $sellingSubtotal -= ($rsInvoiceItem[$j]['taxdetail']/(100 + $rsInvoiceItem[$j]['taxdetail'])) * $sellingSubtotal;
            
            
        //$obj->setLog($rsInvoiceItem[$j]['total'] .'*'. $rate,true);
        
        $tabelItem .= '<tr>
                            <td>'.$rsDetail[$i]['code'].'</td>
                            <td>'.$rsContainer[$rsInvoiceItem[$j]['containerkey']].'</td>
                            <td style ="text-align:right">'.$obj->formatNumber($rsInvoiceItem[$j]['qtyinbaseunit'],-2).'</td>
                            <td></td>
                            <td>'.$rsService[$rsInvoiceItem[$j]['itemkey']].'</td>
                            <td style ="text-align:center">'.$rsCurrency[$rsInvoiceItem[$j]['currencykey']].'</td>
                            <td style ="text-align:right">'.$obj->formatNumber($rsInvoiceItem[$j]['priceinunit'],-2).'</td> 
                            <td style ="text-align:right">'.$obj->formatNumber($rate,-2).'</td> 
                            <td style ="text-align:right">'.$obj->formatNumber($sellingSubtotal).'</td> 
                        </tr>
                        ';
        $totalSelling += $sellingSubtotal;
    }
}  
...
$balance = $totalSelling - $totalBuying - $totalAPCommission - $totalRefund - $totalCN + $totalDN ;
