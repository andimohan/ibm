...
array_push($cellArray, array('label' => $obj->lang['qty'], 'align' => 'right', 'width' => '50'));
...
$generateCostTable = function ($obj, $rs, $outsource = 0, &$totalCost){
    ...
    for($i=0;$i<count($rsCost);$i++){
        ...
        $qty = $rsCost[$i]['qty']; 
        $taxPercentage =  $rsCost[$i]['taxpercentage']; 
        ...
        <td class="'.$borderStyle.'" style="text-align:right">'.$obj->formatNumber($taxPercentage).' %</td>  
        <td class="'.$borderStyle.'" style="text-align:right">'.$obj->formatNumber($subtotal).'</td>  
        </tr>
        ';
...
$generateReimbursementCost = function ($obj,$rs, &$totalCost){
    ...
    array_push($cellArray, array('label' => $obj->lang['PPN'],'align' => 'right', 'width' => '40'));
    array_push($cellArray, array('label' => $obj->lang['total'],'align' => 'right', 'width' => '80')); 
    $html .= $obj->generatePrintTableRow( array('class' => 'col-header','docWidth' => '330', 'cell' =>  $cellArray));  
...
array_push($cellArray, array('label' => $obj->lang['description']));
array_push($cellArray, array('label' => $obj->lang['qty'], 'align' => 'right', 'width' => '50'));
array_push($cellArray, array('label' => $obj->lang['cost'],'align' => 'right', 'width' => '70'));
array_push($cellArray, array('label' => $obj->lang['PPN'],'align' => 'right', 'width' => '40')); 
array_push($cellArray, array('label' => $obj->lang['total'],'align' => 'right', 'width' => '80')); 
...
$useRealization = $obj->useRealization();
if($useRealization)    
    $html .= '<br><div style="font-style:italic">*) Belum direalisasi</div>';    
...
<table cellpadding="4">
 <tr>
    ...
    <td style="width: 100px; text-align:right; ">SURAT JALAN</td>
    <td style="width: 30px; border:1px solid #333"></td>
    <td style="width: 100px; text-align:right;">LIFT ON</td>
    <td style="width: 30px; border:1px solid #333"></td>
    <td style="width: 100px; text-align:right;">KASBON</td>
    <td style="width: 30px; border:1px solid #333"></td>
    <td style="width: 130px; text-align:right;">SI / DO</td>
    <td style="width: 30px; border:1px solid #333"></td>
 </tr> 
  <tr>
    <td></td>
    <td style="text-align:right; ">INV. VENDOR</td>
    <td style="border:1px solid #333"></td>
    <td style="text-align:right;">LIFT OFF</td>
    <td style="border:1px solid #333"></td>
    <td style="text-align:right;">NPE / PEB</td>
    <td style="border:1px solid #333"></td>
    <td style="text-align:right; font-size:0.8em">FAKTUR PAJAK (DEPO)</td>
    <td style="border:1px solid #333"></td>
 </tr>
</table> 
<br>
...
<table cellpadding="2">
 <tr>
    <td style="width: 250px; border:1px solid #333; text-align:center;" colspan="2">KASBON</td>
    <td style="width: 130px; border:1px solid #333; text-align:center;">PREPARE BY</td>
    <td style="width: 130px; border:1px solid #333; text-align:center;">CHECKED BY</td>
    <td style="width: 170px; border:1px solid #333; text-align:center;" colspan="2">APPROVED BY</td> 
 </tr> 
  <tr>
    <td style=" border:1px solid #333; height: 60px;"></td>
    <td style=" border:1px solid #333;"></td>
    <td style=" border:1px solid #333;"></td>
    <td style=" border:1px solid #333;"></td>
    <td style=" border:1px solid #333;"></td>
 ...
 <tr>
    <td style="width: 250px; border:1px solid #333; text-align:left; font-size:0.8em" colspan="2">OPS:</td>
    <td style="width: 130px; border:1px solid #333; text-align:left; font-size:0.8em">FAT:</td>
    <td style="width: 130px; border:1px solid #333; text-align:left; font-size:0.8em">ADMIN-OPS:</td>
    <td style="width: 170px; border:1px solid #333;" colspan="2"></td> 
 </tr> 
 </table>
 </div>
'; 
...
$generateContainerTable = function ($obj,$rs){
    ...
    array_push($cellArray, array('label' => $obj->lang['supplier'] ));
    ...
    for($i=0;$i<count($rsSPK);$i++){   
        $rsCarDetail = $serviceWorkOrder->getCarDetail($rsSPK[$i]['pkey']); 
        
        $rsSupplier = $supplier->getDataRowById($rsSPK[$i]['supplierkey']);
        $driverName = $rsSupplier[0]['name'];
        
        $borderStyle = 'col-border-bottom';
        foreach($rsCarDetail as $carRow)
            $html  .= '<tr>
                        <td class="'.$borderStyle.'">'.$carRow['container'].'</td>
                        <td class="'.$borderStyle.'">'.$carRow['seal'].'</td>
                        <td class="'.$borderStyle.'">'.$carRow['carregistrationnumber'].'</td>
...
$costTable .= ' <br><br>'.$containerTable;
