...
$splitReimbursement = 1; // blm bisa by option, karena harus update header segala //(isset($_POST) && !empty($_POST['isSplitReimburse']) && $_POST['isSplitReimburse'] == 1) ? 1 : 0;
...
if(count($rsCurrency) == 1){
    ...
    $arrDataStructure['amount'] = array('title'=>ucwords($obj->lang['total']),'dbfield' => 'totalamount', 'width'=>"130px" , "sortable" => false,'format'=>'number','calculateTotal' => true);
}else{
...
if($splitReimbursement == 1){    
    ...
    if(count($rsCurrency) == 1){
        $arrDataStructure['totalselling'] = array('title'=> ucwords($obj->lang['selling']). ' ' .$currRow['name'],  'width'=>"110px", 'dbfield' => 'totalselling'.$currRow['pkey'],  "sortable" => false, "format" => 'number','calculateTotal' => true, 'textColor' => '568203');   
        $arrDataStructure['totalreimbursement'] = array('title'=> ucwords($obj->lang['reimburse']). ' ' .$currRow['name'],  'width'=>"110px", 'dbfield' => 'totalreimbursement'.$currRow['pkey'],  "sortable" => false, "format" => 'number','calculateTotal' => true, 'textColor' => '0093AF');    
    } else{
       foreach($rsCurrency as $currRow) {
...
if (isset($_POST) && !empty($_POST['hidAction'])){   
    ...
    if($splitReimbursement == 1){
        ...
        $rsInvoice = $emklOrderInvoice->searchData('','',true, ' and '.$emklOrderInvoice->tableName.'.pkey in ('.$class->oDbCon->paramString(array_column($rs,'refheaderkey'),',').')');
    ...
    foreach($rs as $row){
        ...
        $arrAging = array();
         ...
         if($splitReimbursement == 1){    
             ...
             if(!isset($rsInvoice[$row['refheaderkey']])){
                 $arrIndex = 'totalselling' ;
             }else{ 
                 // pisah per selling/ invoice
                 $arrInv = $rsInvoice[$row['refheaderkey']];
