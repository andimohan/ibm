...
$generateReportContent = function ($dataset) {
    ...
    $invoiceType = (isset($_GET['type']) ? $_GET['type'] : 1);
    ...
    $html .='<table><tr>
                        ...
                        <td style="width:215px;text-align:left; font-size:0.9em;"><h3>' . strtoupper($obj->loadSetting('companyName')) . '</h3><br>' . nl2br($obj->loadSetting('companyAddress')) . '</td>
                        </tr></table></td>
					</tr></table>';
    ...
    <table cellpadding="3" style="width:100%l; font-size:0.9em"> 
            <tr>
                <td style="font-size:10px;font-weight:bold;width:25px;text-align:center;border-left:1px solid black;border-top:1px solid black;border-bottom:1px solid black; text-align:right">NO</td>
                <td style="font-size:10px;font-weight:bold;width:70px;text-align:center;border-left:1px solid black;border-top:1px solid black; border-bottom:1px solid black;">TANGGAL</td>
                <td style="font-size:10px;font-weight:bold;width:115px;text-align:center;border-left:1px solid black;border-top:1px solid black; border-bottom:1px solid black;">TUJUAN</td>
                <td style="font-size:10px;font-weight:bold;width:65px;text-align:center;border-left:1px solid black;border-top:1px solid black; border-bottom:1px solid black;">JENIS MOBIL</td>
                <td style="font-size:10px;font-weight:bold;width:65px;text-align:center;border-left:1px solid black;border-top:1px solid black; border-bottom:1px solid black;">NO POL</td>
                ...
                <td style="font-size:10px;font-weight:bold;width:115px;text-align:center;border-left:1px solid black;border-top:1px solid black; border-bottom:1px solid black;">SHIP/SPPK</td>
    ...
    if($invoiceType == 1) {
        //TIPE ONCALL
            ...
            for ($i = 0; $i < count($rsData); $i++) {
                ...
                if (isset($itemIndex[$pkey][$itemkey])) {
                    ...
                    $description = isset($aliasMap[$pkey][$itemkey]) && !empty($aliasMap[$pkey][$itemkey]) ? $aliasMap[$pkey][$itemkey] : '';
                    ...
                    foreach ($rsDetailCargoWOCol as $index => $rsCargoWO) {

                        $displayQty = ($index === 0) ? $obj->formatNumber($qty) : '0';
                        $displaySubtotal = ($index === 0) ? $obj->formatNumber($subtotal) : '0';

                        $carType = $rsData[$i]['carcategoryname'];

                        $destinationCargo = ($rsCargoWO['destination'] ?: $rsCargoWO['destinationname']);

                        // kalo pake outsource 
                        if ($rsData[$i]['isoutsource'] == 1) {
                            $rsData[$i]['policenumber'] = $rsData[$i]['outsourcecarregistrationnumber'];
                            $carType = $rsWorkOrderCol[0]['outsourcecarcategoryname'];
                        }
                        ...
                        $html .= '
                                <tr>
                                    ...
                                    <td style="border-left:1px solid black;border-bottom:1px solid black;">' . $destinationCargo . '</td>
                                    <td style="text-align:center;border-left:1px solid black;border-bottom:1px solid black;">' . $carType . '</td>
                                    <td style="text-align:center;border-left:1px solid black;border-bottom:1px solid black;">' . $rsData[$i]['policenumber'] . '</td>
                                    <td style="text-align:center; border-left:1px solid black;border-bottom:1px solid black;">' . $displayQty . '</td>
                                    <td style="text-align:center; border-left:1px solid black;border-bottom:1px solid black;">' . $rsCargoWO['workorder'] . '</td> 
                                    <td style="text-align:right; border-left:1px solid black;border-bottom:1px solid black;border-right:1px solid black;">' . $displaySubtotal . '</td>
                ...
                if (!empty($rsSellingCol)) {
                    for ($j = 0; $j < count($rsSellingCol); $j++) {
                        ...
                        $rsWorkOrderCol = $rsWorkOrder[$rsSellingCol[$j]['joheaderkey']];
                        ...
                        if ((isset($itemIndex[$pkey][$costkey]))) {
                            ...
                            $destination = ($rsSellingCol[$j]['destination'] ?: $rsSellingCol[$j]['destinationname']);

                            $carType = $rsSellingCol[$j]['carcategoryname'];
                            $policeNumber = $rsSellingCol[$j]['policenumber'];
                            if($rsData[$i]['isoutsource'] == 1) {
                                $carType = $rsWorkOrderCol[0]['outsourcecarcategoryname'];
                                $policeNumber = $rsWorkOrderCol[0]['outsourcecarregistrationnumber'];
                            }
                            ...
                            $html .= '
                                    <tr>
                                        ...
                                        <td style="text-align:center;border-left:1px solid black;border-bottom:1px solid black;">' . $carType . '</td>
                                        <td style="text-align:center;border-left:1px solid black;border-bottom:1px solid black;">' . $policeNumber . '</td>
        ...
        for($k = 0; $k < count($rsJobOrderContract); $k++) {
            ...
            $rsWorkOrderCol = $rsWorkOrder[$rsJobOrderContract[$k]['joheaderkey']];
            ...
            $carType = $rsJobOrderContract[$k]['carcategoryname'];
            $policeNumber = $rsJobOrderContract[$k]['policenumber'];
            if($rsWorkOrderCol[0]['isoutsource'] == 1) {
                $carType = $rsWorkOrderCol[0]['outsourcecarcategoryname'];
                $policeNumber = $rsWorkOrderCol[0]['outsourcecarregistrationnumber'];
            }
        ...
        for ($k = 0; $k < count($rsJobOrderContract); $k++) {
            ...
            $subTotal = $rsDetailCols[0]['amount'];
    ...
    // di round agar gk muncul informasi koma di saynumber
    $totalInvoice = round($rs[0]['grandtotal'] - $rs[0]['tax23value']);
