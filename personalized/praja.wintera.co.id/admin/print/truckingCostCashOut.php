...
array_push($cellArray, array('label' => $obj->lang['costName'], 'width' => '110'));
array_push($cellArray, array('label' => $obj->lang['coalink'], 'width' => '180'));
array_push($cellArray, array('label' => $obj->lang['description']));
array_push($cellArray, array('label' => $obj->lang['cost'], 'align' => 'right', 'width' => '80'));
...
$rsItemCol = $item->searchDataRow(array($item->tableName.'.pkey',$item->tableName.'.code',$item->tableName.'.name',$item->tableName.'.costcoakey'),' and '.$item->tableName.'.pkey in ('. $item->oDbCon->paramString(array_column($rsDetail,'costkey'), ',') .')');
$arrCostCOAKey = array_column($rsItemCol,'costcoakey');
$rsItemCol = array_column($rsItemCol,null,'pkey');
    
// pake costcoakey dulu biar gk berat
// kecuali nanti pake per cateogry seeprti di form
// gk bisa karena gk semuanya diisi,    
    
//$rsCOACol = $chartOfAccount->searchDataRow(array($chartOfAccount->tableName.'.pkey',$chartOfAccount->tableName.'.code',$chartOfAccount->tableName.'.name'),' and '.$chartOfAccount->tableName.'.pkey in ('. $chartOfAccount->oDbCon->paramString($arrCostCOAKey, ',') .')');
//$rsCOACol = array_column($rsCOACol,null,'pkey');    
    
for($i=0;$i<count($rsDetail);$i++){  
    $rsItem = $rsItemCol[$rsDetail[$i]['costkey']]; 
    $coakey = $cost->getCostCOAKeyByJobCategory($rsDetail[$i]['costkey'],$rs[0]['categorykey'],$rs[0]['warehousekey']) ;  
    $rsCOA = $chartOfAccount->getDataRowById($coakey);
    $html .= '<tr><td style="text-align:right">'.$obj->formatNumber($rsDetail[$i]['qty']).'</td> <td>'.$rsItem['name'].'</td><td>'.$rsCOA[0]['code']. ' - '.$rsCOA[0]['name'].'</td><td>'.$rsDetail[$i]['description'].'</td><td style ="text-align:right">'.$obj->formatNumber($rsDetail[$i]['costvalue']).'</td><td style ="text-align:right">'.$obj->formatNumber($rsDetail[$i]['amount']).'</td></tr>';
} 
