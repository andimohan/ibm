...
$generateReportContent = function ($dataset){ 
    ...
    $sayNumber =  $obj->sayNumberInEnglish($rs[0]['beforetaxtotal']); //$obj->sayNumberInEnglish($rs[0]['grandtotal']);
 ...
 /*   if($rs[0]['customcodekey'] == 2 && !empty($rsConsignee)){ 
    ...
    <table cellpadding="2" > 
    <tr><td><div class="title" style="text-decoration:underline; font-size:1.8em">'.strtoupper($rsInvoiceType[0]['title']).'</div></td></tr>
                ...
                for($j=0;$j<count($rsInvoiceItemDetail);$j++){
                    ...
                    if($rsInvoiceItemDetail[$j]['servicecost'] == 1){
                        ...
                        $itemname = $obj->formatNumber($rsInvoiceItemDetail[$j]['qtyinbaseunit']);
                         ...
                         array_push($detailDesc,$rsDetail[$i]['description']); 
                         $itemname = $obj->formatNumber($rsInvoiceItemDetail[$j]['qtyinbaseunit']).'x '.$itemname;
                    }
...
//    	}
        ...
        array_push($arrSubtotalRows, '<td style="text-align:center; font-weight:bold; border:1px solid #000">TOTAL</td><td style="text-align:right; font-weight:bold; border:1px solid #000">'.$obj->formatNumber($rs[0]['beforetaxtotal']).'</td>');
    
        if($rs[0]['totaldownpayment'] <> 0){
            
            //$outstanding = $rs[0]['grandtotal'] - $rs[0]['totaldownpayment']; 
			$outstanding = $rs[0]['beforetaxtotal'] - $rs[0]['totaldownpayment']; 
	...
	$ppnNote = ($rs[0]['taxvalue'] > 0) ? '<br><br><table cellpadding="4" style="border:1px solid #333"><tr><td>PPN DIBEBASKAN SESUAI PP NOMOR 146 TAHUN 2000 SEBAGAIMANA TELAH DIUBAH DENGAN PP NOMOR 38 TAHUN 2003<br><br>Revisi e-faktur dapat dilakukan selama bulan yang sedang berjalan atau selambat-lambatnya s/d tanggal 5 di bulan berikutnya.</td></tr></table>': '';
	
    $html .= ' 
    <table> 
    <tr><td style="width:410px;"></td> 
    <td>
    <table>  
    ...
    <tr><td style="font-weight:bold;">ACC NO</td><td style="font-weight:bold;">:</td><td style="font-weight:bold;">244 020 1477</td></tr>
    <tr><td style="font-weight:bold;">UNDERSIGN</td><td style="font-weight:bold;;">:</td><td style="font-weight:bold;">HENDRA TANDIYONO</td></tr>
    <tr><td style="font-weight:bold;">BRANCH</td><td style="font-weight:bold;">:</td><td style="font-weight:bold;">KCP PLUIT KENCANA</td></tr>
    </table> 
    </div>
    ';
