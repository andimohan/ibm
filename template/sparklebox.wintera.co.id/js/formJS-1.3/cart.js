function Cart(opt){   
    ...
    this.updateCartTotalSummary = function updateCartTotalSummary(){
        // nana function harus sama, karena akan kepanggil dari popup-voucher.js
    }
    ...
    this.loadOnReady = function loadOnReady(){   
        ...
        numeric: {
        message: lang.zipCodeNumber
        },
        stringLength: {
        max: 5,
        min: 5,
        message: lang.zipCodeMust5
        }
        }
        },
        }
        ...
        $(".mnv-delete, .mobile-delete-col").click(function() {
            ...
            if (recipientZipcode && recipientZipcode.length === 5 && !isNaN(recipientZipcode)) {
               ...
               if ($shipmentSelect.val()) {
                   updateShipmentOptions($shipmentSelect.val());
                    ...
                    $('.error-message').text(lang,);
                    $shipmentSelect.prop('disabled', true);
            } else {
               $('.error-message').text(lang.zipCodeRequired);
               $shipmentSelect.prop('disabled', true);
               $btnCo.prop('disabled', true);
            }
        }
   ...
   function checkRate(serviceKey) {
                ...
                const recipientZipcode = $('[name="recipientZipcode"]').val();
                if (recipientZipcode.length < 5 || isNaN(recipientZipcode)) {
                    showError(lang,);
                    $('[name="selShipmentService"]').prop('selectedIndex', 0);
                    hideLoading();
                    $('.price-rate').text('0');
                ...
                $.ajax({
                    ...
                    if (response.price !== undefined) {
                        ...
                        if ($('[name="hidLatLng"]').val() === '') {
                            $('.map-picker').removeClass('hidden');
                            $("body").addClass("no-scroll");
                            $("#maps").removeClass("hidden");
                            initMap('add', '');

                        } else {
                            showError(lang.invalidChooseShipping);
                            $('[name="selShipmentService"]').prop('selectedIndex', 0);
 
                        }

                    if ($('[name="recipientZipcode"]').val() === '' || $('[name="recipientZipcode"]').val().length < 5) {
                            showError(lang.invalidZipCode);
                    }
                        $('.price-rate').text('0');
                        $btnCo.prop('disabled', true);
                        hideLoading();
        ...
        const hidLatLngElement = $('[name="hidLatLng"]')[0];
        if (hidLatLngElement) {
            const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'value') {
                // console.log('hidLatLng value changed via JS, rechecking rate...');
                // updateShipmentOptions($shipmentSelect.val());
                }
            });
            });
            
            observer.observe(hidLatLngElement, {
            attributes: true,
        ...
        function showLoading() {
            $shipmentSelect.prop('disabled', true);
            $btnCo.prop('disabled', true);
            $('.price-rate').text(lang.calculateShipping);
        }
        ...
        function showError(message) {
            alert(message);
             ...
             $('#choose-voucher').on('click', function (e) {
                e.preventDefault(); 
                loadOverlayScreen({ url: '/popup-voucher' }); 
            });
        ...
        $(".btn-map").on("click", function () {
            $("#maps").removeClass("hidden");
            $("body").addClass("no-scroll");
            var pin = $('[name="hidLatLng"]').val();
            initMap('add', pin);
        });


    $('.card-map .btn-close').on('click', function(e) {
        e.preventDefault();
        if ($('[name=hidLatLng]').val() === '') {
            showError(lang.selectShipping);
        ...
        // console.log($('#address-geocoding-add').val());
    });
    ...
    function cartStatusCheck() {
        var $parent = $('.cart-list-panel');
        isCartEmpty = $parent.find('.cart-item').length === 0;  

        if (isCartEmpty) {
            $shipmentSelect.prop('disabled', true);
            $btnCo.prop('disabled', true);
            isCartEmpty = true;
            $('.error-message').text(lang.cartEmpty);
        } else {
            $shipmentSelect.prop('disabled', false);
            $btnCo.prop('disabled', false);
            isCartEmpty = false;
    ...
    if (totalItems === 1) {
        ...
        for (let i = 1; i < totalItems - 1; i++) {
            $cartItems.eq(i).css('border-bottom', '1px solid #001748');
        }
        
        $cartItems.eq(totalItems - 1).css('border-bottom', 'none');
    }
