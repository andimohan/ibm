function ShippingAddress(opt) {
          ...
          $('.form-multiaddress').each(function () {
                ...
                .on('success.form.bv', function (e) {
                  // Prevent form submission
                    ...
                    if ($form.find('[name="hidLatLngAdd[]"]').val() === "") {
                        alert(lang.pinLocation);
    ...
    $('.btn-edit').on('click', function (e) {
      ...
      var latlngDiv = row.find('.shipping-latlng');
      var latlng = $.trim(latlngDiv.text()) || null;

      var mapId = row.find('[id^="edit-"]').attr('id');

      if (mapId) {
        initMap(mapId, latlng);
      }
    });
    ...
    function checkAddress() {
      var $parent =$('.content-profile');
      isAddressEmpty = $parent.find('.address-item').length === 0;
      if (isAddressEmpty) {
        $('.new-address-panel .label-show').show();
        return;
      }
      $('.new-address-panel .label-show').hide();
    }
    checkAddress();
};   
}