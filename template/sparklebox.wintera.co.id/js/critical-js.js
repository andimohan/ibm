...
function loadOverlayScreen(arrParam){      
    ...
    $("#popup-panel").css({ display: "flex", opacity: 0 }).animate({ opacity: 1 }, 400);       // fade in manually
    // agar flex nya bisa kepanggil
...
function quickSearch(obj){  
     ...
     //prioritasin select box
     //kalo gk ad, dari parameter form (utk yg modelnya klik menu kategori)
     //if(selQuickSearch.lenght > 0){  
     //    var quickSearchCategory = encodeURIComponent(selQuickSearch.val()) || 0; 
     //    parameter += 'cat='+quickSearchCategory;
     //}
    
     // dipindah ke parameterForm aj, karena ad yg  pake menu kategori, bukan selectbox
    
     var categorykey =  $('[name=hidSearchCategoryKey]').val() || 0;
     if(categorykey != 0)
         parameter += '&categorykey='+categorykey;
    
     $('[name=\'parameter-form\']').attr("action","/products/" +parameter+'&key='+encodeURIComponent(searchkey)).submit();
}
...
function showHidePassword(obj){
 ...
 function sendWAMsg(msg){
	...
	/* ---------- 1. Build the message ---------- */ 
	var url = window.location.href;             // or any custom URL
	var fullTxt = encodeURIComponent(msg + ' ' + url);

	/* ---------- 2. Produce the deep-link ---------- */
	// modern schema
	var waLink = 'https://wa.me/?text=' + fullTxt;
