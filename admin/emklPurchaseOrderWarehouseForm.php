...
$emklJobOrderHeaderWarehouse = createObjAndAddToCol(new EMKLJobOrderHeader(EMKL['jobType']['warehouse']));
...
$formAction = 'emklPurchaseOrderWarehouseList';
...
$rsJOType = $obj->getTableKeyAndObj($emklJobOrderWarehouse->tableName,array('key'));
$rsJOHeaderType = $obj->getTableKeyAndObj($emklJobOrderHeaderWarehouse->tableName,array('key'));
...
if (!empty($_GET['id'])){ 
    ...
    if($autoSellingReimburse)
        $rsJODetail = $emklJobOrderWarehouse->getDetailByColumn('refkey', $rs[0]['refkey']);
    ...
    if($rs[0]['reftabletype']==$rsJOType['key']){
        // JO
        $rsEmkl = $emklJobOrderWarehouse->searchData($emklJobOrderWarehouse->tableName.'.pkey',$rs[0]['refkey']);
        $rsVolumeDetail = $emklJobOrderWarehouse->getDetailVolume($rs[0]['refkey']); 
        ...
        $_POST['depot'] = $rsEmkl[0]['depotname']; 
         ...
         // JO HEADER 
         $rsEmkl = $emklJobOrderHeaderWarehouse->searchData($emklJobOrderHeaderWarehouse->tableName.'.pkey',$rs[0]['refjoheaderkey']); 
         $rsVolumeDetail =  $emklJobOrderHeaderWarehouse->getDetailWithRelatedInformation($rs[0]['refjoheaderkey']);
...
$arrJob = $class->convertForCombobox($emklJobOrderWarehouse->getJobType(),'pkey','name');  
$arrTransportType = $class->convertForCombobox($emklJobOrderWarehouse->getTransportationType(),'pkey','name');  
$arrContainer = $class->convertForCombobox($emklJobOrderWarehouse->getLoadContainer(),'pkey','name');  
...
//$arrVolume = $class->convertForCombobox($emklJobOrderWarehouse->getVolumeUnit(),'pkey','name');  
                                                         ...
                                                         <?php    
                                                            echo $obj->inputAutoComplete(array( 
                                                                                            ...
                                                                                            'source' =>array(
                                                                                                                ...
                                                                                                                'data' => array( 'action' =>'searchData', 'statuskey' => '(1)', 'jobtypekey' => EMKL['jobType']['warehouse'])
                                                                                                            ) , 
                                                                                            'allowedStatusForEdit' => array(1),
                                                                                            'callbackFunction' => 'getTabObj().onChangeJobHeader('.EMKL['jobType']['warehouse'].')'
                                                                                          )
                                                                                    );  
                                                        ?> 
                                                    </div> 
                                                      ...
                                                      <?php    
                                                            echo $obj->inputAutoComplete(array( 
                                                                                            ...
                                                                                            'source' =>array(
                                                                                                                ...
                                                                                                                'data' => array( 'action' =>'searchData', 'statuskey' => '(1,2,3)', 'jobtypekey' => EMKL['jobType']['warehouse'])
                                                                                                            ) , 
                                                                                            'allowedStatusForEdit' => array(1),
                                                                                            'callbackFunction' => 'getTabObj().onChangeJobOrder('.EMKL['jobType']['warehouse'].')'
                                                                                          )
                                                                                    );  
                                                        ?> 
                                                    </div>
