...
if (!empty($_GET['id'])){  
    ...
    for($i=0;$i<count($rsDetail);$i++){ 
        $rate = $obj->getCurrencyLastRate($rsDetail[$i]['pkey']);
                        ...
                        <div class="div-table-caption border-purple"><?php echo $obj->lang['currencyRate']; ?></div>
                             <div class="div-table  mnv-transaction transaction-detail" style="width:100%"> 
                                <div class="div-table-row"> 
                                    ...
                                    <div class="div-table-col detail-col-header" style="width:150px; text-align:right;border:0"><?php echo ucwords($obj->lang['lastRate']); ?></div>  
                                        ...
                                        for ($i=0;$i<=$totalRows; $i++){  
                                            ...
                                            if ($i == $totalRows ){
                                                ...
                                                if(!empty($rs[0]['pkey']) && $rs[0]['statuskey'] == 1){ 
                                                    $rate = $obj->getCurrencyLastRate($rsDetail[$i]['currencykey']); 
                                                    $rsDetail[$i]['ratebefore'] = $rate[0]['rate'];
                                                }
                                 ...
                                 <div class="div-table-row <?php echo $class; ?>"> 
                                    <div class="div-table-col detail-col-detail"><?php echo $obj->inputText('currencyName[]',array('overwritePost' => $overwrite, 'readonly' =>true,'etc' => $etc)); ?><?php echo $obj->inputHidden('hidCurrencyKey[]',array('overwritePost' => $overwrite, 'etc' => $etc)); ?><?php echo $obj->inputHidden('hidDetailKey[]',array('overwritePost' => $overwrite, 'etc' => $etc)); ?></div> 
