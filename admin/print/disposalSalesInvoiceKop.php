...
$generateReportContent = function ($dataset){  
    ...
    $rsItemDetail = $obj->getItemDetail(array_column($rsDetail,'pkey'));
    $arrWOKey = array_column($rsItemDetail,'wokey'); 
    $rsItemDetail = $obj->reindexDetailCollections($rsItemDetail,'refkey');
    
    $rsSPK = $disposalWorkOrder->searchDataRow(array($disposalWorkOrder->tableName.'.pkey',$disposalWorkOrder->tableName.'.workordercode'),
                                              ' and '. $disposalWorkOrder->tableName.'.pkey in ('.$disposalWorkOrder->oDbCon->paramString($arrWOKey,',').')'
                                );

   
    $npplCode = implode('<br>',array_column($rsSPK,'workordercode'));

    
    $rsJo = $disposalJobOrder->getDataRowById($rsDetail[0]['salesorderkey']);
    $rsDisposalContract = $disposalContract->getDataRowById($rsJo[0]['contractkey']);
    ...
    $logo = '<img src="'.$obj->templateImgPath.'kop.jpeg" style="height:120px">';                
    ...
    for($i=0;$i<$totalRs;$i++){ 
        ...
        $rsJo = $disposalJobOrder->getDataRowById($rsDetail[$i]['salesorderkey']);
        $maxWeight = $rsJo[0]['maximumweight'];
        $rsService = $service->getDataRowById($rsJo[0]['servicekey']);
    ...
    //        $rsInvoiceItemDetail = $obj->getItemDetail($rsDetail[$i]['pkey']);
            ...
            if ($rsDetail[$i]['serviceprice'] > 0){
                $no++;
                 $serviceName = "Jasa Layanan Pengambilan Limbah";
                 $amount = $rsDetail[$i]['serviceprice'];
                 $htmlDetail .= '<tr><td style="text-align:right;">'.($no).'.</td><td>'.$serviceName.'</td><td></td><td></td><td style ="text-align:right">'.$obj->formatNumber($amount).'</td></tr>';
            }

             if ($rsDetail[$i]['detailordergrandtotal'] > 0){ 
                
                 $arrItemDetails = $rsItemDetail[$rsDetail[$i]['pkey']];
                 
                //  $amount = $rsDetail[$i]['detailordergrandtotal'];

                 $disposalWeight = 0; 
                 foreach($arrItemDetails as $itemRow) {

                    if ($itemRow['exceedpricearea'] > 0) { 
                        $no++;
                        $serviceName = "Jasa Layanan Pengambilan Limbah";
                        $amount = $itemRow['exceedpricearea'];
                        $htmlDetail .= '<tr><td style="text-align:right;">'.($no).'.</td><td>'.$serviceName.'</td><td></td><td></td><td style ="text-align:right">'.$obj->formatNumber($amount).'</td></tr>';
                    }
                    $serviceName = "Jasa Pemusnahan Limbah - ". $itemRow['wastecode'];
                     $no++;
                     $exceedPrice = $itemRow['exceedweightpricearea'];
                    //  $disposalWeight +=  $itemRow['disposalweight'] - $maxWeight;
                     $amount =  $itemRow['total'];
                     // khusus bhuman dibulatkan keatas
                      $htmlDetail .= '<tr><td style="text-align:right;">'.($no).'.</td><td>'.$serviceName.'</td><td style="text-align:right">'.$obj->formatNumber($itemRow['chargedweight'], 2).' Kg</td><td  style="text-align:right">'.$obj->formatNumber($exceedPrice).'</td><td style ="text-align:right">'.$obj->formatNumber($amount).'</td></tr>';
                 }
			...
			$no++;
			$htmlDetail .= '<tr><td style="text-align:right;">'.($no).'.</td><td>'.$rsDetail[$i]['itemname'].'</td><td style="text-align:right"></td><td  style="text-align:right"></td><td style ="text-align:right">'.$obj->formatNumber($rsDetail[$i]['amount']).'</td></tr>';
		} 
...
<table cellpadding="2" > 
<tr><td><div class="title">'.strtoupper('INVOICE').''.$proforma.'</div></td></tr>
</table>  
<div style="clear:both"></div>
<table >
 <tr>
    ...
    <td style="width: 170px">
        <table cellpadding="2"> 
            ...
            <tr><td class="header-row-header" style="width: 80px">No. NPPL</td><td style="text-align:center">:</td><td>'.$npplCode.'</td></tr>
        </table>
    </td>
 </tr>
</table>
...
$html .= '<style>
	...
	<table cellpadding="4" style="width: 100%;">
		<tr style="height: 100%">
			<td style="text-align: center; border:1px solid #333;"><b><i>Contract No</i></b><br>'.$rsDisposalContract[0]['code'].'</td>
			<td style="text-align: center; border:1px solid #333;"><b><i>Contract Start</i></b><br>'.$obj->formatDBDate($rsDisposalContract[0]['startingdate']).'</td>
			<td style="text-align: center; border:1px solid #333;"><b><i>Contract End</i></b><br>'.$obj->formatDBDate($rsDisposalContract[0]['validdate']).'</td>
			</tr>
		<tr>
			<td style="text-align: center; border:1px solid #333;"><b><i>Package</i></b><br>'.$rsService[0]['name'].'</td>
			<td style="text-align: center; border:1px solid #333;"><b><i>Load Min. Kg/bulan</i></b><br>0</td>
			<td style="text-align: center; border:1px solid #333;"><b><i>Load Max. Kg/bulan</i></b><br>'.$obj->formatNumber($rsJo[0]['maximumweight']).'</td>
			</tr>
	</table>
	<br>
	<br>';
