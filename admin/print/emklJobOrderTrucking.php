...
$obj = new EMKLJobOrder(EMKL['jobType']['trucking']);  
$emklPurchaseOrderTrucking = new EMKLPurchaseOrder(EMKL['jobType']['trucking']);  
...
$emklPurchaseOrderTruckingAccess = $security->hasSecurityAccess( $obj->userkey ,$security->getSecurityKey($emklPurchaseOrderTrucking->securityObject),10);
...
<table cellpadding="2" > 
<tr><td><div class="title">'.$obj->lang['jobOrderSummary'].' Trucking</div></td></tr>
...
$html .= '<div></div>';
  ...
  // BUYING
    ...
    $rsBuying = array();
    if($emklPurchaseOrderTruckingAccess){
          //$rsBuying = $emklPurchaseOrderTrucking->searchData($emklPurchaseOrderTrucking->tableName.'.refkey', $rs[0]['pkey'], true, ' and '.$emklPurchaseOrderTrucking->tableName.'.statuskey in (1,2,3)');
        $rsBuying = $emklPurchaseOrderTrucking->searchData('','', true, ' and '.$emklPurchaseOrderTrucking->tableName.'.refkey in ('.$obj->oDbCon->paramString($jobOrderKeys,',').') and '.$emklPurchaseOrderTrucking->tableName.'.statuskey in (1,2,3)');

        foreach($rsBuying as $buyingRow){ 
            $rsDetailBuying = $emklPurchaseOrderTrucking->getDetailWithRelatedInformation($buyingRow['pkey']);
