...
$obj = new EMKLJobOrder(EMKL['jobType']['warehouse']);  
$emklPurchaseOrderWarehouse = new EMKLPurchaseOrder(EMKL['jobType']['warehouse']);  
...
$emklPurchaseOrderWarehouseAccess = $security->hasSecurityAccess( $obj->userkey ,$security->getSecurityKey($emklPurchaseOrderWarehouse->securityObject),10);
...
<table cellpadding="2" > 
<tr><td><div class="title">'.$obj->lang['jobOrderSummary'].' Warehouse</div></td></tr>
...
$html .= '<div></div>';
  ...
  // BUYING
    ...
    $rsBuying = array();
    if($emklPurchaseOrderWarehouseAccess){
          //$rsBuying = $emklPurchaseOrderWarehouse->searchData($emklPurchaseOrderWarehouse->tableName.'.refkey', $rs[0]['pkey'], true, ' and '.$emklPurchaseOrderWarehouse->tableName.'.statuskey in (1,2,3)');
        $rsBuying = $emklPurchaseOrderWarehouse->searchData('','', true, ' and '.$emklPurchaseOrderWarehouse->tableName.'.refkey in ('.$obj->oDbCon->paramString($jobOrderKeys,',').') and '.$emklPurchaseOrderWarehouse->tableName.'.statuskey in (1,2,3)');

        foreach($rsBuying as $buyingRow){ 
            $rsDetailBuying = $emklPurchaseOrderWarehouse->getDetailWithRelatedInformation($buyingRow['pkey']);
