...
$html .= ' 
<table cellpadding="2" > 
<tr><td><div class="title">'.strtoupper($obj->lang['employeeAPPayment']).'</div></td></tr>
...
for ($i=0;$i<count($rsDetail);$i++){   
    $rsAp = $ap->searchData($ap->tableName.'.pkey',$rsDetail[$i]['apkey']);
    $refCode = array();
    if(!empty($rsAp[0]['refcode'])) array_push($refCode, $rsAp[0]['refcode']) ;
    if(!empty($rsAp[0]['refcode2'])) array_push($refCode, $rsAp[0]['refcode2']) ;
 //<td>'.$rsAp[0]['reftranscode2'].'</td><td>'.$rsAp[0]['customername'].'</td>
  $html .= '<tr><td>'.$rsAp[0]['code'].'</td><td style="text-align:center">'. $obj->formatDBDate($rsAp[0]['trdate'],'d / m / Y',array('returnOnEmpty' => true, 'value' => '')) .'</td><td>'.implode(', ', $refCode).'</td><td style="text-align:right">'.$obj->formatNumber($rsDetail[$i]['amount']).'</td></tr>' ; 
}
$html .= '</table>' ;
...
$payment = '';
$rsAPPaymentMethodDetail = $obj->getPaymentMethodDetail($rs[0]['pkey']);    
...
$payment .= '<tr>';
$payment .= '<td style="width: 120px;">'.$rsAPPaymentMethodDetail[$j]['paymentmethodname'].'</td>';
$payment .= '<td style="text-align:center; width: 15px;">:</td>';
$payment .= '<td style="text-align:right; width: 80px;">'.$obj->formatNumber($rsAPPaymentMethodDetail[$j]['amount']).'</td>';
$payment .= '</tr>'; 
