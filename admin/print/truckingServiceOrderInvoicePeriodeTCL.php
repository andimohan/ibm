...
$generateReportContent = function ($dataset) {
    ...
    $html .= '<table cellpadding="2" width="100%">
            <thead>
                <tr>
                    ...
                    <th class="border-top-right-bottom align-center font-bold bg-blue" style="' . $widthTruckNo . $fontSize . '">License Plate Number</th>
                    <th class="border-top-right-bottom align-center font-bold bg-blue" style="' . $widthTruckType . $fontSize . '">Type</th>
                    <th class="border-top-right-bottom align-center font-bold bg-blue" style="' . $widthAddCost . $fontSize . '">Rit</th>
                    <th class="border-top-right-bottom align-center font-bold bg-blue" style="' . $widthPrice . $fontSize . '">Rate</th>
                    <th class="border-top-right-bottom align-center font-bold bg-blue" style="' . $widthTotalPrice . $fontSize . '">Total Price (Rp)</th>
                </tr>
            </thead>
            <tbody>
        ';
    ...
    for ($i = 0; $i < count($rsDetail); $i++) {
        ...
        foreach($rsJODetail as $detailRow) {
            ...
            $rsJODetailCol = $rsJODetailCols[$detailRow['refkey']]; // Perbaikan: gunakan refkey bukan salesorderkey
            $rsWOCol = $rsWOCols[$detailRow['refkey']];
            ...
            array_push($arrJobOrderData, array(
                ...
                'desc' => trim(isset($rsJODetailCols[$detailRow['refkey']][0]['trdesc']) ? $rsJODetailCols[$detailRow['refkey']][0]['trdesc'] : ''),
                'itemkey' => $detailRow['itemkey'],
                'itemname' => $detailRow['itemname'],
                'priceinunit' => $detailRow['priceinunit'],
                'total' => $detailRow['total'],
                'trip' => $detailRow['qtyinbaseunit'],
                'indexkey' =>  $rsWOCol[0]['policecode'] . '-'. $rsJODetailCols[$detailRow['refkey']][0]['trdesc']
            ));
        ...
        $arrJOSummary = array();
            ...
            foreach($arrJobOrderDataCol as $index => $row) {
                ...
                $period = $obj->formatDBDate($date, 'F Y');
                $desc = trim($row[0]['desc']);
                $truckType = $row[0]['itemname'];
                $price = $row[0]['priceinunit'];

                // $tripCount = count($row);
                $tripCount = array_sum(array_column($row, 'trip'));
                $total = array_sum(array_column($row, 'total'));

                array_push($arrJOSummary, 
                        array(
                            // 'period' => $period,
                            'period' => $obj->toLocalDate($period),
                            'route' => $desc,
                            'trucktype' => $truckType,
                            'desc' => $desc,
                            'triptotal' => $tripCount,
    ...
    foreach($arrJOSummary as $rowData) {
        $html .= '
                    <tr>
                        <td class="border-left-right-bottom align-center" style="' . $widthNo  . $fontSize. '">' . $no . '</td>
                        <td class="border-right-bottom align-center" style="' . $widthDate . $fontSize . '">' . $rowData['period'] . '</td>
                        <td class="border-right-bottom align-center" style="' . $widthKet . $fontSize . '">' . $rowData['desc'] . '</td>
                        ';
                        ...
                        $html .= '
                        <td class="border-right-bottom align-center" style="' . $widthTruckNo . $fontSize . '">' . $policeNumber . '</td>
                        ';
                        $html .= '
                        <td class="border-right-bottom align-center" style="' . $widthTruckType . $fontSize . '">' . $rowData['trucktype'] . '</td>
                        <td class="border-right-bottom align-right" style="' . $widthAddCost . $fontSize . '">' . $rowData['triptotal'] . '</td>
                        <td class="border-right-bottom align-right" style="' . $widthPrice . $fontSize . '">' . $obj->formatNumber($rowData['price']) . '</td>
                        <td class="border-right-bottom align-right" style="' . $widthTotalPrice . $fontSize . '">' . $obj->formatNumber($rowData['grandtotal']) . '</td>
                        </tr>
                        ';
                        $no++;
     }
    ...
    $grandTotal = $rs[0]['grandtotal'] - $tax23Value;
    ...
    if ($tax23Value > 0) {
        $html .= '<tr>
            <td class="border-left-right-bottom align-center font-bold" colspan="' . $colspan . '" style="'. $fontSize.'">PPh Final ('.$obj->formatNumber($tax23Percentage,1).'%)</td>
