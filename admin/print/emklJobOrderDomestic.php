...
$obj = new EMKLJobOrder(EMKL['jobType']['domestic']);  
$emklPurchaseOrderDomestic = new EMKLPurchaseOrder(EMKL['jobType']['domestic']);  
...
$emklPurchaseOrderDomesticAccess = $security->hasSecurityAccess( $obj->userkey ,$security->getSecurityKey($emklPurchaseOrderDomestic->securityObject),10);
...
<table cellpadding="2" > 
<tr><td><div class="title">'.$obj->lang['jobOrderSummary'].' Domestic</div></td></tr>
...
$html .= '<div></div>';
  ...
  // BUYING
    ...
    $rsBuying = array();
    if($emklPurchaseOrderDomesticAccess){
          //$rsBuying = $emklPurchaseOrderDomestic->searchData($emklPurchaseOrderDomestic->tableName.'.refkey', $rs[0]['pkey'], true, ' and '.$emklPurchaseOrderDomestic->tableName.'.statuskey in (1,2,3)');
        $rsBuying = $emklPurchaseOrderDomestic->searchData('','', true, ' and '.$emklPurchaseOrderDomestic->tableName.'.refkey in ('.$obj->oDbCon->paramString($jobOrderKeys,',').') and '.$emklPurchaseOrderDomestic->tableName.'.statuskey in (1,2,3)');

        foreach($rsBuying as $buyingRow){ 
            $rsDetailBuying = $emklPurchaseOrderDomestic->getDetailWithRelatedInformation($buyingRow['pkey']);
