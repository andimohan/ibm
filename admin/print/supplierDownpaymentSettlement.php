<?php 
includeClass('SupplierDownpaymentSettlement.class.php');
...
$obj = $supplierDownpaymentSettlement;
$generateReportContent = function ($dataset){ 

$obj = new SupplierDownpaymentSettlement();    
...
$html .= ' 
<table cellpadding="2" > 
<tr><td><div class="title">PENYELESAIAN UANG MUKA SUPPLIER</div></td></tr>
...
for ($i=0;$i<count($rsDetail);$i++){   
  $rsDPSupplier = $supplierDownpayment->getDataRowById($rsDetail[$i]['downpaymentkey']);

  $refCode = array();
  if(!empty($rsDPSupplier[0]['refcode'])) array_push($refCode, $rsDPSupplier[0]['refcode']) ;
  $html .= '<tr><td>'.$rsDPSupplier[0]['code'].'</td><td>'.implode(', ',$refCode).'</td><td style="text-align:center">'. $obj->formatDBDate($rsDPSupplier[0]['trdate'],'',array('returnOnEmpty' => true, 'value' => '')) .'</td><td style="text-align:right">'.$obj->formatNumber($rsDetail[$i]['amount']).'</td></tr>' ; 
}
$html .= '</table>' ;
