...
// ex. User boleh delete (yg otomatis ubah status ke pembatalan, tp user tidak memiliki hak akses utk pembatalan)
	...
	switch ($_POST['action']) {
		...
		case 'resendEmail': if(!$security->isAdminLogin($securityObject,11,false)) die;
  		...
  		case 'duplicate': if(!$security->isAdminLogin($securityObject,11,false)) die;
						...
						for ($i=0;$i<count($arrPkey);$i++){
                            ...
                            $obj->startNewErrorLogSession(); 
							$arrTemp = $obj->duplicateData($arrPkey[$i],true,array('reuseCode' => false));
  		...
  		case 'changestatus':  
                                ...
                                // sementara karena masi ad class yg overwrite change status yg blm diupdate
                                if(!$security->isAdminLogin($securityObject,$arr['newStatus'],false)){  
                                    array_push($arrReturn,array('valid'=>false, 'message' => $obj->errorMsg[252])); 
       ...
       case 'resyncgps' :  $arrPkey = $arr['selectedPkey'];  
                                    $obj->createFleetGPS($arrPkey);   
                                    break;
        ...
        case 'regenerateXML' :  $arrPkey = $arr['selectedPkey'];  
								$obj->printData($arrPkey[0]); // sementara hanya ambil index pertama
                                break;
