...
$formAction = 'apEmployeePaymentList';
...
$rsAPEmployeePaymentDetail = array();
...
if (!empty($_GET['id'])){ 
	...
	$rsAPEmployeePaymentDetail = $obj->getDetailWithRelatedInformation($id);
...
<script type="text/javascript">  
	...
	jQuery(document).ready(function(){  
        ...
        var tabID = <?php echo ($isQuickAdd) ?  $_GET['tabID'] :  'selectedTab.newPanel[0].id';  ?>  
         ...
         var apEmployeePayment = new APEmployeePayment(tabID, <?php echo json_encode($rs); ?>);
    
         prepareHandler(apEmployeePayment);
...
<div style="width:100%; margin:auto; " class="tab-panel-form">   
 ...
 <div class="div-table-col" style="padding:0">
				...
				<?php
                    ...
                    $totalRows = count($rsAPEmployeePaymentDetail);
                    for ($i=0;$i<=$totalRows; $i++){  
                        ...
                        if ($i == $totalRows ){
                            ...
                            $rsAP = $ap->getDataRowById($rsAPEmployeePaymentDetail[$i]['apkey']); 
                            $_POST['hidDetailKey[]'] =  $rsAPEmployeePaymentDetail[$i]['pkey'];
                            $_POST['hidAPKey[]'] =  $rsAPEmployeePaymentDetail[$i]['apkey']; 
                            $_POST['apCode[]'] =  $rsAPEmployeePaymentDetail[$i]['apcode'];
                            $_POST['refCode[]'] =  $rsAPEmployeePaymentDetail[$i]['refcode'];
                            $_POST['jobOrderCode[]'] =  $rsAPEmployeePaymentDetail[$i]['reftranscode2'];
                            $_POST['customerName[]'] =  $rsAPEmployeePaymentDetail[$i]['customername'];
                            $_POST['refDate[]'] = $obj->formatDBDate($rsAPEmployeePaymentDetail[$i]['refdate'],'d / m / Y');
                            $_POST['apAmount[]'] =  $obj->formatNumber($rsAPEmployeePaymentDetail[$i]['amount']);
                            $_POST['outstanding[]'] =  $obj->formatNumber($rsAPEmployeePaymentDetail[$i]['outstanding']); 
                            $_POST['amount[]'] =   $obj->formatNumber($rsAPEmployeePaymentDetail[$i]['amount']);  
                            $_POST['chkPick[]'] =  1;
                        }
                 ?>
             <div class="div-table-row <?php echo $class; ?>"> 
                        ...
                        <div class="div-table" style="width:100%">
                            <div class="div-table-row"> 
                                <div class="div-table-col detail-col-detail"><?php echo $obj->inputText('apCode[]',array('disabled' => $disabled,'overwritePost' => $overwrite )); ?><?php echo $obj->inputHidden('hidAPKey[]',array('overwritePost' => $overwrite, 'disabled' => $disabled)); ?><?php echo $obj->inputHidden('hidDetailKey[]',array('disabled' => $disabled,'overwritePost' => $overwrite)); ?></div> 
