...
includeClass(array('PurchaseOrder.class.php','PurchaseReceive.class.php','Supplier.class.php','TermOfPayment.class.php','PaymentMethod.class.php','ItemUnit.class.php'));
...
$formAction = 'purchaseReceiveList';
...
if (!empty($_GET['id'])){ 
    ...
    $rsPurchaseOder = $purchaseOrder->getDataRowById($rs[0]['refkey']);
    $_POST['hidPurchaseOrderKey'] = $rsPurchaseOder[0]['pkey']; 
    $rsSupplier = $supplier->getDataRowById($rsPurchaseOder[0]['supplierkey']);
    $_POST['purchaseOrder'] = $rsPurchaseOder[0]['code'] . ' - ' . $rsSupplier[0]['name'] ;
...
<script type="text/javascript">   
	...
	jQuery(document).ready(function(){   
        ...
        purchaseReceive = new PurchaseReceive(tabID,<?php echo json_encode($rs); ?>,cashTOP);
        prepareHandler(purchaseReceive); 
            ...
            <div class="div-table-row">
                <div class="div-table-col"> 
                    <div class="div-tab-panel"> 
                        ...
                        <div class="form-group">
                            <label class="col-xs-3 control-label"><?php echo ucwords($obj->lang['poCode']); ?></label> 
                            <div class="col-xs-9"> 
                                 <?php  echo $obj->inputAutoComplete(array( 
                                                                        'objRefer' => $purchaseOrder,
                                                                        'revalidateField' => true,
                                                                        'element' => array('value' => 'purchaseOrder',
				...
				<?php 
                    ...
                    for ($i=0;$i<=$totalRows; $i++){  
                        ...
                        if ($i == $totalRows ){
                            ...
                            $unitname = $rsPurchaseDetail[$i]['baseunitname'];
                            $_POST['hidDetailKey[]'] =  $rsPurchaseDetail[$i]['pkey'];
                            $_POST['hidPODetailKey[]'] = $rsPurchaseDetail[$i]['refpodetailkey']; 
                            $_POST['hidItemKey[]'] = $rsPurchaseDetail[$i]['itemkey']; 
                            $_POST['itemName[]'] = $rsPurchaseDetail[$i]['itemname']; 
                            $_POST['orderedQtyInBaseUnit[]'] = $obj->formatNumber($rsPurchaseDetail[$i]['orderedqtyinbaseunit']);
                            $_POST['qtyMinusInBaseUnit[]'] = $obj->formatNumber($rsPurchaseDetail[$i]['qtyminusinbaseunit']); 
                            $_POST['receivedQtyInBaseUnit[]'] = $obj->formatNumber($rsPurchaseDetail[$i]['receivedqtyinbaseunit']);  
                        } 
                        
                    ?>
            
                <div class="div-table-row  <?php echo $class; ?>">
                    <div class="div-table-col detail-col-detail"><?php echo $obj->inputText('itemName[]',array('overwritePost' => $overwrite,'readonly' => true,  'etc' => $etc)); ?><?php echo $obj->inputHidden('hidItemKey[]',array('overwritePost' => $overwrite, 'etc' => $etc)); ?><?php echo $obj->inputHidden('hidPODetailKey[]',array('overwritePost' => $overwrite, 'etc' => $etc)); ?><?php echo $obj->inputHidden('hidDetailKey[]',array('overwritePost' => $overwrite, 'etc' => $etc)); ?></div> 
                    ...
                    <div class="div-table-col detail-col-detail"><?php echo $obj->inputNumber('receivedQtyInBaseUnit[]', array('overwritePost' => $overwrite, 'etc' => 'style="text-align:right;" ' .$etc)); ?></div>  
