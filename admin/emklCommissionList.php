...
$addDataFile = 'emklCommissionForm';
...
array_push($arrSearchColumn, array('Job Order', $emklJobOrderImport->tableName. '.code')); 
...
function generateQuickView($obj,$id){ 
	...
	$basicInformation  = ' <div class="data-card border-orange">
						...
						<div class="div-table  general-information-table">
							...
							<div class="div-table-row">
								<div class="div-table-col">'.ucwords($obj->lang['jobOrderCode']).'</div> 
								<div class="div-table-col">'.$rs[0]['jocode'].'</div> 
							</div>
							<div class="div-table-row">
								<div class="div-table-col">'.ucwords($obj->lang['currency']).' / '.ucwords($obj->lang['rate']).'</div> 
								<div class="div-table-col">'.$rs[0]['currencyname'].' / '.$obj->formatNumber($rs[0]['rate']).'</div> 
							</div>
                            <div class="div-table-row">
								<div class="div-table-col">'.ucwords($obj->lang['total']).'</div> 
								<div class="div-table-col">'.$obj->formatNumber($rs[0]['grandtotal'],2).'</div> 
							</div>
							<div class="div-table-row">
		...
		for ($i=0;$i<count($rsDetail);$i++){
            $decimalPrice = ($rsDetail[$i]['currencykey'] == CURRENCY['idr'] ) ? 0 : 2;  
			$decimalPriceSubtotal = ($rs[0]['currencykey'] <> $rsDetail[$i]['currencykey'] || $rsDetail[$i]['currencykey'] <> CURRENCY['idr']) ? 2 : 0;
			$detailInformation  .= '
				<div class="div-table-row">  
