					...
					for ($k=0;$k<count($rsContent) ; $k++){ 
                        ...
                        if ($rsContent[$k]['code'] == '-'){
				 		...
				 		if ($rsContent[$k]['multivalue'] == 1){
									...
									$rsSettingDetail = $obj->getDetailById($rsContent[$k]['pkey'],'',' order by orderlist asc');
									for ($j=0;$j<count($rsSettingDetail); $j++){ 
										if ($j==0){   
											$_POST[$rsContent[$k]['code'].'Label[]'] =  $rsSettingDetail[$j]['label']; 
											$_POST[$rsContent[$k]['code'].'Desc[]'] =  $rsSettingDetail[$j]['desc']; 
											$_POST[$rsContent[$k]['code'].'[]'] =  $rsSettingDetail[$j]['value'];  
											$_POST[$rsContent[$k]['code'].'OrderList[]'] =  $rsSettingDetail[$j]['orderlist']; 
										}else{  
										    $arrPostValue = array();
											array_push($arrPostValue,array("selector" => $rsContent[$k]['code'].'Label', "value" =>  $rsSettingDetail[$j]['label'])) ;
											array_push($arrPostValue,array("selector" => $rsContent[$k]['code'].'Desc', "value" =>  $rsSettingDetail[$j]['desc'])) ;
											array_push($arrPostValue,array("selector" => $rsContent[$k]['code'], "value" =>    $rsSettingDetail[$j]['value'])) ; 
											array_push($arrPostValue,array("selector" => $rsContent[$k]['code'].'OrderList', "value" =>    $rsSettingDetail[$j]['orderlist'])) ; 
								...
								$contenttab .= ' <div class="form-group">
                                                    ...
                                                    <div class="col-xs-3">
                                                        '.$obj->inputText($rsContent[$k]['code'].'Desc[]').' 
                                                    </div>  
                                                    <div class="col-xs-1">
                                                        '.$obj->inputText($rsContent[$k]['code'].'OrderList[]', array('etc' => 'style="text-align:center"')).' 
                                                    </div>  
                                                    <div class="col-xs-5">
                                                        '.$inputType.' 
                                                    </div>  
								...
								$contenttab .= ' <div class="form-group '.$rsContent[$k]['code'].'-row-template" style="display:none;" > 
                                                    <div class="col-xs-3">
                                                       ...
                                                       <div class="col-xs-3">
                                                         '.$obj->inputText($rsContent[$k]['code'].'Desc[]', array('disabled' => true)).' 
                                                    </div>  
                                                    <div class="col-xs-1">
                                                         '.$obj->inputText($rsContent[$k]['code'].'OrderList[]', array('disabled' => true,'etc' => 'style="text-align:center"')).' 
                                                    </div>  
                                                    <div class="col-xs-4">
                                                         '.$inputType.' 
                                                    </div>  
