...
includeClass(array('APPayment.class.php','APPayableTax23Payment.class.php','Warehouse.class.php','Supplier.class.php'));
...
$arrDataStructure['taxPeriod'] = array('title' => 'Masa Pajak',  'width'=>"100px", 'dbfield' => 'taxperiod', 'align' => 'center');
...
$arrDataStructure['totalPayment'] = array('title'=>ucwords($obj->lang['paymentAmount']),'dbfield' => 'totalpaid', 'width'=>"100px" ,'format'=>'number','calculateTotal' => true);
...
if(isset($_POST) && !empty($_POST['selDateType'])){
   ...
   if(isset($_POST) && !empty($_POST['selSupplier'])) { 
    ...
    for( $i=0;$i<count($rs);$i++) {   
        ...
        if(empty($rs[$i]['supplierkey'])) {
            $arrAPKey = array_column($rsDetail,'apkey');
            $rsAPCol = $ap->searchDataRow(array($ap->tableName.'.pkey',
                                                $ap->tableName.'.code',
                                                $ap->tableName.'.trdate',
                                                $ap->tableName.'.supplierkey',
                                                $ap->tableName.'.refcode',
                                                $ap->tableName.'.amount'), 
                                            ' and ' .$ap->tableName.'.pkey in ('. $obj->oDbCon->paramString($arrAPKey,',').')'
                                        ); 
            ...
            $arrSupplierName = array_unique(array_column($rsSupplierCol, 'name'));
            $supplierNames = implode('<br>', $arrSupplierName);
            
            $rs[$i]['suppliername'] = $supplierNames;
        }
...
echo $twig->render('reportAPPayableTax23Payment.html', $arrTwigVar);   
?>
