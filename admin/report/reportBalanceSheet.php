...
if (isset($_POST) && !empty($_POST['hidAction'])){
	...
	switch($reportType){
		...
		case 0:
				$rsIncome = generateIncome(array('assets'), $_POST['trEndDate']); 
				$arrIncome[$_POST['trEndDate']] = $rsIncome;

				$rsExpense = generateIncome(array('liability','equity'),  $_POST['trEndDate'], -1 ); 
				$arrExpense[$_POST['trEndDate']] = $rsExpense;
		...
		case 1:
				...
				$arrIncome = getCOAAmount(array('assets'),$startPeriod,$endPeriod,$reportType);
				$arrExpense = getCOAAmount(array('liability','equity'),$startPeriod,$endPeriod,$reportType,-1);
		...
		case 4: 
				...
				foreach($arrYear as $yearRow){
					...
					$arrIncome= array_merge($arrIncome,getCOAAmount(array('assets'),$startPeriod,$endPeriod,$reportType));
					$arrExpense= array_merge($arrExpense, getCOAAmount(array('liability','equity'),$startPeriod,$endPeriod,$reportType,-1)); 
				}
...
echo $twig->render('reportBalanceSheet2.html', $arrTwigVar);
...
function getCOAAmount($arrCOAType, $startPeriod,$endPeriod,$reportType,$invert=1){
	global $class;
...
function generateRow($arrCOAValue, &$arrIncomeTotal, $reportType, $totalGroupYOY=0){
	...
	foreach($arrCOAValue as $coakey=>$row){
		...
		//init ulang
        $tempRow = array();
		// if ($hideEmptyAmount && $row['amount'] == 0)
		// 	continue;
...
function generateIncome($arrCOAType,$endDt,$invert=1){
