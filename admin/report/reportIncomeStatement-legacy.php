...
if (isset($_POST) && !empty($_POST['hidAction'])){
    ...
    $rsIncome = generateIncome(array('income'), $_POST['trStartDate'], $_POST['trEndDate'],  ucwords($class->lang['totalIncome']),$selectedWarehouse ); 
    $income = $rsIncome['amount'];
    $tempreport .= $rsIncome['report'];
    
    $tempreport .= '<tr class="row-plain"><td style="height:2em"></td><td></td></tr>';
    
    $rsExpense = generateIncome(array('expense'), $_POST['trStartDate'], $_POST['trEndDate'], ucwords($class->lang['totalExpense']),$selectedWarehouse); 
    ...
    $profitloss = $income + ($cost * $balanceInPositive);
    $tempreport .= generateTotalIncomeStatement($profitloss);
...
function generateIncome($arrCOAType,$startDt,$endDt,$totalText,$arrWarehouse){
    ...
    for ($i=0;$i<count($rs);$i++){ 
            ...
            $tabledetail .= '<td style="'.$firstRowStyle.' text-align:right">';
                // sementara, karena HPP harus positif di income statement, meskipun saldo normal kredit
                // gk boleh di edit di nilai amoutnnya, agar hasil pengurangannya tdk berubah
                $tabledetail .= $chartOfAccount->formatNumber( ($balanceInPositive == -1 && ($rs[$i]['pkey'] == 5 || $rs[$i]['rootkey'] == 5)) ? $rs[$i]['amount'] * -1 : $rs[$i]['amount']);
            $tabledetail .= '</td>';
            $tabledetail .= '</tr>';
    }
...
function exportToExcel($reportTitle,$arrTemplate, $arrContent){ 
     ...
     foreach($rsSide as $side){
        ...
        $profitLoss = $arrContent['profitloss'];
