...
$rsAPType = $obj->getAPType(); 
define('AP_IMPORT_TYPE',array_column($rsAPType,'name','pkey')); 
...
if($isGrouping){
        ...
        if(count($rsCurrency) == 1){
            $arrDataStructure['ammount'] = array('title'=>ucwords($obj->lang['total']),'dbfield' => 'totalamount', 'width'=>"150px" ,'format'=>'number','calculateTotal' => true, 'sortable' => false);       
            $arrDataStructure['outstanding'] = array('title'=>ucwords($obj->lang['outstanding']),'dbfield' => 'totaloutstanding', 'width'=>"150px" ,'format'=>'number','calculateTotal' => true, 'sortable' => false);       
        }else{ 
            foreach($rsCurrency as $currRow){
                $arrDataStructure['ammount'.$currRow['pkey']] = array('title'=>ucwords($obj->lang['amount']). ' ' .$currRow['name'],'dbfield' => 'totalamount'.$currRow['pkey'],"sortable" => false, 'width'=>"150px" ,'format'=>'number','calculateTotal' => true, 'sortable' => false);
                $arrDataStructure['outstanding'.$currRow['pkey']] = array('title'=>ucwords($obj->lang['outstanding']). ' '.$currRow['name'],'dbfield' => 'totaloutstanding'.$currRow['pkey'],"sortable" => false, 'width'=>"150px" ,'format'=>'number','calculateTotal' => true, 'sortable' => false);
            }
        }
     ...
     switch($EXPORT_TYPE){
        ...
        default :
            ...
            if ( in_array(PLAN_TYPE['categorykey'], array(COMPANY_TYPE['trucking'],COMPANY_TYPE['forwarding']))){
                ...
                $arrDataStructure['refinvoicecode'] = array('title'=>ucwords($obj->lang['invoiceReference']),  'width'=>"130px", 'dbfield' => 'refinvoicecode');
            ...
            if ( in_array(PLAN_TYPE['categorykey'], array(COMPANY_TYPE['trucking'],COMPANY_TYPE['forwarding']))){
                ...
                if(PLAN_TYPE['categorykey'] == COMPANY_TYPE['forwarding']){
                    $arrDataStructure['shipper'] = array('title'=>ucwords($obj->lang['importirExportir']),'dbfield' => 'shippername', 'width'=>"200px" );
                }
...
$arrHeaderTemplate['reportTitle'] = $obj->lang['APReport'];  
    ...
    default :
        ...
        if($isGrouping){
            ...
            if ( in_array(PLAN_TYPE['categorykey'], array(COMPANY_TYPE['trucking'],COMPANY_TYPE['forwarding']))){
                ...
                if(PLAN_TYPE['categorykey'] == COMPANY_TYPE['forwarding']){
                    $arrDataDetailStructure['shipper'] = array('title'=>ucwords($obj->lang['importirExportir']),'dbfield' => 'shippername', 'width'=>"200px" );
                }
                $arrDataDetailStructure['refinvoicecode'] = array('title'=>ucwords($obj->lang['invoiceReference']),  'width'=>"130px", 'dbfield' => 'refinvoicecode');
           }else{
...
if (isset($_POST) && !empty($_POST['hidAction'])){   
	...
	if(isset($_POST) && !empty($_POST['trStartDate'])){
		$criteria .= ' AND '.$ap->tableName.'.trdate between '.$class->oDbCon->paramDate( $_POST['trStartDate'],' / ').' AND '.$class->oDbCon->paramDate( $_POST['trEndDate'],' / ','Y-m-d 23:59'); 
    ...
    //$rs = (!$isGrouping) ? $obj->searchData('','',true,$criteria,$order) :  $obj->generateAPReport($criteria,$order);
    $rs = $obj->searchData('','',true,$criteria,$order) ;
    if($isGrouping) 
        $rs = $obj->groupAPBySupplier($rs,$rsCurrency); 
 
	if($isShowDetail && !$isGrouping)
           $rsPaymentDetail = $apPayment->getDetailPaymentCollections($rs, 'apkey');  
...
echo $twig->render('reportAP.html', $arrTwigVar);   
?>
