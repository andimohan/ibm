...
if($isGrouping){
    ...
    if(count($rsCurrency) == 1){
        ...
        $arrDataStructure['outstanding'] = array('title'=>ucwords($obj->lang['outstanding']),'dbfield' => 'totaloutstanding', 'width'=>"150px" ,'format'=>'number',"sortable" => false,'calculateTotal' => true,'mergeExcelCell' =>  ($isActiveModule['truckingserviceorder'] || $isActiveModule['emkljoborder']) ? 12 : 9 );       
    }else{ 
    ...
    switch($EXPORT_TYPE){
        ...
        default :
            ...
            $arrDataStructure['refcode'] = array('title'=>ucwords($obj->lang['invoiceCode']),  'width'=>"150px", 'dbfield' => 'refcode');
			 ...
			 if($isActiveModule['truckingserviceorder'] || $isActiveModule['emkljoborder'])
            ...
            if(count($rsCurrency) == 1){
                ...
                //    $arrDataStructure['tax23Value'] = array('title'=>ucwords($obj->lang['tax23']),'dbfield' => 'tax23value', 'width'=>"90px" ,'format'=>'number','calculateTotal' => true);
                //}
                ...
                foreach($rsCurrency as $currRow){
                    ...
                    //if ( in_array(PLAN_TYPE['categorykey'], array(COMPANY_TYPE['trucking'],COMPANY_TYPE['forwarding']))){
                    //    $arrDataStructure['tax23Value'.$currRow['pkey']] = array('title'=>ucwords($obj->lang['tax23']). ' '.$currRow['name'],'dbfield' => 'tax23value'.$currRow['pkey'], 'width'=>"90px" ,'format'=>'number','calculateTotal' => true);
                    //}
...
switch($EXPORT_TYPE){
    ...
    if($isGrouping){
        ...
        $calculateTotal = (count($rsCurrency) > 1) ? false : true;
        ...
        $arrDataDetailStructure['amount'] = array('title'=>ucwords($obj->lang['amount']),'dbfield' => 'amount', 'width'=>"110px" ,'format'=>'number','calculateTotal' => $calculateTotal);
        $arrDataDetailStructure['outstanding'] = array('title'=>ucwords($obj->lang['outstanding']),'dbfield' => 'outstanding', 'width'=>"110px" ,'format'=>'number','calculateTotal' => $calculateTotal);
        
        //if ( in_array(PLAN_TYPE['categorykey'], array(COMPANY_TYPE['trucking'],COMPANY_TYPE['forwarding']))){
        //    $arrDataDetailStructure['tax23Value'] = array('title'=>ucwords($obj->lang['tax23']),'dbfield' => 'tax23value', 'width'=>"90px" ,'format'=>'number');
        //}
...
//$arrTwigVar['COMPANY_TYPE'] = COMPANY_TYPE; // gk bisa pake isactivemodule, karena database lama active semau truckingordernya
...
if (isset($_POST) && !empty($_POST['hidAction'])){   
	...
	// harusnya utamain forwarding, nanti coba pake if else
    if ($isActiveModule['emkljoborder']){ 
        
		$emklType = $obj->getTableKeyAndObj($emklOrderInvoice->tableName ,array('key'))['key'];
        
		// no job
		$rsJobOrder = $emklOrderInvoice->getJODetail($arrInvoiceKey);
		$arrJobOrder[$emklType] = $obj->reindexDetailCollections($rsJobOrder,'refkey');
        
        // container detail
        $rsContainer = $emklJobOrder->getDetailContainer(array_column($rsJobOrder,'jokey'));
        $arrContainer = $obj->reindexDetailCollections($rsContainer,'refkey');
    }
	...
	if($isActiveModule['truckingserviceorder']){ 
		...
		// no job
        // yg dipake arrJobOrder, jd harusnya aman rsJobOrdernya gk ketimpa
    ...
    // nanti kalo 1 perusahaan ad dua jenis, TMS dan FMS, perlu dipisah salah satu yg utama, atau digabung JO nya
                        ...
                        if($isActiveModule['truckingserviceorder'] || $isActiveModule['emkljoborder']){ 
							...
							foreach($jobInformation as $jobRow){ 
								if(!in_array($jobRow['jocode'],$joCode)){ 
                                   ...
                                   array_push($arrJOKey,$jobRow['jokey']); 
                                }
                            }
                            ...
                            $containerNumber =array();
							 if ($isActiveModule['emkljoborder']){
                                    foreach($arrJOKey as $jokey){
                                        if (isset($arrContainer[$jokey]) && !empty($arrContainer[$jokey])){ 
                                            $arrCont = $arrContainer[$jokey]; 
                                            $containerNumber = array_merge($containerNumber, array_column($arrCont,'containerno') ); 
                                        } 
                                    } 
                                }

                            
                            // harsnya if else, tp bebrapa database customer dulu semua securityobject ke load
                            if (empty($containerNumber) && $isActiveModule['truckingserviceorder']){
