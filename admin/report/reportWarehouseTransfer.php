...
$securityObject = 'reportWarehouseTransfer'; // the value of security object is manually inserted to handle 
...
$arrHeaderTemplate['reportTitle'] = $obj->lang['warehouseTransferReport'];
...
if (isset($_POST) && !empty($_POST['hidAction'])){
    ...
    if(isset($_POST) && !empty($_POST['selWarehouse'])) { 
        ...
        $key = implode(",", $class->oDbCon->paramString($_POST['selWarehouse']));   
       	...
       	$criteria .= ' AND fromwarehousekey in('.$key.')';  
	    ...
	    array_push($arrFilterInformation,array("label" => 'Gudang Asal', 'filter' => $warehouseName ));
        
	}
    
    if(isset($_POST) && !empty($_POST['selWarehouseTo'])) { 
        
        $key = implode(",", $class->oDbCon->paramString($_POST['selWarehouseTo']));   
        
       	$criteria .= ' AND towarehousekey in('.$key.')';  
		...
		$warehouseName = implode(", ",$arrTempStatus); 
	    array_push($arrFilterInformation,array("label" => 'Gudang Tujuan', 'filter' => $warehouseName ));
    ...
    $rs = (!$isGrouping) ? $obj->generateWarehouseTransferReport($criteria.$detailCriteria,$order) :  $obj->searchData('','',true,$criteria,$order);
    $tempreport = '';  
...
echo $twig->render('reportWarehouseTransfer.html', $arrTwigVar);   
