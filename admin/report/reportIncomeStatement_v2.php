...
foreach($arrKeyPeriod as $keyPeriod => $period){ 
    $arrDataStructure['beginningBalance'.$keyPeriod] = array('title'=>ucwords($obj->lang['beginningBalance']),'group' => $period['label'], 'dbfield' => 'beginningbalance'.$keyPeriod, 'width'=>"90px" ,"format" => 'accounting',  "sortable" => false);
    $arrDataStructure['debit'.$keyPeriod] = array('title'=>ucwords($obj->lang['debit']),'group' => $period['label'],'dbfield' => 'debit'.$keyPeriod, 'width'=>"90px","format" => 'accounting', "sortable" => false);
    $arrDataStructure['credit'.$keyPeriod] = array('title'=>ucwords($obj->lang['credit']),'group' => $period['label'],'dbfield' => 'credit'.$keyPeriod, 'width'=>"90px","format" => 'accounting', "sortable" => false);
    $arrDataStructure['endingBalance'.$keyPeriod] = array('title'=>ucwords($obj->lang['endingBalance']),'group' => $period['label'],'dbfield' => 'endingbalance'.$keyPeriod, 'width'=>"90px","format" => 'accounting', "sortable" => false, "textColor" => '568203');
}
...
if (isset($_POST) && !empty($_POST['hidAction'])){ 
    ...
    $_POST['trStartDate'] = '01 / 01 / 2015';
        
    // ambil semua coa
   /* $rsCoaIncome = $chartOfAccount->searchDataRow(array($chartOfAccount->tableName.'.pkey'),
                                                  ' and '. $chartOfAccount->tableName.'.coatype = \'income\''
                                                );
    
    $rsCoaIncome = $chartOfAccount->searchData('','',true,' and '. $chartOfAccount->tableName.'.pkey in () or '. $chartOfAccount->tableName.'.rootkey in () ' );
    ...
    // akan error kalo bagian expense ad yg contra account
...
function generateIncome($arrCOAType,$startDt,$endDt,$totalText){
    ...
    for ($i=0;$i<count($rs);$i++){ 
            ...
            // biar kalo gk cukup panajng, nama COA nya tetep rapi
