...
$apEmployee = $obj->getAPObj();
$securityObject = 'reportAPEmployeePayment'; // the value of security object is manually inserted to handle 
...
$arrHeaderTemplate['reportTitle'] = $obj->lang['employeeAccountsPayablePaymentReport'];
...
if (isset($_POST) && !empty($_POST['hidAction'])){   
    ...
    for( $i=0;$i<count($rs);$i++) {   
        ...
        for ($j=0;$j<count($rsDetail);$j++){   
            $rsAP = $apEmployee->getDataRowById($rsDetail[$j]['apkey']);
...
echo $twig->render('reportAPEmployeePayment.html', $arrTwigVar);   
?>
