...
$securityObject = 'ReportMedicalRequestClaim'; // the value of security object is manually inserted to handle 
...
$_POST['module'] = IMPORT_TEMPLATE['medicalRequestClaim'];
...
switch($EXPORT_TYPE){
    case 2 :
        ...
        $arrDataStructure['callerName'] = array('title' => ucwords($obj->lang['callerName']), 'dbfield' => 'callername', 'width' => "120px");
        ...
        $arrDataStructure['insuranceCompany'] = array('title' => ucwords($obj->lang['insuranceCompany']), 'dbfield' => 'insurancecompanyname', 'width' => "200px");
...
$arrHeaderTemplate['reportTitle'] = $obj->lang['newRequestReport'];
...
if (isset($_POST) && !empty($_POST['hidAction'])){  
	...
	if(isset($_POST) && !empty($_POST['callerName'])) {
		$criteria .= ' AND '.$obj->tableName.'.callerName LIKE  ('.$class->oDbCon->paramString('%'.$_POST['callerName'].'%').')'; 
		array_push($arrFilterInformation,array("label" => $obj->lang['callerName'], 'filter' =>  $_POST['callerName']));
	} 
	...
	if(isset($_POST) && !empty($_POST['insuredName'])) {
		$criteria .= ' AND '.$obj->tableName.'.insuredName LIKE  ('.$class->oDbCon->paramString('%'.$_POST['insuredName'].'%').')'; 
    ...
    if(isset($_POST) && !empty($_POST['selCategory'])) { 
        ...
        $key = implode(",", $class->oDbCon->paramString($_POST['selCategory']));
       	...
       	$criteria .= ' AND '.$customer->tableName.'.categorykey in('.$key.')';  

        $rsCriteria = $customerCategory->searchData('','',true, ' and '.$customerCategory->tableName.'.pkey in ('.$key.')');
        // $rsCriteria =  $obj->getCategoryById ($key);
	 
        $arrTempStatus = array();
		for ($k=0;$k<count($rsCriteria);$k++)
		 	array_push($arrTempStatus,$rsCriteria[$k]['name']);
			
		$arrCategory = implode(", ",$arrTempStatus); 
...
$arrTwigVar['inputCallerName'] =  $class->inputText('callerName');   
...
echo $twig->render('reportMedicalRequestClaim.html', $arrTwigVar);  
