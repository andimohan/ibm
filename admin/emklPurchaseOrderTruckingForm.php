...
$emklPurchaseOrderTrucking = createObjAndAddToCol(new EMKLPurchaseOrder(EMKL['jobType']['trucking']));
$emklJobOrderTrucking = createObjAndAddToCol(new EMKLJobOrder(EMKL['jobType']['trucking']));
$emklJobOrderHeaderTrucking = createObjAndAddToCol(new EMKLJobOrderHeader(EMKL['jobType']['trucking']));
...
$obj= $emklPurchaseOrderTrucking;
$securityObject = $obj->securityObject; // the value of security object is manually inserted to handle 
										// some modules that have different security object from that of their class

if(!$security->isAdminLogin($securityObject,10,true));
 
$formAction = 'emklPurchaseOrderTruckingList';

$isQuickAdd = ( isset($_GET) && !empty($_GET['quickadd'])) ? true : false;

$autoSellingReimburse = (in_array($obj->loadSetting('autoSellingReimburse'),array(1,2)))  ?  true : false;
$useJobOrderHeader = ($obj->loadSetting('useJobOrderHeader') == 2) ? false : true;  // tdk semua ad settingan nilai 1
...
$rsJOType = $obj->getTableKeyAndObj($emklJobOrderTrucking->tableName,array('key'));
$rsJOHeaderType = $obj->getTableKeyAndObj($emklJobOrderHeaderTrucking->tableName,array('key'));
...
if (!empty($_GET['id'])){ 
    ...
    if($autoSellingReimburse)
        $rsJODetail = $emklJobOrderTrucking->getDetailByColumn('refkey', $rs[0]['refkey']);
    ...
    if($rs[0]['reftabletype']==$rsJOType['key']){
        // JO
        $rsEmkl = $emklJobOrderTrucking->searchData($emklJobOrderTrucking->tableName.'.pkey',$rs[0]['refkey']);
        $rsVolumeDetail = $emklJobOrderTrucking->getDetailVolume($rs[0]['refkey']); 
        ...
        $_POST['depot'] = $rsEmkl[0]['depotname']; 
         ...
         // JO HEADER 
         $rsEmkl = $emklJobOrderHeaderTrucking->searchData($emklJobOrderHeaderTrucking->tableName.'.pkey',$rs[0]['refjoheaderkey']); 
         $rsVolumeDetail =  $emklJobOrderHeaderTrucking->getDetailWithRelatedInformation($rs[0]['refjoheaderkey']);
...
$arrJob = $class->convertForCombobox($emklJobOrderTrucking->getJobType(),'pkey','name');  
$arrTransportType = $class->convertForCombobox($emklJobOrderTrucking->getTransportationType(),'pkey','name');  
$arrContainer = $class->convertForCombobox($emklJobOrderTrucking->getLoadContainer(),'pkey','name');  
...
//$arrVolume = $class->convertForCombobox($emklJobOrderTrucking->getVolumeUnit(),'pkey','name');  

if($autoSellingReimburse)
    $arrJODetail = $class->convertForCombobox($rsJODetail,'pkey','code');
...
$arrType = array();
if ($useJobOrderHeader) $arrType[$rsJOHeaderType['key']] = 'Header';
...
$rondedType = $obj->loadSetting('invoiceTaxRoundType');
...
<script type="text/javascript">  
	...
	jQuery(document).ready(function(){  
         ...
         var varConstant = {  
                            ...
                            TAX_ROUND_TYPE : <?php echo json_encode($rondedType); ?>,
                                                         ...
                                                         <?php    
                                                            echo $obj->inputAutoComplete(array( 
                                                                                            ...
                                                                                            'source' =>array(
                                                                                                                ...
                                                                                                                'data' => array( 'action' =>'searchData', 'statuskey' => '(1)', 'jobtypekey' => EMKL['jobType']['trucking'])
                                                                                                            ) , 
                                                                                            'allowedStatusForEdit' => array(1),
                                                                                            'callbackFunction' => 'getTabObj().onChangeJobHeader('.EMKL['jobType']['trucking'].')'
                                                                                          )
                                                                                    );  
                                                        ?> 
                                                    </div> 
                                                      ...
                                                      <?php    
                                                            echo $obj->inputAutoComplete(array( 
                                                                                            ...
                                                                                            'source' =>array(
                                                                                                                ...
                                                                                                                'data' => array( 'action' =>'searchData', 'statuskey' => '(1,2,3)', 'jobtypekey' => EMKL['jobType']['trucking'])
                                                                                                            ) , 
                                                                                            'allowedStatusForEdit' => array(1),
                                                                                            'callbackFunction' => 'getTabObj().onChangeJobOrder('.EMKL['jobType']['trucking'].')'
                                                                                          )
                                                                                    );  
                                                        ?> 
                                                    </div>
                   ...
                   <div class="div-table-row">   
                        <div class="div-table-col detail-col-header" >
                                <div class="div-table" style="width:100%">
                                     <div class="div-table-row">   
                                        ...
                                        <div class="div-table-col " style="width:100px;"><?php echo ucwords($obj->lang['unit']); ?></div>
                    ...
                    <?php  
                        ...
                        for ($i=0;$i<=$totalRows; $i++){  
                            ...
                            $overwrite = true;
                            $disable = '';  
                            //$detailDecimalPrice = 2; //($rsBuyDetail[$i]['currencykey'] == CURRENCY['idr'] ) ? 0 : 2;
                            ...
                            if ($i == $totalRows ){
                                ...
                                $_POST['detailSelUnit[]'] = $rsBuyDetail[$i]['unitkey'];
                                ...
                                <div class="div-table" style="width:100%">
                                    <div class="div-table-row"> 
                                        ...
                                        <div class="div-table-col detail-col-detail"  style="width:80px;"><?php echo $obj->inputDecimal('qty[]', array('overwritePost' => $overwrite,'value' => 1, 'etc' => 'style="text-align:right;"', 'disabled' =>  $disable)); ?></div>
                                         <div class="div-table-col detail-col-detail" style="width:100px;"><?php echo $obj->inputSelect('detailSelUnit[]', $arrUnit, array('overwritePost' => $overwrite)); ?></div>
...
-->
                                ...
                                <div class="flex">
                                     <div class="consume" style="padding-left:0.5em"> <?php echo $obj->inputText('description[]',array('overwritePost' => $overwrite, 'add-class' => 'label-style','etc' =>'placeholder="'.$obj->lang['description'].'"', 'disabled' =>  $disable)); ?></div>
