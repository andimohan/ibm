...
$addDataFile = $obj->getPersonalizedFiles($addDataFile);
...
array_push($arrNavbarButton, '<li id="btn-refresh" class="btn-action will-spin "  title="'.$obj->lang['refresh'].'" style="font-size:20px"><i class="fas fa-sync"></i></li>');

if($FILE_NAME == 'salesOrderList' && $obj->isActiveModule('marketplace')) 
...
if($FILE_NAME == 'employeeCommissionList') 
    array_push($arrNavbarButton, '<li id="btn-generate-employee-commission" class="btn-action"  title="'.$obj->lang['generateEmployeeCommission'].'" style="font-size:20px"><i class="far fa-money-check-edit"></i></li>');

// sementara utk Prajamitra dulu
if (isset($obj->importUrl) && !empty($obj->importUrl)){  
    
    if($FILE_NAME == 'truckingServiceOrderList' && in_array(DOMAIN_NAME, array( 'praja.wintera.co.id',  'prajademo.wintera.co.id')) ){
        
        $sellingPriceAllowed = $security->isAdminLogin('SellingPrice', 10);
...
if($FILE_NAME == 'truckingServiceOrderList' && in_array(DOMAIN_NAME, array( 'praja.wintera.co.id',  'prajademo.wintera.co.id'))) {
    ...
    // sementara utk Prajamitra dulu
//    if (isset($obj->importUrl) && !empty($obj->importUrl) && $sellingPriceAllowed){  
//         $importBtn = ($hasAddAccess) ? '<li id="btn-import"  class="'.$addDataActionClass.'" title="'.$obj->lang['import'].'" ><a href="'.$obj->importUrl.'" target="_blank"><i class="fas fa-file-import"></i></a></li>' :  '<li id="btn-import"  class="'.$addDataActionClass.'" title="'.$obj->lang['import'].'" ><i class="fas fa-file-import"></i></li>'; 
//         array_push($arrNavbarButton, $importBtn);  
//    }  
    
    $importCost = 'import/jobOrderAndCost';
    $importSelling = 'import/workOrderSellingPrice';
    $importBtn = '<li id="btn-import"  class="'.$addDataActionClass.'" title="'.$obj->lang['import'].'" ><a href="'.$importCost.'" target="_blank"><i class="fas fa-file-import"></i></a></li>'; 
    array_push($arrNavbarButton, $importBtn);  
    
    if($sellingPriceAllowed){
        $importBtn = '<li id="btn-import"  class="'.$addDataActionClass.'" title="'.$obj->lang['importSellingPrice'].'" ><a href="'.$importSelling.'" target="_blank"><i class="fas fa-file-import"></i></a></li>'; 
        array_push($arrNavbarButton, $importBtn);   
    }
}
 	...
 	$(document).ready(function() {  
            ...
            <?php 
                if ($obj->isTransaction){  
                      ...
                      echo 'var defaultStatusFilter = ['.$defaultStatusFilter.'];';  
            ...
            // utk oceanus, mungkin bisa utk yg lain nanti
        
			selectedTabObj.find("#btn-generate-employee-commission").attr("id","btn-generate-employee-commission-"+selectedTabId);   
        	$("#btn-generate-employee-commission-" + selectedTab.newPanel[0].id).bind( "click",    
                                                                                function(event){
                                                                                    var arrParam=[];
                                                                                    arrParam['url'] = 'employeeCommissionParameterForm.php?reloadAfterUpdate='+selectedTabId; 
                                                                                    loadPopup(arrParam);
                                                                                }
                                                                   );
...
<div class="panel-data-list">    
 <div class="container">
    	<div class="top-anchor" style="clear:both; height:1px"></div>
	 
		<div class="nav-sticky" style="background-color:#fff; position: sticky; top:0px; z-index:1">
				...
				<div class="table-data-list">
					<div class="div-table-row">    
						...
						for($j=0;$j<count($arrColumn);$j++){   
							...
							echo '<div style="'.$textAlign.' '.$width.' " class="div-table-col-5 col-header sortable" relcol="'.$dbfield.'" relwidth="'.$widthVal.'" reltype="-1">'.$title.'<div class="order-type"></div></div>';
						}
						?>	  
