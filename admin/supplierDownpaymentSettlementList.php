...
$supplierDdownpaymentSettlement = createObjAndAddToCol( new SupplierDownpaymentSettlement()); 


$obj = $supplierDdownpaymentSettlement;
$securityObject = $obj->securityObject; // the value of security object is manually inserted to handle 
										// some modules that have different security object from that of their class
 
if(!$security->isAdminLogin($securityObject,10,true));
 
$addDataFile = 'supplierDownpaymentSettlementForm';
	...
	$basicInformation  = ' <div class="data-card border-orange">
        ...
        if($rs[0]['typekey'] == 1){
            ...
            for ($j=0;$j<count($rsDPSettlementPaymentMethodDetail);$j++){ 
                    ...
                    $paymentInformation .= '<div class="div-table-col" style="text-align:right">'.$obj->formatNumber($rsDPSettlementPaymentMethodDetail[$j]['amount']).'</div> '; 
                    ...
                    $paymentInformation .= '<div class="div-table-row">';
                    $paymentInformation .= '<div class="div-table-col" style="width: 45%;">'.$rsCOAHeader[0]['code'].' - '.$rsCOAHeader[0]['name'].'</div>';
                    $paymentInformation .= '<div class="div-table-col" style="text-align:right">'.$obj->formatNumber($rs[0]['totalcoa']).'</div> '; 
                    $paymentInformation .= '</div>'; 
        }
