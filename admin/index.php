...
if (isset ($_SESSION[$class->loginAdminSession])){
    // ntah kenapa beberapa subdomain selalu diarahin ke index dulu, akhirnya pd ke logout
    header('location: /admin/list');
    die;
    //$class->setLog($_SERVER['HTTP_REFERER'],true);
    //$class->setLog("in",true);
...
if ($companyName == 'PT. Thomas Trans') $companyName = 'Yellow-Egg.com';
...
if($companyName == 'Yellow-Egg.com')    $profileImg = 'yellow-egg.jpg';
    
$bgImage = $class->loadSetting('adminBackgroundImage');

$avatarImg = (!empty($profileImg)) ? 'background-image:url(\'../phpthumb/phpThumb.php?src='.PHPTHUMB_URL_PATH .'setting/companyLogo/'.$profileImg.'&far=C&f=png&hash='.getPHPThumbHash($profileImg).'\');' : '' ;
...
<div id="body-login"> 
    <div class="login-panel-background">
        ...
        <div class="avatar">
			<div class="img-panel"  style="<?php echo $avatarImg; ?>" ></div> 
		</div>  
		</div>
        <div style="text-align:center; line-height:2em; font-size: 1.2em"><?php echo $companyName; ?></div> 
