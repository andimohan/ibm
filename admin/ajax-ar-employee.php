    ...
    switch ( $_GET['action']){  
        case 'searchData' :    
            ...
            if (isset($_GET) && !empty($_GET['employeekey']))
                array_push ($arrCriteria, $obj->tableName.'.customerkey ='. $obj->oDbCon->paramString($_GET['employeekey']) );  
        ...
        case 'getTotalOutstanding':
            ...
            if (empty($_GET['employeekey'])) die; 
            $employeeKey = $_GET['employeekey']; 
            $rsAR = $obj->getTotalOutstanding($employeeKey); 
            echo json_encode($rsAR);
        ...
        case 'getAvailableAR' :
            ...
            if (empty($_GET['employeekey']))  die; 
            $employeeKey = $_GET['employeekey']; 
            $rs = $obj->getAvailableAR($employeeKey); 
            echo json_encode($rs);
