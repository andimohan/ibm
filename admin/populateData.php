...
function addDataListRow($rs,$arrColumn){
	...
	for($i=0;$i<count($rs);$i++){   
		if(isset($rs[$i]['name']) && !empty($rs[$i]['name']))
			$rs[$i]['name'] = preg_replace('/[^\x00-\x7F]/', '', $rs[$i]['name']);
...
for($k=0;$k<count($filterCriteria);$k++){ 
    ...
    $arrSelectedCriteriaKey = (isset($_POST['filterCriteria'][$k]['selectedCriteriaKey']) && !empty($_POST['filterCriteria'][$k]['selectedCriteriaKey'])) ? $_POST['filterCriteria'][$k]['selectedCriteriaKey'] : array();
	
	// cast php8
	$filterCriteria[$k]['selectedCriteriaKey'] = array_map('intval', $arrSelectedCriteriaKey);
