...
abstract class Shuttle_Dumper {
	...
	static function create($db_options) {
		...
		if (self::has_shell_access() 
            ...
            echo 'Export using shell access.<br>';
			$dumper = new Shuttle_Dumper_ShellCommand($db);
		} else {
            echo 'Export using native PHP.<br>';
...
class Shuttle_Dumper_Native extends Shuttle_Dumper {
	public function dump($export_file_location, $table_prefix='') {
        ...
        $this->folder_name = rtrim($this->folder_name,'/') .'-'.date('dmY_Hi').'/';
        if(!is_dir($this->folder_name)) 
            mkdir ($this->folder_name,  0755, true);
        
        $export_file_location =  $this->folder_name.$export_file_location;
