        ...
        <ul class="progress-list"> 
            <?php 
                ...
                for($col=5;$col<=$highestColumnIndex;$col++){
                        ...
                        $movementDate = $worksheet->getCellByColumnAndRow($col, 1)->getValue();  
                        ...
                        for ($row = 2; $row <= $highestRow; ++$row) { 
                            ...
                            $movementValue = trim($worksheet->getCellByColumnAndRow(4, $row)->getValue());
                            if (empty($movementValue)) $movementValue = 0;
                            $movementQty = trim($worksheet->getCellByColumnAndRow($col, $row)->getValue());
                            if (empty($movementQty)) $movementQty = 0;

                            if (empty($itemname) || empty($movementQty)) continue; 

                            $arrItemColl = (!empty($itemcode)) ? $arrItemCollByCode : $arrItemCollByName;
                            $itemColIndex = (!empty($itemcode)) ? $itemcode : $itemname;
                            $itemColIndex = strtolower($itemColIndex);
                            
                            if(empty($arrItemColl[$itemColIndex]['pkey'])){ 
                                $obj->setLog($itemColIndex . ' not found');
                                continue;
                            }
                            ...
                            // make sure negatif utk barang keluar
                            //if($movementType == MOVEMENT['out']){ $movementQty = abs($movementQty) * -1;}
                ...
                foreach( $indexKey as $index){
                    ...
                    //echo '<br>PROCESSING INDEX : ' . $index .'<br>';
                    ...
                    //$temp = array_column($result,'message');
                    //print_r($temp);
                    
                    if($result[0]['valid'] == true){  
                        $confirmResult = $obj->changeStatus($result[0]['data']['pkey'], 2);    
                        if ($confirmResult[0]['valid'] == false)
                            die($confirmResult[0]['message'].'<br>');
                    }else{ 
                        die($result[0]['message'].'<br>');
                    }
