...
class Page extends BaseClass{
   ...
   function __construct(){
		...
		$this->tableName = 'page';    
		$this->tableNameDetail = 'page_category_detail'; 
		$this->tableCategory = 'page_category'; 
		$this->securityObject = 'Page'; 
		$this->tableStatus = 'master_status'; 
	    $this->tableLangValue = 'page_lang';
   ...
   function getQuery(){
    ...
    function getCategoryTree($parentkey = 0, $arrFilter = array(), $pathSeparator = ' / ', $leafOnly = true, $level = 0){
        $level++;
        
		$sql = 'select * 
				from '.$this->tableName.' 
				where 
					'.$this->tableName . '.parentkey = ' . $this->oDbCon->paramString($parentkey) .' and statuskey = 1 and
					'.$this->tableName.'.pkey in ('.$this->oDbCon->paramString($arrFilter,',').')
		...
		for ($i=0;$i<count($rs);$i++){  
            ...
            $arr = $this->getCategoryTree($rs[$i]['pkey'], $arrFilter, $pathSeparator,$leafOnly,$level);
	...
	function compileChildArray($pagekey,$arrFilter = array(), &$arrResult = array()){
		...
		$rs = $this->searchDataRow(array( $this->tableName . '.pkey', $this->tableName . '.parentkey',$this->tableName . '.pagename', $this->tableName . '.title', $this->tableName . '.isleaf'),
                                        ' and '.$this->tableName . '.statuskey = 1 and '.$this->tableName . '.parentkey =  ' .$this->oDbCon->paramString($pagekey).'
										  and '.$this->tableName.'.pkey in ('.$this->oDbCon->paramString($arrFilter,',').')
										',
                                        $orderby);
		...
		for($i=0;$i<count($rs);$i++)   
            $this->compileChildArray($rs[$i]['pkey'],$arrFilter,$arrResult); 
	...
	function getCategoryDetail($categorykey){
		$sql = 'select refkey from '.$this->tableNameDetail.' where categorykey = '.$this->oDbCon->paramString($categorykey).'';
