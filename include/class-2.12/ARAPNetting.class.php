...
class ARAPNetting extends BaseClass{
  
   function __construct(){
		
		parent::__construct();
		
		$this->tableName = 'arap_netting_header';
		$this->tableNameDetail = 'arap_netting_ar_detail';
		$this->tableAPDetail = 'arap_netting_ap_detail';
		...
		$this->securityObject = 'ARAPNetting';
        ...
        $this->arrDataDetail['taxamount'] = array('arTax23',array('datatype' => 'number'));
        ...
        $this->arrAPDetail['taxamount'] = array('apTax23',array('datatype' => 'number'));
        ...
        $this->arrData['totaltaxar'] = array('totalTaxARAmount','number');
        $this->arrData['totaltaxap'] = array('totalTaxAPAmount','number');
        ...
        array_push($this->arrDataListAvailableColumn, array('code' => 'customer','title' => 'customer','dbfield' => 'customername', 'default'=>true, 'width' => 170));
        array_push($this->arrDataListAvailableColumn, array('code' => 'supplier','title' => 'supplier','dbfield' => 'suppliername', 'default'=>true, 'width' => 170)); 
        array_push($this->arrDataListAvailableColumn, array('code' => 'desc','title' => 'note','dbfield' => 'trnotes','default'=>true, 'width' => 180)); 
      
       
        $this->printMenu = array();
        array_push($this->printMenu,array('code' => 'printTransaction', 'name' => $this->lang['printTransaction'],  'icon' => 'print', 'url' => 'print/arapNetting'));
	   ...
	   $this->includeClassDependencies(array(
        ...
        $this->arap = array();
        $this->arapConstant['ar'] = array(  'obj' => new AR(),
                                            'hidrefkey' => 'hidARKey',
                                            'refkey' => 'arkey',
                                            'hidrecipientkey' => 'hidCustomerKey',
        ...
        $this->arapConstant['ap'] = array( 
                                        ...
                                        'hidrefkey' => 'hidAPKey',
                                        'refkey' => 'apkey',
                                        'hidrecipientkey' => 'hidSupplierKey',
	...
	function reCountGrandtotal($arrParam){
				...
				$arrTaxAR = $arrParam['arTax23'];
				$arrTaxAP = $arrParam['apTax23']; 
				...
				for ($i=0;$i<count($arrARkey);$i++){
				    ...
				    if (!empty($arrTaxAR[$i]) && $arrTaxAR[$i]>0)  
						$taxar += $this->unFormatNumber($arrTaxAR[$i]);    
                ...
                for ($i=0;$i<count($arrAPkey);$i++){
				    ...
				    if (!empty($arrTaxAP[$i]) && $arrTaxAP[$i]>0)  
						$taxap += $this->unFormatNumber($arrTaxAP[$i]);    
                     
				}  
        
                // total yg dilunasin
                $grandtotalar = $totalar - $taxar;
                $grandtotalap = $totalap - $taxap;
			 
				$reCountResult = array();
				$reCountResult['totalARAmount'] = $totalar;
				$reCountResult['totalAPAmount'] = $totalap;
				$reCountResult['totalTaxARAmount'] = $taxar;
				$reCountResult['totalTaxAPAmount'] = $taxap;
	...
	function validateForm($arr,$pkey = ''){
		...
		$totalTaxAR = $this->unFormatNumber($arr['totalTaxARAmount']);
		$totalTaxAP = $this->unFormatNumber($arr['totalTaxAPAmount']);
		$grandTotalAR = $this->unFormatNumber($arr['grandtotalARAmount']);
		$grandTotalAP = $this->unFormatNumber($arr['grandtotalAPAmount']);
		//$arrPick = $arr['chkPick'];  
         
		
		$arrARkey = array_values(array_filter($arrARkey)); 
		$arrAPkey = array_values(array_filter($arrAPkey));
        ...
        $arrAP = array();
        $rsAP =  (!empty($arrAPkey)) ? $ap->searchData('','',true, ' and '.$ap->tableName.'.pkey in ('.implode(',',$this->oDbCon->paramString($arrAPkey)).') ') : array(); 
	...
	// gk perlu lg karena sudah difilter diatas 
//        $hasAR = false; 
        for($i=0;$i<count($arrARkey);$i++) { 
...
//            if (!empty($arrARkey[$i])) $hasAR = true;  
            ...
            if($this->unFormatNumber($arrARAmount[$i]) >  $this->unFormatNumber($arrAROutstanding[$i]))
...
//			if (!empty($arrAPkey[$i]))  $hasAP = true;  
            ...
            if($this->unFormatNumber($arrAPAmount[$i])>$this->unFormatNumber($arrAPOutstanding[$i]))
...
//            $this->addErrorList($arrayToJs,false, $this->errorMsg['ap'][1]); 	
	...
	function validateConfirm($rsHeader){
        ...
        if (USE_GL){ 
            ...
            $arrCOA = array();
            array_push($arrCOA, 'ar','ap', 'otherrevenue','othercost', 'prepaidtax23', 'payabletax23'); 
        ...
        $taxAR = $rsHeader[0]['totaltaxar'];
        $taxAP = $rsHeader[0]['totaltaxap'];
        $grandtotalAP = $rsHeader[0]['grandtotalap'];
        $grandtotalAR = $rsHeader[0]['grandtotalar'];
        if(($taxAR > 0 || $taxAP > 0) && $grandtotalAR <> $grandtotalAP)
            $this->addErrorLog(false,'<strong>'.$rsHeader[0]['code'].'</strong>. '.$this->errorMsg[201].'<br> '.$this->errorMsg['arap'][1]); 
	...
	function confirmTrans($rsHeader){  
        if($rsHeader[0]['totaltaxar'] > 0 || $rsHeader[0]['totaltaxap'] > 0 ) 
            $this->updatePaymentWithTax($rsHeader); 
        else
    ...
    function updateARAP($rsHeader){
        ...
        for($i=0;$i<count($arapDetail);$i++){
            ...
            $arrDetail[$i]['rate'] = $arapDetail[$i]['rate'];  
    ...
    function updatePaymentWithTax($rsHeader){
     ...
     function addPayment($rsHeader,$arapConstant){
        ...
        // default sementara pph 23 dulu
        $tax = new Tax();
        $rsTax = $tax->searchDataRow(array($tax->tableName.'.pkey'), ' and '.$tax->tableName.'.namecode = ' . $this->oDbCon->paramString(WITHHOLDING_TAX['art23']));
        $PPHTypeKey = (!empty($rsTax)) ? $rsTax[0]['pkey'] : 0;
        ...
        for($i=0;$i<count($rsDetail);$i++){   
            ...
            $arrParam['taxPPH'][$i] = $rsDetail[$i]['taxamount']; 
            $arrParam['selPPhType'][$i] = $PPHTypeKey; 


            $totalPayment += $arrParam['amount'][$i];
            $amount = $rsDetail[$i]['amount'] * $rsDetail[$i]['rate']; 
            $totalAmountIDR += $amount;
            $totalAmount += $rsDetail[$i]['amount'];
        }
        $rate = $totalAmountIDR / $totalAmount ;
        ...
        $arrParam['currencyPayment'] = $rsHeader[0]['currencykey'];
        $arrParam['hidSaveAndProceed'] = 1;
        $arrParam['total'] = $totalPayment; // harus ada, utk hitung ulang amountIn 
...
*/
	...
	function updateGL($rs){
        ...
        if((!empty($rsAPPayment))){
            ...
            $apCOAKey =  $supplier->getAPCOAKey($rsAPPayment[0]['supplierkey'],$warehousekey);
        }
        ...
        if((!empty($rsARPayment))){
            ...
            $arCOAKey =  $customer->getARCOAKey($rsARPayment[0]['customerkey'],$warehousekey);
        }
		...
		$arr['createdBy'] = 0; 
        $arr['trDesc'] = $this->lang['arapNetting'];
		$arr['selWarehouseKey'] = $rs[0]['warehousekey'];
        $dataARAP = $this->countTotalIDR($rs[0]['pkey']);
        
		$temp = -1; 
        
        // kalo ad PPH 23
        if($rs[0]['totaltaxar'] <> 0 || $rs[0]['totaltaxap'] <> 0){
            
            // cari tipe php23, kalo gk ad, pake 0
            // sementara semua neeting pasti php23 dulu
            $tax = new Tax();
            $rsTax = $tax->searchDataRow(array($tax->tableName.'.pkey'), ' and '.$tax->tableName.'.namecode = ' . $this->oDbCon->paramString(WITHHOLDING_TAX['art23']));
            $arrPPHTypeKey = (!empty($rsTax)) ? $rsTax[0]['pkey'] : 0;
                
            $temp++;
            $arr['hidCOAKey'][$temp] = $apCOAKey;
            $arr['debit'][$temp] = $dataARAP['totalapidr']; 
            $arr['credit'][$temp] = 0;  
            $arr['debitSource'][$temp] = $dataARAP['totalap'];  // pake rate rata2
            $arr['creditSource'][$temp] = 0;  
            $arr['selCurrencyKey'][$temp] = $headerCurrencyKey;  
            $arr['rate'][$temp] = $dataARAP['rateap'];
            ...
            $temp++;
            $arr['hidCOAKey'][$temp] =  $rsCOA[$arrPPHTypeKey]['coakey'];
            $arr['debit'][$temp] =  0;
            $arr['credit'][$temp] = $rs[0]['totaltaxap'] *  $dataARAP['rateap'];
            $arr['debitSource'][$temp] = 0 ; // pake rate rata2
            $arr['creditSource'][$temp] = $rs[0]['totaltaxap'] ;  
            $arr['selCurrencyKey'][$temp] = $headerCurrencyKey;  
            $arr['rate'][$temp] = $rate;  
            ...
            $arr['debit'][$temp] = 0; 
            $arr['credit'][$temp] = $dataARAP['totalaridr']; 
            $arr['debitSource'][$temp] = 0 ; // tdk masalah harusnya kalo rate semua sama, karnea gk ad selisih kurs  
            $arr['creditSource'][$temp] = $dataARAP['totalar']; 
            $arr['selCurrencyKey'][$temp] = $headerCurrencyKey;  
            $arr['rate'][$temp] = $dataARAP['ratear']; 
            ...
            $temp++;
            $arr['hidCOAKey'][$temp] = $rsCOA[$arrPPHTypeKey]['coakey'];
            $arr['debit'][$temp] =  $rs[0]['totaltaxar'] * $dataARAP['ratear']; 
            $arr['credit'][$temp] = 0;
            $arr['debitSource'][$temp] = $rs[0]['totaltaxar'] ; // tdk masalah harusnya kalo rate semua sama, karnea gk ad selisih kurs  
            $arr['creditSource'][$temp] = 0 ;  
            $arr['selCurrencyKey'][$temp] = $headerCurrencyKey;  
            $arr['rate'][$temp] = $dataARAP['ratear']; 
	...
	// $totalPayment = min(array($rs[0]['totalar'], $rs[0]['totalap']));
            $balance = $dataARAP['totalar'] - $dataARAP['totalap'];
            
            // cek lg
            if($balance > 0){
                
                $debit = $dataARAP['totalapidr'];
                $debitSource = $dataARAP['totalap'];
                $debitRate = $dataARAP['rateap'];
                
                $credit = $dataARAP['totalap'] * $dataARAP['ratear'];
                $creditSource = $dataARAP['totalap'];
                $creditRate = $dataARAP['ratear'];
                 
            }else{
                
                $debit = $dataARAP['totalaridr'];
                $debitSource = $dataARAP['totalar'];
                $debitRate = $dataARAP['ratear'];
                
                $credit = $dataARAP['totalar'] * $dataARAP['rateap'];
                $creditSource = $dataARAP['totalar'];
                $creditRate = $dataARAP['rateap'];
            }

        
            $temp++;
            $arr['hidCOAKey'][$temp] = $apCOAKey;
            $arr['debit'][$temp] = $debit; 
            $arr['credit'][$temp] = 0; 
            $arr['debitSource'][$temp] = $debitSource ; 
            $arr['creditSource'][$temp] = 0 ;  
            $arr['selCurrencyKey'][$temp] = $headerCurrencyKey;  
            $arr['rate'][$temp] = $debitRate;  

            $temp++;
            $arr['hidCOAKey'][$temp] = $arCOAKey;
            $arr['debit'][$temp] = 0; 
            $arr['credit'][$temp] = $credit;  
            $arr['debitSource'][$temp] = 0 ; 
            $arr['creditSource'][$temp] =  $creditSource;  
            $arr['selCurrencyKey'][$temp] = $headerCurrencyKey;  
            $arr['rate'][$temp] = $creditRate;  

            $balanceJournal = $debit - $credit; 
        }
     ...
     if ($balanceJournal <> 0) {
            if($balanceJournal > 0){
                ...
                $arr['debit'][$temp] = 0; 
                $arr['credit'][$temp] = $balanceJournal; 
                $arr['debitSource'][$temp] = 0 ; 
                $arr['creditSource'][$temp] = $balanceJournal;  
                $arr['selCurrencyKey'][$temp] = 1;  
                $arr['rate'][$temp] = 1;  
            }else{
                $rsCOA = $coaLink->getCOALink ('lossprofitrate', $warehouse->tableName,$warehousekey, 0); 
                $temp++;
                $arr['hidCOAKey'][$temp] = $rsCOA[0]['coakey'];
                // $arr['debit'][$temp] = $balanceJournal ; 
                // $arr['credit'][$temp] = 0; 
                // $arr['debitSource'][$temp] = $balanceJournal ; 
                // $arr['creditSource'][$temp] = 0 ;  
                $arr['debit'][$temp] = 0; 
                $arr['credit'][$temp] = $balanceJournal; 
                $arr['debitSource'][$temp] = 0 ; 
    ...
    function countTotalIDR($id){ 
        ...
        for($i=0;$i<count($rsDetailAR);$i++) { 
            $amount = $rsDetailAR[$i]['amount'] * $rsDetailAR[$i]['rate'];
            $totalARIDR += $amount;
            $totalAR += $rsDetailAR[$i]['amount'];
        }

        for($i=0;$i<count($rsDetailAP);$i++) { 
            $amount = $rsDetailAP[$i]['amount'] * $rsDetailAP[$i]['rate'];
            $totalAPIDR += $amount;
            $totalAP += $rsDetailAP[$i]['amount'];
        }

        $result = array();
        $result['totalap'] =  $totalAP;
        $result['totalapidr'] = $totalAPIDR;
        $result['totalar'] = $totalAR;
        $result['totalaridr'] = $totalARIDR;
        $result['rateap'] = $totalAPIDR / $totalAP;
        $result['ratear'] = $totalARIDR / $totalAR;
    ...
    function normalizeParameter($arrParam, $trim = false){
        ...
        $arrParam['totalTaxARAmount'] = $reCountResult['totalTaxARAmount'];
        $arrParam['totalTaxAPAmount'] = $reCountResult['totalTaxAPAmount'];
    ...
    function getDetailARAP($arrConstant, $pkey,$criteria=''){ 
      ...
      $sql = 'select
			  ...
			  where
			  	...
			  	'.$tableAPDetail.'.refkey = '.$this->oDbCon->paramString($pkey);
