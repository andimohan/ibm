...
//		$this->arrData['apicacoakey'] = array('hidAPICACOAKey');  
	...
	function resetReferralKeyIfQuotaExceed($pkey)
	{
		...
		$rsCustomer = $this->searchDataRow(
			array($this->tableName . '.referralkey'),
			' and ' . $this->tableName . '.pkey = ' . $this->oDbCon->paramString($pkey)
		);

		if (empty($rsCustomer) || $rsCustomer[0]['referralkey'] == 0)
			return false;
	...
	function afterUpdateData($arrParam, $action)
	{
		...
		if ($this->activeModule['customerinsurancepolicy'] && $arrParam['selCategory'] == 1) {
			...
			$rsCust = $customerInsurancePolicy->searchDataRow(
				array($customerInsurancePolicy->tableName . '.*'),
...
//    }
	...
	function sendActivationEmail($userkey)
	{
		...
		$lang = new Lang();
		$rsLang = $lang->searchDataRow(
			array($lang->tableName . '.code'),
		...
		if (!empty($this->loadSetting('WAGatewayAPIKey'))) {
			...
			if (!empty($rsCust[0]['mobilecode']))
				$rsCust[0]['mobile'] = $rsCust[0]['mobilecode'] . $rsCust[0]['mobile'];
	...
	function requestRecoverAccount($arr)
	{
		...
		$rsCust = $this->searchDataRow(
			array($this->tableName . '.pkey', $this->tableName . '.activationhashkey', $this->tableName . '.name', $this->tableName . '.langkey', $this->tableName . '.email'),
			' and ' . $this->tableName . '.email = ' . $this->oDbCon->paramString($arr['email']) . '
                             and ' . $this->tableName . '.statuskey = 2'
		); // cuma yg aktif yg boleh reset password
	...
	function resetPassword($userkey, $activationhashkey)
	{
		...
		$rsCust = $this->searchDataRow(
			array($this->tableName . '.pkey', $this->tableName . '.name', $this->tableName . '.email', $this->tableName . '.langkey', $this->tableName . '.mobilecode', $this->tableName . '.mobile'),
			' and ' . $this->tableName . '.pkey = ' . $this->oDbCon->paramString($userkey) . '
                             and ' . $this->tableName . '.activationhashkey = ' . $this->oDbCon->paramString($activationhashkey) . ' 
                             and ' . $this->tableName . '.statuskey = 2'
		);
...
//    }
	...
	function getARCOAKey($customerkey, $warehousekey, $isReimburse = false)
	{
		...
		if ($isReimburse) {
			...
			if (!empty($rsCustomer[0]['reimbursearcoakey']))
				return $rsCustomer[0]['reimbursearcoakey'];

			// kalo gk ad ambil dari reimburse warehouse 
			$rsCOA = $coaLink->getCOALink('arreimbursement', $warehouse->tableName, $warehousekey);
			if (!empty($rsCOA[0]['coakey']))
				return $rsCOA[0]['coakey'];
	...
	function normalizeParameter($arrParam, $trim = false)
	{
		...
		if ($this->activeModule['customerinsurancepolicy']) {
			...
			if (in_array($arrParam['selCategory'], array(3)))
				$arrParam['isInsured'] = 0;
			if ($arrParam['isInsured'] == 0)
				$arrParam['selSupplierKey'] = array();
...
//    }
	...
	function calculateActiveSubscriptionPeriod($pkey)
	{
		...
		$rs = $membershipSubscription->searchDataRow(
			array($this->tableMembershipSubscription . '.membershiplevelkey'),
		...
		return array(
			'activeperiodmonth' => $rs[0]['totalactiveperiod'],
	...
	function generateComboboxOpt($opt = array(), $queryOpt = array(), $preselected = '', $relOpt = array())
	{
		...
		if (isset($queryOpt['criteria']))
			$queryOpt['criteria'] .= $this->getCustomerCriteria();
	...
	function updateSettings($arr)
	{
		...
		if (isset($arr['selLang']))
			array_push($arrUpdate, $this->tableName . '.langkey = ' . $this->oDbCon->paramString($arr['selLang']));
		if (isset($arr['selTimeZone']))
			array_push($arrUpdate, $this->tableName . '.gmt = ' . $this->oDbCon->paramString($arr['selTimeZone']));


		if (isset($arr['selEmailPrivacyKey']))
			array_push($arrUpdate, $this->tableName . '.emailprivacykey = ' . $this->oDbCon->paramString($arr['selEmailPrivacyKey']));
		if (isset($arr['selMobilePrivacyKey']))
			array_push($arrUpdate, $this->tableName . '.mobileprivacykey = ' . $this->oDbCon->paramString($arr['selMobilePrivacyKey']));
		...
		try {
			...
			// kalo ad session
			if (isset($_SESSION[$this->loginSession]) && isset($arr['selTimeZone']))
				$_SESSION[$this->loginSession]['gmt'] = $arr['selTimeZone'];
...
//	}
	...
	function getGamificationStat($pkey)
	{
		...
		$rsCustomer = $customer->searchDataRow(
			array($customer->tableName . '.pkey', $customer->tableName . '.membershiplevel', $customer->tableName . '.hostlevelkey', $customer->tableName . '.totaljoin', $customer->tableName . '.totalhost'),
	...
	function autoUpdateData($arrKey, $arrParam)
	{
		...
		$rs = $this->searchDataRow(
			array($this->tableName . '.pkey', $this->tableName . '.code', $this->tableName . '.modifiedon'),
	...
	function getAddressForInvoice($pkey)
	{
		...
		$rs = $this->searchDataRow(
			array($this->tableName . '.address', $this->tableName . '.taxregistrationaddress'),
			' and ' . $this->tableName . '.pkey = ' . $this->oDbCon->paramString($pkey)
		);
