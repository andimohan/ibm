<?php  
class Plating extends BaseClass{
 
    function __construct(){
		
		parent::__construct();

        $this->tableName = 'plating';
		$this->tableStatus = 'master_status'; 

		$this->securityObject = 'Plating';
    ...
    function validateForm($arr,$pkey = ''){
		...
		$arrayToJs = parent::validateForm($arr,$pkey); 
	 	...
	 	$platingName = $arr['name'];

        $rs = $this->isValueExisted($pkey, 'name', $platingName);
        if (empty($platingName )) {
            $this->addErrorList($arrayToJs, false, $this->errorMsg['plating'][1]);
        } else if (count($rs) <> 0) {
            $this->addErrorList($arrayToJs, false, $this->errorMsg['plating'][2]);
        }
