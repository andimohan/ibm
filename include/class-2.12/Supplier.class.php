...
class Supplier extends BaseClass{ 
   ...
   function __construct(){
        ...
        $this->tableSupplierBankDetail = 'supplier_bank_detail';
        ...
        $this->arrSupplierBankDetail = array();
        $this->arrSupplierBankDetail['pkey'] = array('hidSupplierBankDetailKey');
        $this->arrSupplierBankDetail['refkey'] = array('pkey','ref');
        $this->arrSupplierBankDetail['bankname'] = array('bankName'); 
        $this->arrSupplierBankDetail['accountname'] = array('accountName'); 
        $this->arrSupplierBankDetail['accountNumber'] = array('accountNumber'); 
        $this->arrSupplierBankDetail['address'] = array('address'); 
        $this->arrSupplierBankDetail['branch'] = array('branch'); 
        $this->arrSupplierBankDetail['swift'] = array('swift'); 
        ...
        array_push($arrDetails, array('dataset' => $this->arrSupplierBankDetail, 'tableName' => $this->tableSupplierBankDetail));
      ...
      // $this->arrData['accountno'] = array('accountNo');
        ...
        $this->arrData['isinsurance'] = array('chkIsInsurance'); 
        ...
        array_push($this->arrLockedTable, array('table'=>'emkl_order_header','field'=> 'carrierkey'));   
        array_push($this->arrLockedTable, array('table'=>'emkl_job_order_header','field'=> 'carrierkey'));   
        ...
        array_push($this->arrDataListAvailableColumn, array('code' => 'alias','title' => 'alias','dbfield' => 'alias', 'width' => 150));
        ...
        array_push($this->arrSearchColumn, array('Alias', $this->tableName . '.alias'));
        array_push($this->arrSearchColumn, array('Kategori', $this->tableSupplierCategory . '.name'));
   ...
   function getQuery(){
	   ...
	   $sql = '
			select
				...
				from 
					'.$this->tableName . ' 		 
                        left join '. $this->tableSupplierCategory .' on '. $this->tableName .'.categorykey = '. $this->tableSupplierCategory .'.pkey
    ...
    function getAPCOAKey($supplierkey,$warehousekey,$apTypeKey = ''){ 
        $coaLink = new COALink();
        ...
        if (!empty($rsSupplier[0]['apcoakey'])){  
            ...
            $rsCOA = $coaLink->getCOALink ($coaName, $warehouse->tableName,  $warehousekey);   
    ...
    function getSupplierBankDetail($pkey)
    {
        $sql = 'select 
                '. $this->tableSupplierBankDetail .'.*
                from

                '. $this->tableSupplierBankDetail . '
                where
            ' . $this->tableSupplierBankDetail . '.refkey = ' . $this->oDbCon->paramString($pkey) . ' ';
