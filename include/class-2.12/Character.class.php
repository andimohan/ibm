...
class Character extends BaseClass
{
    ...
    function __construct()
    {
        ...
        $this->tableName = 'item_character';
        $this->tableStatus = 'master_status';

        $this->securityObject = 'Character';
    ...
    function validateForm($arr, $pkey = '')
    {
        ...
        if (empty($ringSizeName)) {
            $this->addErrorList($arrayToJs, false, $this->errorMsg['character'][1]);
        } else if (count($rs) <> 0) {
            $this->addErrorList($arrayToJs, false, $this->errorMsg['character'][2]);
        }
