...
class Category extends BaseClass{
   ...
   function getQuery(){
	  ...
	  //      $sql .=  $fieldname .' like '. $this->oDbCon->paramString('%'.$searchkey.'%');
			...
			if (is_array($fieldname)) {
				...
				foreach ($fieldname as $field) {
					$conditions[] = $field . " LIKE " . $this->oDbCon->paramString('%' . $searchkey . '%');
				}
				$sql .= ' AND (' . implode(' OR ', $conditions) . ')';
			} else {
				$sql .= ' AND ' . $fieldname . " LIKE " . $this->oDbCon->paramString('%' . $searchkey . '%');
			}
    ...
    function normalizeParameter($arrParam, $trim = false){  
        ...
        // jgn diaktifkan karena beberapa kategori sudha menggunakan model baru
...
//        $level = 0;
//        
//        $rsCat = $this->searchDataRow(array($this->tableName.'.pkey',$this->tableName.'.parentkey'),' and ' = $this->oDbCon->paramString($id));
//        if (empty($rsCat)) return 0;
//        
