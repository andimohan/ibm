...
class Excel extends BaseClass{
 ...
 function exportToSaveFromArray($arr, $companyInformation = array(),$fileDataName=''){
		...
		if (!is_dir($fullpath)) 
		 ...
		 // kalo sdh ad filenya langsung download saja
		...
		$reportTitle = str_replace('&','',$arr[0]['reportTitle']); 
		...
		$fileIndexName = (empty($fileDataName)) ? date('dmY_his') : $fileDataName;
	    $this->filenameRespon = $reportTitle.' - '.$fileIndexName.'.xlsx'; 
		
		$fullpath .= $this->filenameRespon;
 
		$arrDataToWrite = $arr[0]['dataToExport'];
		
		$runningRow = 1;
		
		// write header
		// sementara asumsi row data pertama paling lebar kolomnya
		$totalCol =  (isset($arrDataToWrite[0])) ? count($arrDataToWrite[0]) : 0;  
		$this->writeReportHeader($runningRow,$this->getColumnAlpha($totalCol), $companyName,$reportTitle, $arr[0]['filterInformation']);

		$runningRow++;
		
		$totalRows = count($arrDataToWrite);  
		
		for($i=0;$i<$totalRows;$i++){ 
				
			$totalCol =  (isset($arrDataToWrite[$i])) ? count($arrDataToWrite[$i]) : 0; 
			  
			for($j=0;$j<$totalCol;$j++){ 
				 
				$runningCol = $j+1;
				$columnAlpha = $this->getColumnAlpha($runningCol);  
				$format = $arrDataToWrite[$i][$j]['format'];
				$arrStyle = (isset($arrDataToWrite[$i][$j]['style'])) ? $arrDataToWrite[$i][$j]['style'] : array();
				
				$this->activeSheet->setCellValueByColumnAndRow($runningCol,$runningRow , strip_tags(str_replace('<br>',chr(13),$arrDataToWrite[$i][$j]['excelValue']))); 
				
				// style
				
				$cellStyle = $this->activeSheet->getStyle($columnAlpha.$runningRow);
                  
				$this->formatCell($cellStyle,$format);
						   
				if(!empty($arrStyle))
					$cellStyle->applyFromArray($arrStyle);
			}
  ...
  function exportToSave($arrTemplate, $arrExportParam, $companyInformation = array(), $fileDataName = ''){ 
      ...
      switch($exportType){
          ...
          default :
			  ...
			  $this->writeReportHeader($tableHeaderRow,$highestColumnAlpha, $companyName,$reportTitle, $arrTemplate[0]['filterInformation']);
      ...
      if (!empty($rsData)){
            ...
            foreach ($headerDataStructure as $el){    
                ...
                if ($calculateTotal){ 
                    ...
                    // index pertama 0, sepertinya utk level/index 0
                    $cellRangeSum = '=SUM('.implode(',',$this->subtotalColumn[$level][$col]).')';
                    //$this->setLog($cellRangeSum,true);
                    
                    // error skrg, gk bisa pake name ranged lg
                    // $cellRange = $this->subtotalAddNamedRange.$col;   
                    // $cell->setCellValueByColumnAndRow($colToWrite, $runningRows, '=SUM('.$cellRange.')');
                    
                    $cell->setCellValueByColumnAndRow($colToWrite, $runningRows, $cellRangeSum);
...
//
//                $namedRange = $this->subtotalAddNamedRange.$key; 
//                $cellRange = ($totalLevel > 1) ? implode(',',$col) : $col[0] .':'.$col[count($col)-1];  
//                 
//                $this->setLog($cellRange,true);
//                $this->spreadsheetRespon->addNamedRange( new \PhpOffice\PhpSpreadsheet\NamedRange($namedRange, $this->spreadsheetRespon->getActiveSheet(),'='.$cellRange) );
//            }
//        }
	...
	function writeReportHeader(&$reportHeaderRows, $highestColumnAlpha, $companyName,$reportTitle, $filterInformation){
