...
class TruckingCostCashOut extends BaseClass{ 
   
   function __construct(){
		
		parent::__construct();
       
		$this->tableName = 'trucking_cost_cash_out_header';
		$this->tableNameDetail = 'trucking_cost_cash_out_detail';
        ...
        $this->tableFile = 'trucking_cost_cash_out_file';
        $this->uploadFileFolder = 'trucking-cost-cash-out/';
        ...
        $this->arrData['trsubmissiondate'] = array('trSubmissionDate','date');
        ...
        $this->arrData['iscostcargo'] = array('isCostCargo');
        ...
        array_push($this->arrDataListAvailableColumn, array('code' => 'date','title' => 'JOWODate','dbfield' => 'trdate','default'=>true, 'width' => 140, 'align' =>'center', 'format' => 'datetime'));
        array_push($this->arrDataListAvailableColumn, array('code' => 'submissiondate','title' => 'submissionDate','default'=>true,'dbfield' => 'trsubmissiondate', 'width' => 140, 'align' =>'center', 'format' => 'datetime'));
        ...
        array_push($this->arrDataListAvailableColumn, array('code' => 'recipient','title' => 'recipient','dbfield' => 'employeename','default'=>true, 'width' => 200));
            array_push($this->arrDataListAvailableColumn, array('code' => 'containerNumber','title' => 'containerNumber','dbfield' => 'containernumber', 'width' => 200));
        ...
        $this->includeClassDependencies(array(
              ...
              'TruckingCostCashIn.class.php',
              'TruckingCashOutRequest.class.php'
        ));
       
        $this->printMenu = array();
        array_push($this->printMenu,array('code' => 'printTransaction', 'name' => $this->lang['printTransaction'],  'icon' => 'print', 'url' => 'print/truckingCostCashOut'));
        array_push($this->printMenu,array('code' => 'printTransactionAndAttachment', 'name' => $this->lang['printTransaction'] .' &amp; ' . $this->lang['attachment'],  'icon' => 'print', 'url' => 'print/truckingCostCashOut?attachment=1'));
   ...
   function getQuery(){
       ...
       $sql =  '
			SELECT '.$this->tableName.'.* , 
               ...
               CONCAT_WS(\', \', '.$this->tableSalesWorkOrder.'.containernumber, '.$this->tableSalesWorkOrder.'.container2number) as containernumber
			FROM 
                '.$this->tableStatus.',  
                '.$this->tableName.'    
...
//            $this->updateFile($arrParam['pkey'], $arrParam['token-item-file-uploader'], $arrParam['item-file-uploader']);
    ...
    function delete($id,$forceDelete = false, $reason = ''){
        ...
        if($this->isTransaction){ 
             $arrayToJs = $this->changeStatus($id, 5,$reason,false,$forceDelete);  
		...
		try{ 
	 		...
	 		$arrayToJs = $this->validateDelete($id,$forceDelete);
    ...
    function proceedUpdateStatus($status,$rsHeader,$copy){ 
        
      // untuk dioverwrite di subclass
	       switch ($status){ 
                    
				case 2 : $this->onRequestTrans($rsHeader);  
                         break; 
  ...
  function validateStatus($status,$rsHeader,$autoChangeStatus){ 
      ...
      // untuk dioverwrite di subclass
                switch ($status){
                   case 1 : $this->validateInput($rsHeader); 
						  break;
                    case 2 :  $this->validateOnRequest($rsHeader, $autoChangeStatus); 
                              break;
                    case 3 : if ($rsHeader[0]['statuskey'] < $status )
                              ...
                              break;
                    case 4 :  $this->validateClose($rsHeader, $autoChangeStatus); 
                              break; 
    ...
    function updateRequestkey($id,$requestkey){
        
        $truckingServiceWorkOrder = new TruckingServiceWorkOrder();
        $truckingServiceOrder = new TruckingServiceOrder();    
 
        $rsHeader = $this->searchDataRow(array($this->tableName.'.pkey',$this->tableName.'.reftabletype'),
        ...
        if ($tableName == $truckingServiceWorkOrder->tableName){
            $tableName = $truckingServiceWorkOrder->tableCost;
        }else if ($tableName == $truckingServiceOrder->tableName){ 
            $tableName = $truckingServiceOrder->tableHeaderCost;
        } else{
            return;
        }
 
        foreach($rsDetail as $row){
             $sql = 'update '.$tableName.' set refrequestkey = ' . $this->oDbCon->paramString($requestkey) .' 
                where pkey = ' .  $this->oDbCon->paramString(  $row['refheadercostkey']  );
     ...
     function validateOnRequest($rsHeader,$autoChangeStatus){    
         
        // hanya boleh diubah dr modul request 
         
        // nanti baru diupdate validasi nya
        ...
        $this->validateProcessConfirm($rsHeader,true);
     }   
...
/*		if (!empty($pkey)){
    ...
    function normalizeParameter($arrParam, $trim=false){ 
        ...
        if( !$this->isActiveModule('TruckingCashOutRequest'))
            $arrParam['trSubmissionDate'] = date('d / m / Y H:i');
        ...
        $arrTrdesc = array();
        $itemname = array();
        $spktablekey = $this->getTableKeyAndObj($truckingServiceWorkOrder->tableName,array('key'));
        $spktablekey =  $spktablekey['key'];
      
        if($arrParam['hidRefTable']==$spktablekey){
            $rsWO = $truckingServiceWorkOrder->searchData($truckingServiceWorkOrder->tableName.'.pkey',$arrParam['hidRefKey'],true);
        ...
        $rsJO = $truckingServiceOrder->searchData($truckingServiceOrder->tableName.'.pkey',$joHeaderKey,true);
        $rsJODetail = $truckingServiceOrder->getDetailWithRelatedInformation($joHeaderKey);
        ...
        for($i=0;$i<count($rsJODetail);$i++){ 
			$party =  $this->formatNumber($rsJODetail[$i]['qtyinbaseunit'],$partyDecimal); 
			array_push($itemname, $party.'x '.$rsJODetail[$i]['itemname']);
		}
    ...
    function getTransactionType($tableName){ 
     ...
     function getAttachmentToPrint($id){
        ...
        foreach($rsItemFile as $fileRow){ 
            $filePath = ($this->useStorage) ? $this->createPresignedURL(DOMAIN_NAME.'/'. $this->uploadFileFolder.$fileRow['refkey'].'/'.$fileRow['file']) : $this->defaultDocUploadPath. $this->uploadFileFolder.$fileRow['refkey'].'/'.$fileRow['file']; 
    ...
    function updateGL($rsHeader,$rsDetail,$arrCashBank){  
        ...
        $rsJO = $truckingServiceOrder->getDataRowById($rsHeader[0]['jokey']);
    ...
    function onRequestTrans($rsHeader){   
//        $truckingServiceWorkOrder = new TruckingServiceWorkOrder();
//        $truckingServiceOrder = new TruckingServiceOrder();    
//        
//        $id = $rsHeader[0]['pkey'];
...
//        } 
//        
        // dipindah ketika konfirmasi request
//        for($i=0;$i<count($rsDetail);$i++){ 
//            $sql = 'update '.$tableName.' set refrequestkey = ' . $rsDetail[$i]['refkey'] .' where pkey = ' . $rsDetail[$i]['refheadercostkey'];
//            $this->oDbCon->execute($sql); 
//        }
    ...
    function confirmTrans($rsHeader){   
        ...
        $updateEmployeeKey = '';
        if ($tableName == $truckingServiceWorkOrder->tableName){
            if($rsHeader[0]['iscostcargo'] == 0) {
                $tableName = $truckingServiceWorkOrder->tableCost;
                $updateEmployeeKey = ', employeekey = '.$rsHeader[0]['employeekey'].' '; 
            } else {
                $tableName = $truckingServiceWorkOrder->tableWorkOrderCostCargoDetail;
            }
        ...
        for($i=0;$i<count($rsDetail);$i++){
            ...
            if ( $rsDetail[$i]['costkey'] == DEFAULT_COST['outsourceDownpayment']) {
               $sql = 'update '.$truckingServiceWorkOrder->tableName.' set refcashoutdownpaymentkey = ' . $rsDetail[$i]['refkey'] .', downpaymentemployeekey = '.$rsHeader[0]['employeekey'].' where pkey = ' . $rsHeader[0]['refkey']; 
            }else{
    ...
    function validateConfirm($rsHeader){
        ...
        // gk boleh inherit, karena bisa ubah status dari "pengajuan"
//        parent::validateConfirm($rsHeader);
        
        if($rsHeader[0]['statuskey'] > 2) 
            $this->addErrorLog(false,'<strong>'.$rsHeader[0]['code'].'.</strong> '.$this->errorMsg[203],true);   
        
        $this->validateProcessConfirm($rsHeader);
        
	 }		
     
    function validateProcessConfirm($rsHeader, $onRequest = false){
        // di gabung agar yg dari on Request bisa pake jg
        ...
        $rsCashOutKey = $this->getTableKeyAndObj($truckingServiceWorkOrder->tableName,array('key'));  
          
        if ($rsHeader[0]['reftabletype'] == $rsCashOutKey['key']){    
            $rsSPK = $truckingServiceWorkOrder->getDataRowById($rsHeader[0]['refkey']);
            ...
            if ($rsSPK[0]['statuskey'] < 2 || $rsSPK[0]['statuskey'] > 3){ 
                ...
                // gk jadi, pindahin aj ke JO, sebelum closing, gk boleh ad kas pending
                ////KALAU SPK STATUS SUDAH SELESAI TIDAK BOLEH DI KONFIRMASI 
                //if ($rsSPK[0]['statuskey'] == TRANSACTION_STATUS['selesai']) {
                //    $this->addErrorLog(false, '<strong>' . $rsHeader[0]['code'] . '</strong>. ' . $this->errorMsg[201] . '<br><strong>' . $rsSPK[0]['code'] . '</strong>, ' .  $this->errorMsg[204]);
                //}

                //cek semua status di SPK, sama gk semua detailnya   
      		if($rsHeader[0]['iscostcargo'] == 0) { //dari cost cargo atau bukan
                    $rsSPKCost = $truckingServiceWorkOrder->getCostDetail($rsHeader[0]['refkey']); 
                } else {
                    $rsSPKCost = $truckingServiceWorkOrder->getCargoCostDetail('', $rsHeader[0]['refkey']);
                }
                for($i=0;$i<count($rsDetail);$i++){
                    ...
                    for($j=0;$j<count($rsSPKCost);$j++){
                        ...
                        $total = $rsSPKCost[$j]['total'];
                        
                        // khusus kargo
                        if($rsHeader[0]['iscostcargo'] == 1) {  
                            $total = ($rsSPKCost[$j]['ismultipliedqty'] == 1) ? $rsSPKCost[$j]['qty'] * $rsSPKCost[$j]['price'] : $rsSPKCost[$j]['price'];
                        }
                        
                        if ($rsDetail[$i]['refheadercostkey'] == $rsSPKCost[$j]['pkey']){ 
                            $foundTheCost = true;
                            
                            $fieldName = ($onRequest) ?  'refrequestkey' : 'refcashoutkey';
                            
                            if(!empty($rsSPKCost[$j][$fieldName])) 
                                $this->addErrorLog(false,'<strong>'.$rsSPKCost[$j]['name'].'</strong>. ' . $this->errorMsg['truckingServiceWorkOrder'][3]); 
                            else if ($rsDetail[$i]['costkey'] != $rsSPKCost[$j]['costkey'] || $rsDetail[$i]['amount'] != $total)  // <-- dulu $rsSPKCost[$j]['requestamount']
        ...
        $rsCashOutKey = $this->getTableKeyAndObj($truckingServiceOrder->tableName,array('key')); 
         
        
        if ($rsHeader[0]['reftabletype'] == $rsCashOutKey['key']){   
            $rsJobOrder = $truckingServiceOrder->getDataRowById($rsHeader[0]['refkey']); 
            ...
            if ($rsJobOrder[0]['statuskey'] < 2 || $rsJobOrder[0]['statuskey'] > 6){ 
                ...
                $rsJobOrderCost = $truckingServiceOrder->getHeaderCost($rsHeader[0]['refkey']); 
                
//                $this->setLog($rsJobOrderCost,true);
                ...
                for($i=0;$i<count($rsDetail);$i++){
                    for($j=0;$j<count($rsJobOrderCost);$j++){
                        ...
                        if ($rsDetail[$i]['refheadercostkey'] == $rsJobOrderCost[$j]['pkey']){ 
                            ...
                            if(!empty($rsJobOrderCost[$j][$fieldName])) 
...
//             $this->addErrorLog(false,'<strong>'.$rsJobOrderCost[$j]['itemname'].'</strong>'); 
    ...
    function cancelTrans($rsHeader,$copy){   
      ...
      if($rsHeader[0]['iscostcargo'] == 0) {
        ...
        for($i=0;$i<count($rsDetail);$i++){
             ...
             if ( $rsDetail[$i]['costkey'] == DEFAULT_COST['outsourceDownpayment']) {
               $sql = 'update '.$truckingServiceWorkOrder->tableName.' set refcashoutdownpaymentkey = 0 where pkey = ' . $rsHeader[0]['refkey']; 
            }else{ 
                $sql = 'update '.$tableName.' set refcashoutkey = 0, isrealization = 0  where pkey = ' . $rsDetail[$i]['refheadercostkey'];
            } 
            
            $this->oDbCon->execute($sql); 
            
            
            // dipindah di TCO request
//            $sql = 'update '.$tableName.' set refrequestkey = 0 where pkey = ' . $rsDetail[$i]['refheadercostkey'];
//            $this->oDbCon->execute($sql); 
    ...
    function validateCancel($rsHeader,$autoChangeStatus=false){ 
        ...
        if(in_array($rsHeader[0]['statuskey'], array(2,3,4))){  
            
            // UTK JO sepertinya boleh dicancel kapan saja
            
            // sementara batasin dari status saja dulu, biar gk byk query
            if ($rsHeader[0]['statuskey'] == 2)
                 $this->addErrorLog(false,'<strong>'.$rsHeader[0]['code'].'</strong>. ' . $this->errorMsg[201].' '.  $this->errorMsg['truckingCostCashOut'][2]); 
    ...
    function generateDefaultQueryForAutoComplete($returnField)
    {
        ...
        $sql = 'select
                    ...
                    ' . $this->tableName . '.code,
                    ' . $this->tableName . '.subtotal as subtotal,
                    ' . $this->tableName . '.trdate,
                    ' . $this->tableStatus . '.status as statusname,
                    ' . $this->tableEmployee . '.name as employeename ,
		...
		$sql = 'select 
				...
				where  	
                    ...
                    '.$this->getWarehouseCriteria($this->tableAlias).'
                        ...
                        left join '.$this->tableConsignee.' on  '.$this->tableAlias. '.consigneekey = '.$this->tableConsignee.'.pkey
    ...
    function addCashBankRealization($rsHeader,$rsDetail,$tableName){
        ...
        $rsWorkOrderKey = $truckingServiceWorkOrder->getTableKeyAndObj($truckingServiceWorkOrder->tableName,array('key')); 
 ...
 function addCashBank($rsHeader,$rsDetail){
	...
	function getTransactionDescription($arrKey,$userkey= ''){
		...
		$arrAvailableField = array( 
                                    array('tableName' => $this->tableName, 'param' => 'JO_CODE', 'field' => $this->tableSalesOrder.'.code'),
                                    array('tableName' => $this->tableName, 'param' => 'WO_CODE', 'field' => $this->tableSalesWorkOrder.'.code'),
                                    array('tableName' => $this->tableName, 'param' => 'EMPLOYEE_NAME', 'field' => $this->tableEmployee.'.name'), 
                                    array('tableName' => $this->tableName, 'param' => 'DESCRIPTION', 'field' => $this->tableName.'.trdesc'), 
                                    array('tableName' => $this->tableName, 'param' => 'CUSTOMER_NAME', 'field' => $this->tableCustomer.'.name'), 
                                    array('tableName' => $this->tableName, 'param' => 'LOCATION', 'field' => $this->tableLocation.'.name'),  
                                    array('tableName' => $this->tableName, 'param' => 'CONSIGNEE_NAME', 'field' => $this->tableConsignee.'.name'),  
                                    array('tableName' => $this->tableName, 'param' => 'ROUTE', 'field' => 'concat_ws(\'-\','.$this->tableSalesWorkOrder.'.routefrom,'.$this->tableSalesWorkOrder.'.routeto)'), 
//                                    array('code' => 'costdetails', 'param' => 'COST_DETAILS', 'dataset' => 'costDetails'),  
	...
	function updateDocumentFiles($pkey, $fieldName, $arrFile){
		  ...
		  try{		
			...
			if($rsHeader[0]['statuskey'] == 3){ // khusus kalo status "konfirmasi", TCO ad 5 status
