...
class HospitalServiceOrderInvoice extends BaseClass{
	
    function __construct(){

    parent::__construct();

    $this->tableName = 'hospital_service_order_invoice_header';
    $this->tableNameDetail = 'hospital_service_order_invoice_detail';
    $this->tableNameItemDetail = 'hospital_service_order_invoice_item_detail';
    $this->tableCustomer = 'customer';
    $this->tableItem = 'item';
    $this->tableStatus = 'transaction_status';
    $this->tableWarehouse = 'warehouse';   
    $this->tablePayment= 'hospital_service_order_invoice_payment'; 
    $this->tableSalesOrder= 'hospital_job_order_header';
    $this->tableWorkOrder= 'hospital_work_order_header';    
    $this->tableConsignee= 'consignee'; 
    $this->tablePaymentMethod = 'payment_method';
    $this->tableDownpaymentDetail = 'hospital_service_order_invoice_downpayment';
    ...
    $this->securityObject = 'HospitalServiceOrderInvoice';
    $this->tableCOA  = 'chart_of_account';

//    $this->autoPrintURL = 'print/hospitalServiceOrderInvoice';
    ...
    array_push($this->printMenu,array('code' => 'printInvoice', 'name' => $this->lang['printInvoice'],  'icon' => 'print', 'url' => 'print/hospitalServiceOrderInvoice'));
    ...
    function reCountGrandTotal($arrParam){
        ...
        for ($i=0;$i<count($arrPick);$i++){ 
                ...
                if (empty($arrSalesOrderKey[$i]) || $arrParam['chkDownpayment'] ){  
                    ...
                    $rsPrice = $hospitalJobOrder->getUnInvoicedItemDetail($sokey);
    ...
    function validateForm($arr,$pkey = ''){ 
        ...
        for($i=0;$i<count($arrSalesOrderKey);$i++) {   
            ...
            if ( (!empty($arrSalesOrderKey[$i]) ) && !empty($arrPick[$i]) )  {
                ...
                $rsSO = $hospitalJobOrder->getDataRowById($arrSalesOrderKey[$i]);
            ...
            if($isReimburse == 1){
                ...
                for($i=0;$i<count($rsDetail);$i++){ 
                    ...
                    if ($rsDetail[$i]['invoicetype'] == 2){
                        ...
                        // kalo sales order
                        $rsJobOrder = $hospitalJobOrder->getDataRowById($rsDetail[$i]['salesorderkey']);
...
/*        $rsCOA = $coaLink->getCOALink ('salesservicediscount', $warehouse->tableName,$warehousekey, 0);
    ...
    function getDetailWithRelatedInformation($pkey,$criteria=''){
        $sql = 'select
            ...
            '.$this->tableSalesOrder.'.patientname ,
            ...
            '.$this->tableNameDetail.'
                left join 
                    (select 
                        ...
                        '.$this->tableSalesOrder.'.donumber, 
                        '.$this->tableSalesOrder.'.patientkey, 
                        '.$this->tableSalesOrder.'.trdate,  
                        '.$this->tableSalesOrder.'.lastwodate,  
                        '.$this->tableCustomerInsurancePolicy.'.name as patientname 
                    from 
                        '.$this->tableSalesOrder.' 
                            left join '.$this->tableCustomerInsurancePolicy.' on '.$this->tableSalesOrder.'.patientkey = '.$this->tableCustomerInsurancePolicy.'.pkey
...
//        $this->setLog($arrParam,true);
        ...
        if($fromAPI){ 
            ...
            if(isset($arrParam['hidRefSODetailKey'])){ // kalo dr sodetailkey langsung 
                ...
                // selling cost
                $sql = 'select '.$hospitalJobOrder->tableSellingCost.'.pkey,'.$hospitalJobOrder->tableSellingCost.'.costkey as itemkey,'.$hospitalJobOrder->tableSellingCost.'.price,'.$hospitalJobOrder->tableSellingCost.'.requestid 
                        from '.$hospitalJobOrder->tableSellingCost.' 
                        where '.$hospitalJobOrder->tableSellingCost.'.requestid in ('. $this->oDbCon->paramString($arrParam['refsorequestid'],',').')';
                $rs = $this->oDbCon->doQuery($sql);
                $rs = array_column($rs,null,'requestid');
                $rsJODetail += $rs;
                    
                // services
                $sql = 'select '.$hospitalJobOrder->tableNameDetail.'.pkey,'.$hospitalJobOrder->tableNameDetail.'.itemkey,'.$hospitalJobOrder->tableNameDetail.'.priceinunit as price,'.$hospitalJobOrder->tableNameDetail.'.requestid
                        from '.$hospitalJobOrder->tableNameDetail.' 
                        where '.$hospitalJobOrder->tableNameDetail.'.requestid in ('. $this->oDbCon->paramString($arrParam['refsorequestid'],',').')';
                $rs = $this->oDbCon->doQuery($sql);
                $rs = array_column($rs,null,'requestid');
                $rsJODetail += $rs;
        ...
        for($i=0;$i<count($detail);$i++){
            ...
            if($arrParam['selInvoiceType'][$i] == 1){
                ...
                $rsJO = $hospitalJobOrder->getDataRowById($arrParam['hidSalesOrderKey'][$i]);
                if(!empty($rsJO)){
       ...
       // if (!$rsHeader[0]['isdownpayment']){   
            foreach($rsDetail as $invoiceDetail){  
               if (empty($invoiceDetail['salesorderkey']))    continue;
                ...
                // $this->setLog("in ===>", true);
               $hospitalJobOrder->updateQtyInvoiced($invoiceDetail['salesorderkey']); // update jml yg sudah diinvoiced
               $hospitalJobOrder->updateAmountInvoiced($invoiceDetail['salesorderkey']); // update nilai yg sudah diinvoiced
...
/*         parent::afterPrintTransaction($rsHeader);    
    ...
    function validateConfirm($rsHeader){ 
        ...
        for($i=0;$i<count($rsDetail);$i++){
            ...
            $rsSO  = $hospitalJobOrder->searchDataRow(array($hospitalJobOrder->tableName.'.pkey',$hospitalJobOrder->tableName.'.code',$hospitalJobOrder->tableName.'.statuskey',$hospitalJobOrder->tableName.'.isapi'),
                                                            ' and ' .$hospitalJobOrder->tableName.'.pkey = ' .  $this->oDbCon->paramString($rsDetail[$i]['salesorderkey'])
                                                           );
       ...
       /*     $minStatusJOToInvoiced = $this->loadSetting('minimumJOStatusToInvoiced');
            ...
            if ($rsSO[0]['statuskey'] < $minStatusJOToInvoiced || $rsSO[0]['statuskey'] > 6){ 
                ...
                $rsUnInvoiced = $hospitalJobOrder->getUnInvoicedItemDetail($rsDetail[$i]['salesorderkey']); 
            ...
            $totalInvoiced = $hospitalJobOrder->getTotalInvoicedAndOutstanding($rsDetail[$i]['salesorderkey'], $rsHeader[0]['customcodekey']); 
    ...
    function addPartialInvoice($rsHeader,$rsDetail){ 
        ...
        foreach ($rsDetail as $invoiceDetail) { 
            ...
            if ($rsHeader[0]['isdownpayment']){
                ...
                $rsDP = $hospitalJobOrder->getTotalInvoicedAndOutstanding($invoiceDetail['salesorderkey'],$rsHeader[0]['customcodekey']);
...
//        $truckingServiceOrderCategory = new TruckingServiceOrderCategory();
        ...
        foreach ($rsDetail as $invoiceDetail) {
            ...
            $rsJO = $hospitalJobOrder->getDataRowById($jobOrderKey);
...
//                foreach ($rsJobType as $jobType) {
                    ...
                    //                                '.$hospitalWorkOrder->tableName.'.jobtypekey = '.$this->oDbCon->paramString($jobType['jobtypekey']).' and 
                    ...
                    $sql = 'update 
                            ...
                            where
                                '.$hospitalWorkOrder->tableName.'.refdetailkey = '.$this->oDbCon->paramString($itemDetail['refsodetailkey']).' and
                                '.$hospitalWorkOrder->tableName.'.invoicekey = 0 and
                                '.$hospitalWorkOrder->tableName.'.statuskey = 3
...
//                }
        ...
        // add to car turnover
        $rsWorkOrder = $hospitalWorkOrder->searchData($hospitalWorkOrder->tableName.'.invoicekey' , $rsHeader[0]['pkey'], true); 
        
        $rsObjKey = $this->getTableKeyAndObj($this->tableName);   
        foreach($rsWorkOrder as $workOrder){
            $rsJO = $hospitalJobOrder->getDataRowById($workOrder['refkey']);
    ...
    function updateVendorCommission($rsHeader,$rsDetail){
        ...
        $rsWorkOrder = $hospitalWorkOrder->searchData($hospitalWorkOrder->tableName.'.invoicekey' , $rsHeader[0]['pkey']); 
        ...
        foreach($rsWorkOrder as $workOrder){
            ...
            $totalcost = 0;
            $rsCost = $hospitalWorkOrder->getCostDetail($workOrder['pkey'],'',' and '.$hospitalWorkOrder->tableItem.'.shareprofit = 1');
	...
	function cancelTrans($rsHeader,$copy){ 
		...
		if ($rsHeader[0]['statuskey'] == 1) 
		 ...
		 for($i=0;$i<count($rsDetail);$i++){
            ...
            $rsSO =  $hospitalJobOrder->getDataRowById($soKey);
            if ($rsSO[0]['statuskey'] == 6)
                $hospitalJobOrder->changeStatus($soKey,5,'',false,true); 
        } 
       ...
       $rsAPVendor = $ap->searchData('','',true,' and '.$ap->tableName.'.reftabletype = '.$this->oDbCon->paramString($rsObjKey['key']).' and '.$ap->tableName.'.refkey = '.$this->oDbCon->paramString($id).' and '.$ap->tableName.'.statuskey = 1');
        ...
        // cancel car turnover
        $sql = 'update '.$hospitalWorkOrder->tableName.' set  invoicekey = 0, sellingprice = 0 where  invoicekey = ' . $this->oDbCon->paramString($id); 
        $this->oDbCon->execute($sql); 
        ...
        for($i=0;$i<count($rsDetail);$i++){
            ...
            for($j=0;$j<count($rsServiceDetail);$j++){ 
                if (!empty($rsServiceDetail[$j]['refsodetailkey'])){
                    $rsSO = $hospitalJobOrder->getDataRowById();
