<?php
class Packaging extends BaseClass
{
    ...
    function validateForm($arr, $pkey = '')
    {
        ...
        $packagingName = $arr['name'];

        $rs = $this->isValueExisted($pkey, 'name', $packagingName);
        if (empty($packagingName)) {
            $this->addErrorList($arrayToJs, false, $this->errorMsg['packaging'][1]);
        } else if (count($rs) <> 0) {
            $this->addErrorList($arrayToJs, false, $this->errorMsg['packaging'][2]);
        }
