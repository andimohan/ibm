...
class Warehouse extends BaseClass
{
    ...
    function __construct()
    {
        ...
        $this->arrData['isnonlabeling'] = array('chkIsNonLabeling');
    ...
    function updateCOALink($warehouseKey, $arrParam)
    {
        ...
        $rsCOA = $chartOfAccount->searchDataRow(
            array($chartOfAccount->tableName . '.pkey', $chartOfAccount->tableName . '.isleaf'),
        ...
        if (!empty($arrParam['coalink'])) {
            for ($i = 0; $i < count($arrParam['coalink']); $i++) {
                if ($arrIsLeaf[$arrParam['hidcoakey'][$i]] == 0)
                    continue;
    ...
    function generateComboboxOpt($opt = array(), $queryOpt = array(), $preselected = '', $relOpt = array())
    {
        ...
        if (isset($queryOpt['criteria']))
            $queryOpt['criteria'] .= $this->getCompanyCriteria() . $this->getWarehouseCriteria();
