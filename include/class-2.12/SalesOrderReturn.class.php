...
class SalesOrderReturn extends BaseClass {
   ...
   function __construct()
   {
      ...
      $this->tableName = 'sales_order_return_header';
      $this->tableNameDetail = 'sales_order_return_detail';
      ...
      $this->securityObject = 'SalesOrderReturn';
      ...
      $this->arrDataDetail['returnqtyinbaseunit'] = array('returnQtyInBaseUnit', 'number');
      ...
      array_push($this->arrSearchColumn, array('Kode SO', $this->tableSalesOrderHeader . '.code'));
   ...
   function validateForm($arr, $pkey = '')
   {
      ...
      //$rsSalesOrder = $salesOrder->getDetailById($salesOrderKey);
      ...
      for ($i = 0; $i < count($arrItemKey); $i++) {
         ...
         if (!empty($arrItemKey[$i]) && $this->unFormatNumber($arrReturnQtyInBaseUnit[$i]) > $this->unFormatNumber($arrQtyMinusInBaseUnit[$i])) {
            $rsItem = $item->getDataRowById($arrItemKey[$i]);
            $this->addErrorList($arrayToJs, false, $rsItem[0]['name'] . '. ' . $this->errorMsg['salesOrderReturn'][1]);
         }
      }
   ...
   function validateConfirm($rsHeader)
   {
      ...
      //cek apakah return melebihi jml yang terkirim ?
      for($j=0; $j < count($rsDetail); $j++) {

         $rsReturn = $salesOrder->getSalesOrderReturn($rsDetail[$j]['refsodetailkey']);
         $arrReturn = array_column($rsReturn, null, 'joinkey');
         $joinKey = $rsDetail[$j]['refsodetailkey'] . '-' . $rsDetail[$j]['itemkey'];
         $rsSODetail = isset($arrReturn[$joinKey]) ? $arrReturn[$joinKey] : array();

         if(!empty($rsSODetail) && ($rsDetail[$j]['itemkey'] == $rsSODetail['itemkey']) && ($rsDetail[$j]['returnqtyinbaseunit'] > $rsSODetail['outstandingqty'])) {
            $this->addErrorLog(false, '<strong>' . $rsHeader[0]['code'] . '. ' . $rsSODetail['itemname'] . '</strong>. ' .  'Jumlah retur tidak boleh melebihi yang terkirim');
         }
      }
   ...
   {
      $salesOrder = new SalesOrder();
      $salesOrder->updateSalesOrderReturnItem($rsHeader[0]['refkey']);
   ...
   function confirmTrans($rsHeader)
   {
      ...
      $note = $rsHeader[0]['code'] . '. ' . $this->ucFirst($this->lang['salesReturn'] . ' ' . $this->lang['from']) . ' ' . $rsSalesOrderHeader[0]['code'] . ', ' . $rsCustomer[0]['name'] . '.';
      ...
      for ($i = 0; $i < count($rsDetail); $i++) {
         if ($rsDetail[$i]['returnqtyinbaseunit'] != 0)
            $itemMovement->updateItemMovement($id, $rsDetail[$i]['itemkey'], $rsDetail[$i]['returnqtyinbaseunit'], $rsDetail[$i]['costinbaseunit'], $this->tableName, $rsHeader[0]['warehousekey'], $note, $rsHeader[0]['trdate']);
      }
   ...
   function updateGL($rs)
   {
      ...
      $arr['trDesc'] = $this->ucFirst($this->lang['salesReturn'] . ' ' . $this->lang['from']) . ' ' . $rsPO[0]['code'] . ', ' . $rsCustomer[0]['name'] . '.';
      ...
      for ($i = 0; $i < count($rsDetail); $i++) {
         ...
         $itemTempCOAKey = $item->getInventoryTempCOAKey($rsDetail[$i]['itemkey'], $warehousekey);//ini untuk COA untuk menampung return nya

         //$rsPODetail = $purchaseOrder->getDetailByColumn('pkey',$rsDetail[$i]['refpodetailkey']);

         $totalItemValue = $rsDetail[$i]['returnqtyinbaseunit'] * $rsDetail[$i]['costinbaseunit'];
