function DisposalSalesWasteInvoice(tabID, cashTOP, varConstant) {
    var thisObj = this;
    var tabObj = $("#" + tabID);
    ...
    this.updateRowInformation = function updateRowInformation(detailRow, objAndValue, ui) {
        ...
        for (i = 0; i < objAndValue.length; i++) {
            ...
            console.log(ui.item[objAndValue[i].value]);
        ...
        detailRow.find("[name='wasteCode[]']").first().val(ui.item['value']);
    }
    ...
    this.updatePartialInvoiceOutstanding = function updatePartialInvoiceOutstanding(detailRow) {
        ...
        if (soKey) {
            ...
            $.ajax({
                type: "GET",
                url: 'ajax-disposal-job-order.php',
                async: false,
            ...
            success: function (data) {
                ...
                row.each(function () {
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        arrPostValue.push({ "selector": "WOCode",  "value": data[i].wocode });
                        arrPostValue.push({ "selector": "hidWasteKey",  "value": data[i].wastekey });
                        arrPostValue.push({ "selector": "wasteCode",  "value": data[i].wastecode });
                        arrPostValue.push({ "selector": "manifestCode",  "value": data[i].manifestcode });
                        arrPostValue.push({ "selector": "hidWODetailKey", "value": data[i].refdetailwokey  });
                        arrPostValue.push({ "selector": "hidWOKey", "value": data[i].wokey  });
                        arrPostValue.push({ "selector": "WODate",  "value": moment(data[i].trdate).format(_DATE_FORMAT_)  });
                        arrPostValue.push({ "selector": "disposalWeight",  "value": data[i].disposalweight    });
                        arrPostValue.push({ "selector": "maximumWeight",  "value": data[i].quotaweight });
                              
                        // var chargedWeight =   data[i].disposalweight - data[i].quotaweight;
                        var chargedWeight =   data[i].chargedweight;
                        chargedWeight = Math.ceil(chargedWeight);
                        
                        if (chargedWeight < 0 ) chargedWeight = 0;  
 
                        arrPostValue.push({ "selector": "chargedWeight", "value": chargedWeight }); 
                        arrPostValue.push({ "selector": "exceedPriceArea", "value": data[i].exceedprice });
                        arrPostValue.push({ "selector": "exceedWeightPriceArea","value": data[i].priceinunit });
                        ...
                        // thisObj.calculateServiceDetail(newrow);
                    }
                });
...
//    
//        var disposalWeight = parseFloat(unformatCurrency(serviceRow.find("[name='disposalWeight[]']").val())) || 0;
//        var maximumWeight = parseFloat(unformatCurrency(serviceRow.find("[name='maximumWeight[]']").val())) || 0;
//        
//        var exceedPriceArea = parseFloat(unformatCurrency(serviceRow.find("[name='exceedPriceArea[]']").val())) || 0;
//        var exceedWeightPriceArea = parseFloat(unformatCurrency(serviceRow.find("[name='exceedWeightPriceArea[]']").val())) || 0;
...
//        
//        var weight = 0 ;
//        if (disposalWeight > maximumWeight) 
//            weight = disposalWeight - maximumWeight;
//    
...
//    }
    ...
    this.calculateDetail = function calculateDetail(obj)
    {
        ...
        var qty = unformatCurrency(row.find("[name='quantity[]']").val());
        var price = unformatCurrency(row.find("[name='priceInUnit[]']").val());

        var subtotal = qty * price;

        row.find("[name='amount[]']").val(subtotal).blur();
    ...
    this.calculateTotal = function calculateTotal() {
        ...
        if ($(this).val() != 1) return;
            ...
            objSubtotal = row.find("[name='detailOrderSubtotal[]']");
            objDownpayment = row.find("[name='salesOrderDownpayment[]']");
            // servicePrice = row.find("[name='servicePrice[]']");
            servicePrice = parseFloat(unformatCurrency(row.find("[name='servicePrice[]']").val())) || 0;
            ...
            if (invoiceType == 1 && !downpaymentType) {
                ...
                row.find("[name='chkService[]']").each(function () {
                    ...
                    var disposalWeight = parseFloat(unformatCurrency(serviceRow.find("[name='disposalWeight[]']").val())) || 0;
                    var maximumWeight = parseFloat(unformatCurrency(serviceRow.find("[name='maximumWeight[]']").val())) || 0;
                    var chargedWeight = parseFloat(unformatCurrency(serviceRow.find("[name='chargedWeight[]']").val())) || 0;
                    var exceedPriceArea = parseFloat(unformatCurrency(serviceRow.find("[name='exceedPriceArea[]']").val())) || 0;
                    var exceedWeightPriceArea = parseFloat(unformatCurrency(serviceRow.find("[name='exceedWeightPriceArea[]']").val())) || 0;
                    
                    // if (disposalWeight > maximumWeight) 
                    //     weight = Math.ceil(disposalWeight - maximumWeight);
                    weight = Math.ceil(chargedWeight);
                     
                    var beforeTaxDetail = parseInt(unformatCurrency(serviceRow.find("[name='beforeTaxDetail[]']").val())) || 0;
                    var subtotalDetail = weight * exceedWeightPriceArea + exceedPriceArea;
                    serviceRow.find("[name='subtotalDetail[]']").val(subtotalDetail).blur();
                ...
                rowTotal = rowTotal + servicePrice;
    ...
    this.rebindEl = function rebindEl() {
        ...
        bindAutoCompleteForTransactionDetail('wasteCode[]', objAndValueForDetailAutoComplete, 'ajax-waste.php?action=searchData', thisObj.updateDetail);
        // bindAutoCompleteForTransactionDetail('salesOrderCode[]', objAndValueForDetailAutoComplete, 'ajax-disposal-job-order.php?action=searchDataForInvoice&statustype=' + statustype + '&customerkey=' + customerkey);
        bindAutoCompleteForTransactionDetail('itemUnitName[]', objAndValueForDetailItemAutoComplete, 'ajax-item-unit.php?action=searchData');
...
//        });
        ...
        bindEl(tabObj.find("[name='quantity[]']"), 'change', function () {
            thisObj.calculateDetail(this);
        });
        bindEl(tabObj.find("[name='priceInUnit[]']"), 'change', function () {
            thisObj.calculateDetail(this);
        });
