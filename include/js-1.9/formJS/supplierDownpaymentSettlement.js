function SupplierDownpaymentSettlement(tabID,tablekey, rs,varConstant){   
    var thisObj = this;
    var tabObj = $("#" + tabID);      
...
//            }
//
//            detailRow.find("[name=\"dpCode[]\"]").first().val(ui.item['code']);
...
//    }
    ...
    this.rebindDownpaymentCode = function rebindDownpaymentCode(){
        bindAutoCompleteForTransactionDetail('dpCode[]',  objAndValueForDetailAutoComplete,'ajax-supplier-downpayment.php?action=searchData&supplierkey=' + tabObj.find("[name=hidSupplierKey]").val()+'&currencykey='+ tabObj.find("[name=selCurrency]").val(),thisObj.updateDetail); 
    } 
    ...
    this.importData = function importData(){ 
        ...
        var ajaxData = "action=searchData&currencykey=" + tabObj.find("[name=selCurrency]").val() + "&supplierkey=" + tabObj.find("[name=hidSupplierKey]").val()+dateParam;  
