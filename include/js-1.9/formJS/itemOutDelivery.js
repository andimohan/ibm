function ItemOutDelivery(tabID,rs){   
        var thisObj = this;
        var tabObj = $("#" + tabID);      
        ...
        this.importData =  function importData(){ 
                ...
                $.ajax({
                    ...
                    data: "action=getRelatedInformation&pkey=" +  tabObj.find("[name=hidItemOutKey]" ).val() ,  
                ...
                $.ajax({
                    ...
                    data: "action=getOutstandingDetail&pkey=" +  tabObj.find("[name=hidItemOutKey]" ).val() ,  
                    success: function(data){ 
                            if (!data) return;
                            var data = JSON.parse(data);  
                            ...
                            for(i=0;i<data.length;i++){    
                                    ...
                                    var arrPostValue = []; 
                                    arrPostValue.push({"selector":"hidItemOutDetailKey", "value":data[i].pkey});
                                    ...
                                    arrPostValue.push({"selector":"qtyMinusInBaseUnit", "value":qtyInBaseUnit - deliveredQtyInBaseUnit}); 
                                    arrPostValue.push({"selector":"deliveredQtyInBaseUnit", "value":qtyInBaseUnit - deliveredQtyInBaseUnit}); 
        ...
        this.updateItemOutDeliveryMinusQty = function updateItemOutDeliveryMinusQty(){  
            ...
            $.ajax({ 
                    ...
                    success: function(data){  
                        ...
                        for(i=0;i<data.length;i++){  
                           ...
                           row.find("[name=\"orderedQtyInBaseUnit[]\"]").first().val(data[i].qtyinbaseunit); 
                           row.find("[name=\"qtyMinusInBaseUnit[]\"]").first().val(data[i].qtyinbaseunit - data[i].deliveredqtyinbaseunit); 
                                 
                           if(data[i].qtyinbaseunit != data[i].deliveredqtyinbaseunit)
        ...
        this.calculateSNNeeded = function calculateSNNeeded(target){
            ...
            detailRow.each(function(){  
                ...
                totalQty = detailRow.find("[name='deliveredQtyInBaseUnit[]']").val();
        ...
        this.rebindEl = function rebindEl(){  
            bindEl(tabObj.find("[name='deliveredQtyInBaseUnit[]']"),'change', function() { thisObj.calculateSNNeeded(this); });  
        ...
        this.loadOnReady = function loadOnReady(){
            ...
            if (thisObj.rs && thisObj.rs.statuskey == 1)
                thisObj.updateItemOutDeliveryMinusQty();
